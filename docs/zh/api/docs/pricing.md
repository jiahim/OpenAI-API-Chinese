...
| gpt-5.4（<272K 上下文长度） | $2.50 | $0.25 | - | $15.00 | $5.00 | $0.50 | - | $22.50 |
...
区域处理（数据驻留）端点对 2026 年 3 月 5 日及之后发布、且符合数据驻留条件的模型加收 10% 的附加费。详情请参阅 [数据处理](https://developers.openai.com/api/docs/guides/your-data) 指南，了解支持的区域和处理详情。 [Amazon Bedrock 中的 OpenAI 模型](https://developers.openai.com/api/docs/guides/amazon-bedrock) 通过 AWS 计费。商业区域的 Bedrock 定价与 OpenAI 等效服务的直接定价一致。Priority processing 已于 2026 年 7 月 30 日更名为 Fast mode。你可以在 API 请求中使用 `service_tier: "priority"` 或 `service_tier: "fast"` 指定。 [详细了解 Fast mode](https://developers.openai.com/api/docs/guides/fast-mode)。GPT-5.6 Sol 的促销定价至少持续到 2026 年 11 月 21 日。
...
区域处理（数据驻留）端点对 2026 年 3 月 5 日及之后发布、且符合数据驻留条件的模型加收 10% 的附加费。详情请参阅 [数据处理](https://developers.openai.com/api/docs/guides/your-data) 指南，了解支持的区域和处理详情。
...
Fast 模式不适用于 GPT-6 Astra 的 EU 数据驻留。请为这些请求使用 Standard 处理。参见 [Fast 模式兼容性](https://developers.openai.com/api/docs/guides/fast-mode)。对于符合数据驻留资格的、于 2026 年 3 月 5 日及之后发布的模型，区域处理（数据驻留）端点加收 10% 的附加费。请参阅我们的 [数据处理](https://developers.openai.com/api/docs/guides/your-data) 指南，了解支持的区域和处理详情。
...
| Containers | 托管 Shell 和 Code Interpreter | 1 GB $0.03，4 GB $0.12，16 GB $0.48，64 GB $1.92，按每个容器 20 分钟会话计费。 |
| 文件搜索 | Storage | $0.10 / GB 每天（1 GB 免费） |
| 文件搜索 | Tool call | $2.50 / 1k 次调用 |
| 智能体 Kit | ChatKit 文件和图像上传存储 | $0.10 / GB-day，账户每月首 1 GB 免费 |

每次 1,000 次调用收费 $10.00 + 搜索内容 token 按模型费率计费。

网页搜索预览版（推理模型，包括 `gpt-5`, `o-series`)

每次 1,000 次调用收费 $25.00 + 搜索内容 token 免费。

Hosted Shell 和 Code Interpreter

内置工具使用的 token 按所选模型的每 token 费率计费。GB 指二进制千兆字节（也称吉比字节），其中 1 GB 等于 2^30 字节。网页搜索内容 token 是从搜索索引中检索并与你的提示词一同提供给模型以生成答案的 token。对于使用非预览版 网页搜索 工具的 gpt-4o-mini 和 gpt-4.1-mini，搜索内容 token 按每次调用固定 8,000 个输入 token 的区块计费。文件搜索工具调用定价仅适用于 Responses API。容器定价包含 Hosted Shell 和 Code Interpreter。符合条件的容器会话将按分钟计费，每次会话最低计费 5 分钟。Responses API、Chat Completions API、Realtime API、Batch API 和 Assistants API 不单独计费。Token 按所选模型的输入和输出费率计费。
...
强化微调中用于模型评分的 token 按该模型的逐 token 费率计费。如果你在创建微调任务时启用数据共享，可享受推理折扣。了解更多信息。