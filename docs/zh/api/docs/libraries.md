...
本页介绍使用以下方式构建的主要方法： [OpenAI API](https://developers.openai.com/api/reference/overview)：用于应用程序代码的官方 SDK、适用于 Shell 原生工作流的 OpenAI CLI、用于编排的 Agents SDK，或你常用的任何 HTTP 客户端。

## 创建并导出 API 密钥

开始之前， [在仪表板中创建一个 API 密钥](https://platform.openai.com/api-keys)，你将使用它来安全地 [访问 API](https://developers.openai.com/api/reference/overview)。请将该密钥保存在安全的位置，例如计算机上的一个 [`.zshrc` 文件](https://www.freecodecamp.org/news/how-do-zsh-configuration-files-work/) 或其他文本文件。生成 API 密钥后，请在终端中将其导出为 [环境变量](https://en.wikipedia.org/wiki/Environment_variable) 。
...
OpenAI SDK 已配置为自动从系统环境变量中读取你的 API 密钥。
...
在 Node.js、Deno 或 Bun 等 服务端 JavaScript 环境中使用 OpenAI API 时，你可以使用官方的 [适用于 TypeScript 和 JavaScript 的 OpenAI SDK](https://github.com/openai/openai-node)。首先使用 [npm](https://www.npmjs.com/) 或你常用的包管理器安装该 SDK：
...
安装好 OpenAI SDK 后，新建一个名为 `example.mjs` 的文件，并将下面的示例代码复制进去：
...
在 Python 中使用 OpenAI API 时，你可以使用官方的 [适用于 Python 的 OpenAI SDK](https://github.com/openai/openai-python)。首先使用 [pip](https://pypi.org/project/pip/):
...
安装好 OpenAI SDK 后，新建一个名为 `example.py` 的文件，并将下面的示例代码复制进去：
...
该公司 与 Microsoft 合作提供了一个官方支持的 C# OpenAI API 客户端。你可以使用 .NET CLI 从 [NuGet](https://www.nuget.org/).

```
dotnet add package OpenAI
```

一个对API的简单请求示例如下（请求目标是 [Responses API](https://developers.openai.com/api/reference/resources/responses) ）：
...
OpenAI 为 Java 语言提供了一个 API 帮助库，目前处于 beta 阶段。你可以通过以下配置添加 Maven 依赖：
...
OpenAI 为 Go 语言提供了一个 API 帮助库，目前处于 beta 阶段。你可以使用以下代码导入该库：
...
对API的首次请求示例如下（请求目标是 [Responses API](https://developers.openai.com/api/reference/resources/responses) ）：
...
安装好 OpenAI SDK 后，新建一个名为 `example.rb` 的文件，并将下面的示例代码复制进去：
...
使用 CLI 处理可重复的终端工作流，例如从文件中提取结构化数据、生成图像、创建语音，以及配合 shell 工具组合 API 调用 `jq`.
...
请使用上述官方 OpenAI SDK 直接发起 API 请求。当你的应用需要以代码优先的方式编排智能体、工具时，请使用 Agents SDK
智能体、工具、
交接、护栏、追踪 或沙箱执行时，请使用 智能体开发工具包。

如果你正在权衡直接发起 API 请求和以代码优先的方式进行编排，
...
Microsoft 的 Azure 团队维护着同时兼容 OpenAI API 和 Azure OpenAI 服务的库。请阅读下面的库文档，了解如何将其与 OpenAI API 配合使用。

- [Azure OpenAI 客户端库（.NET）](https://github.com/Azure/azure-sdk-for-net/tree/main/sdk/openai/Azure.AI.OpenAI)
- [Azure OpenAI 客户端库（JavaScript）](https://github.com/Azure/azure-sdk-for-js/tree/main/sdk/openai/openai)
- [Azure OpenAI 客户端库（Java）](https://github.com/Azure/azure-sdk-for-java/tree/main/sdk/openai/azure-ai-openai)
- [Azure OpenAI 客户端库（Go）](https://github.com/Azure/azure-sdk-for-go/tree/main/sdk/ai/azopenai)

---

## 社区库

以下库由更广泛的开发者社区构建和维护。你也可以 [在 GitHub 上关注我们的 OpenAPI 规范仓库](https://github.com/openai/openai-openapi) ，以便及时了解我们对 API 所做更改的更新。
...
- [swarm](https://github.com/openai/swarm) - 用于教学的项目编排仓库