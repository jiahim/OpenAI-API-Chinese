...
> 如需查看完整文档索引，请参阅 [llms.txt](/llms.txt)。通过在页面 URL 末尾追加 `.md` 即可获取文档页面的 Markdown 版本。

本页介绍使用 [OpenAI API](https://developers.openai.com/api/reference/overview): 面向应用代码的官方 SDK、面向 shell 原生工作流的 OpenAI CLI、用于编排的 Agents SDK，或你惯用的任意 HTTP 客户端。

## 创建并导出 API 密钥

开始之前， [在仪表板中创建 API 密钥](https://platform.openai.com/api-keys)，你将用它来安全地 [访问 API](https://developers.openai.com/api/reference/overview)。请将该密钥保存在安全的位置，例如计算机上的一个 [`.zshrc` 文件](https://www.freecodecamp.org/news/how-do-zsh-configuration-files-work/) 或其他文本文件。生成 API 密钥后，将其导出为 [环境变量](https://en.wikipedia.org/wiki/Environment_variable) ：在终端里。
...
要在 Node.js、Deno 或 Bun 等服务端 JavaScript 环境中使用 OpenAI API，你可以使用官方的 [OpenAI 适用于 TypeScript 和 JavaScript 的 SDK](https://github.com/openai/openai-node)。先使用 [npm](https://www.npmjs.com/) 或你常用的包管理器安装 SDK：
...
安装好 OpenAI SDK 后，新建一个名为 `example.mjs` 的文件，并将示例代码复制到其中：
...
使用 `node example.mjs` （或 Deno、Bun 中对应的命令）执行代码。稍等片刻，你将看到 API 请求的输出。
...
要在 Python 中使用 OpenAI API，你可以使用官方的 [OpenAI 适用于 Python 的 SDK](https://github.com/openai/openai-python)。先使用 [pip](https://pypi.org/project/pip/):
...
安装好 OpenAI SDK 后，新建一个名为 `example.py` 的文件，并将示例代码复制到其中：
...
使用 `python example.py`。稍等片刻，你将看到 API 请求的输出。
...
该公司 与微软合作，为 C# 提供官方支持的 OpenAI API 客户端。你可以使用 .NET CLI 通过以下方式安装： [NuGet](https://www.nuget.org/).

```
dotnet add package OpenAI
```

一个向 API 发出的简单请求示例如下： [Responses API](https://developers.openai.com/api/reference/resources/responses) 代码如下：
...
OpenAI 为 Java 编程语言提供 API 辅助库，目前处于 beta 阶段。你可以使用以下配置引入 Maven 依赖：
...
一个向 API 发出的简单请求示例 [Responses API](https://developers.openai.com/api/reference/resources/responses) 代码如下：
...
OpenAI 为 Go 编程语言提供 API 辅助库，目前处于 beta 阶段。你可以使用以下代码导入该库：
...
一个向 API 发出的首次请求示例 [Responses API](https://developers.openai.com/api/reference/resources/responses) 代码如下：
...
安装好 OpenAI SDK 后，新建一个名为 `example.rb` 的文件，并将示例代码复制到其中：
...
使用 `ruby example.rb`。稍等片刻，你将看到 API 请求的输出。
...
要直接从终端调用 OpenAI API，请安装自动生成的 `openai` 命令行工具：
...
将 CLI 用于可重复的终端工作流，例如从文件中提取结构化数据、生成图像、创建语音，以及使用以下 shell 工具编写 API 调用 `jq`.
...
使用上述官方的 OpenAI SDK 进行直接的 API 请求。当你的应用需要以代码为先的编排时，使用 Agents SDK
用于 智能体、工具、
交接、护栏、追踪或沙箱执行。

如果你在直接的 API 请求与以代码为先的编排之间做选择，
...
以下库由更广泛的开发者社区构建和维护。你也可以 [在 GitHub 上关注我们的 OpenAPI 规范](https://github.com/openai/openai-openapi) 代码仓库，及时获取我们对 API 所做更改的更新。
...
- [async-openai](https://github.com/64bit/async-openai) 由 [64 位](https://github.com/64bit)
