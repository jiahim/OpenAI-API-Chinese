...
> 如需查看完整文档索引，请参阅 [llms.txt](/llms.txt)。可在页面 URL 末尾追加 `.md` 以获取文档页面的 Markdown 版本。

本页介绍使用 [OpenAI API](https://developers.openai.com/api/reference/overview)：用于应用代码的官方 SDK、用于 shell 原生工作流的 OpenAI CLI、用于编排的 Agents SDK，或你常用的任意 HTTP 客户端。

## 创建并导出 API 密钥

开始之前， [在控制台中创建一个 API 密钥](https://platform.openai.com/api-keys)，稍后你需要用它来安全地 [访问 API](https://developers.openai.com/api/reference/overview)。请将该密钥保存在安全的位置，例如保存为计算机上的一个 [`.zshrc` 文件](https://www.freecodecamp.org/news/how-do-zsh-configuration-files-work/) 或其他文本文件。生成 API 密钥后，请在终端中将其导出为 [环境变量](https://en.wikipedia.org/wiki/Environment_variable) ：
...
要在 Node.js、Deno 或 Bun 等 服务端 JavaScript 环境中使用 OpenAI API，可以使用官方的 [面向 TypeScript 和 JavaScript 的 OpenAI SDK](https://github.com/openai/openai-node)。首先使用 [npm](https://www.npmjs.com/) 或你常用的包管理器安装 SDK：
...
要在 Python 中使用 OpenAI API，可以使用官方的 [面向 Python 的 OpenAI SDK](https://github.com/openai/openai-python)。首先使用 [pip](https://pypi.org/project/pip/):
...
向 API 发出的简单请求 [Responses API](https://developers.openai.com/api/reference/resources/responses) 如下所示：
...
向 API 发出的首次请求 [Responses API](https://developers.openai.com/api/reference/resources/responses) 如下所示：
...
要在 Ruby 中使用 OpenAI API，你可以使用官方的 [OpenAI Ruby SDK](https://github.com/openai/openai-ruby)。首先在你的应用中添加该 gem：
...
使用 CLI 完成可重复的终端工作流，例如从文件中提取结构化数据、生成图像、创建语音，以及结合 shell 工具编写 API 调用 `jq`.
...
直接 OpenAI SDK 用于直接的 API 请求。在应用需要以代码为先的编排时使用 Agents SDK
用于智能体、工具、智能体 的以代码为先的编排。
交接、护栏、追踪 或沙箱执行。

如果你在直接的 API 请求和以代码为先的编排之间做选择，
...
Microsoft 的 Azure 团队维护了与 OpenAI API 和 Azure OpenAI 服务兼容的库。阅读下面的库文档，了解如何将它们与 OpenAI API 一起使用。
...
以下库由更广泛的开发者社区构建和维护。你也可以 [关注我们在 GitHub 上的 OpenAPI 规范](https://github.com/openai/openai-openapi) 仓库，以便及时获取我们对 API 所做更改的更新。
...
- [swarm](https://github.com/openai/swarm) - 用于教学的多智能体编排仓库