...
> 查看完整的文档索引，请参阅 [llms.txt](/llms.txt). 文档页面的 Markdown 版本可通过在页面 URL 后追加 `.md` 来获取。

## 概述

随着我们推出更安全、更强大的模型，会定期下线旧版模型。依赖 OpenAI 模型的软件可能需要偶尔更新才能继续使用。受影响的客户将通过电子邮件以及我们的文档收到通知，同时 [博客文章](https://openai.com/blog) 中了解重大变更。

此页面列出了所有 API 弃用项以及建议的替代方案。

## 模型弃用通知周期

我们会在模型下线前提前通知，以便客户有时间进行规划和迁移。当我们宣布模型弃用时，会通过邮件通知正在使用该模型的客户，并在本页记录相关弃用信息。

除非出于安全或合规方面的考虑需要加快进度，否则我们在模型下线前提供以下最短通知期限：

- **正式发布模型：** 至少 6 个月。
- **正式发布模型的专用变体：** 至少 3 个月。例如包括聊天类变体，例如 `gpt-5.1-chat-latest`，Codex 变体，例如 `gpt-5.3-codex`，以及深度研究类变体，例如 `o3-deep-research`.
- **预览模型：** 预览模型（其名称中含有 `preview` ）可能以更短的通知期（例如 2 周）被下线。例如包括 `computer-use-preview` 和 `gpt-4o-audio-preview`。除非你能够在短时间内完成迁移，否则我们不建议将预览模型用于业务关键的生产工作负载。

如果出于安全或合规方面的考虑需要我们更早停用某个模型,我们会在合理可行的范围内尽可能提前通知。

这些通知期为你提供时间,以便评估推荐的替代模型、测试应用行为,并在模型不可用之前完成迁移。在某些情况下,开发者可以在模型停用日期之后配置专用容量以继续访问。如需了解该选项, [请联系我们的销售团队](https://openai.com/contact-sales/).

## 弃用与旧版

我们使用术语“弃用”（deprecation）来指代下线模型或端点的过程。当我们宣布某个模型或端点被弃用时，它会立即进入弃用状态。所有被弃用的模型和端点还会有一个下线日期。在下线日期到达时，该模型或端点将无法再被访问。

我们使用术语“sunset”和“shut down”互换使用，都表示某个模型或端点无法再被访问。

我们使用术语“遗留”（legacy）来指代不再接收更新的模型和端点。我们将端点和模型标记为遗留，是向开发者表明我们作为平台的发展方向，并提示他们应当迁移到更新的模型或端点。你可以预期遗留的模型或端点将在未来的某个时间点被弃用。

## 即将弃用

即将弃用的项如下所示，最新的公告位于顶部。

### 2026-09-11：GPT-5.4-Cyber

该 `gpt-5.4-cyber` 模型已弃用，将在 2026-10-01 从 API 中移除。请迁移到 `gpt-5.6-cyber` ，截止日期为下线日期前。

| 下线日期 | 模型 / 系统  | 建议替代方案 |
| ------------- | --------------- | ----------------------- |
| 2026 年 10 月 1 日   | `gpt-5.4-cyber` | `gpt-5.6-cyber`         |

### 2026-08-26：转写模型

2026 年 8 月 26 日，我们向使用 `whisper-1`, `gpt-4o-transcribe`, `gpt-4o-mini-transcribe`，的开发者发出通知，告知其将于 2027 年 2 月 26 日从 API 中弃用并移除 `gpt-4o-transcribe-diarize` 。
...
2026 年 6 月 11 日,我们通知使用旧版 GPT-5 和 o3 模型快照的开发者,这些模型将于 2026 年 12 月 11 日从 API 中弃用并下线。
...
2026 年 6 月 3 日,我们通知了在仪表板和 API 中使用可复用提示词的开发者,可复用提示词对象即将被弃用。

| 日期         | 更新                                                                       |
| ------------ | ---------------------------------------------------------------------------- |
| 2026 年 6 月 3 日 | 宣布弃用，并在平台中弱化提示创建。     |
| 2026 年 11 月 30 日 | 该 `v1/prompts` API 与可复用的提示对象计划下线。 |

要进行迁移，请将可复用的提示内容移到你的应用代码中。请参阅 [从 prompt 对象迁移](https://developers.openai.com/api/docs/guides/prompting/migrate-from-prompt-object).

### 2026-06-03:Evals 平台

2026 年 6 月 3 日，我们通知使用 Evals 平台的开发者，该产品将被弃用。

| 日期         | 更新                                                  |
| ------------ | ------------------------------------------------------- |
| 2026 年 6 月 3 日 | 已宣布 Evals 平台即将弃用。           |
| 2026/10/31 | 现有 evals 将变为只读。                        |
| 2026 年 11 月 30 日 | Evals 仪表板和 API 计划下线。 |

为 eval 工作流记录的评分器属于此次过渡的一部分。与微调相关的时间线仍由下文自助式微调一节涵盖。
...
2026 年 6 月 3 日，我们已通知使用 智能体 Builder 的开发者该产品即将弃用。ChatKit 仍可使用。

| 日期         | 更新                                   |
| ------------ | ---------------------------------------- |
| 2026 年 6 月 3 日 | 已宣布 智能体 Builder 将弃用。 |
...
2026 年 6 月 2 日，我们已通知使用旧版 GPT Image 模型的开发者，这些模型将于 2026 年 12 月 1 日从 API 中弃用并移除。
...
在 2026 年 5 月 7 日，我们向使用 OpenAI 自助式微调平台的开发者通知了可用性的更新。

在基础模型弃用之前，针对已微调模型的推理将继续可用。

| 日期         | 更新                                                                                                                                                                                           |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026 年 5 月 7 日  | 此前未运行过微调的组织无法创建微调任务或进行训练。                                                                                |
| 2026 年 7 月 2 日 | 在过去 60 天内未对微调模型运行推理的组织将无法再创建微调任务。                                                         |
| 2027 年 1 月 6 日  | 在该日期，现有活跃客户将无法再创建新的微调任务。仅当底层基础模型被弃用时，针对微调模型的推理才会被禁用。 |

### 2026-04-22：旧版 GPT 模型快照

为了提升可靠性并帮助开发者更轻松地选择合适的模型，我们将弃用一组较旧的 OpenAI 模型。这些模型的访问权限将在下述日期关闭。
...
2026 年 3 月 24 日，我们通知使用 Videos API 和 Sora 2 视频生成模型别名及快照的开发者，这些 API 将于 2026 年 9 月 24 日被弃用并从 接口 中移除。
...
为了提升可靠性，并帮助开发者更轻松地选择合适的模型，我们将在未来六到十二个月内逐步下线一批使用量持续下降的较旧 OpenAI 模型。这些模型的访问权限将在以下日期关闭。
...
2026 年 5 月 8 日，我们通知了使用以下版本的开发者 `gpt-5.2-chat-latest` 和 `gpt-5.3-chat-latest` 模型快照将被弃用并从 API 中移除。
...
2025 年 11 月 18 日，我们通知了使用 `chatgpt-4o-latest` 模型快照将于 2026 年 2 月 17 日被弃用并从 API 中移除。
...
2025 年 11 月 17 日，我们向使用 `codex-mini-latest` 模型的开发者发送了通知，告知该模型将于 2026 年 2 月 12 日弃用并从 API 中移除。作为本次弃用的一部分，我们将不再支持原有的本地 shell 工具，该工具仅能与 `codex-mini-latest`。一起使用。新的使用场景，请使用我们最新的 shell 工具。
...
Realtime API Beta 已于 2026 年 5 月 12 日弃用并从 API 中移除。

Realtime beta API 与已发布的正式版 API 中的接口存在一些关键差异。参见 [迁移指南](https://developers.openai.com/api/docs/guides/realtime#beta-to-ga-migration) 以了解当前的正式版接口及相关的 Realtime 文档。
...
2025 年 9 月，我们已通知正在使用的开发者 `gpt-4o-realtime-preview` 相关模型将在六个月后弃用并从 API 中移除。
...
在 2025 年 8 月 26 日，我们通知使用 Assistants API 的开发者该 API 将被弃用，并将在一年后（即 2026 年 8 月 26 日）从 接口 中移除。

在我们发布 [Responses API](https://developers.openai.com/api/reference/resources/responses/methods/create) 于 [2025 年 3 月](https://developers.openai.com/api/docs/changelog)，时，我们宣布计划将 Assistants API 的全部功能迁移到更易使用的 Responses API，并定于 2026 年下线。
...
2025 年 6 月 10 日，我们通知使用 `gpt-4o-realtime-preview-2024-10-01` 的开发者，该功能将在三个月后弃用并从 API 中移除。
...
2025 年 6 月 10 日，我们通知使用 `gpt-4o-audio-preview-2024-10-01` 的开发者，该功能将在三个月后弃用并从 API 中移除。
...
2025 年 4 月 28 日，我们通知了使用 `text-moderation` 的开发者，该 API 将在六个月内弃用并移除。
...
2025 年 4 月 28 日，我们通知了使用 `o1-preview` 和 `o1-mini` 关于它们的弃用以及从 API 中移除的时间，分别为三个月和六个月。
...
2025 年 4 月 14 日，我们已通知开发者，该 `gpt-4.5-preview` 模型已弃用，并将在未来数月内从 API 中移除。
...
在 [2024 年 4 月](https://developers.openai.com/api/docs/assistants/migration) 当我们发布 Assistants API 的 v2 beta 版本时，我们宣布将在 2024 年底前关闭 v1 beta 的访问。v1 beta 的访问将于 2024 年 12 月 18 日停止。
...
在 2024 年 8 月 29 日，我们通知正在微调的开发者 `babbage-002` 和 `davinci-002` 自 2024 年 10 月 28 日起，这些模型将不再支持创建新的微调训练任务。

基于这些基础模型创建的已微调模型不受本次弃用影响，但你将无法再使用这些模型创建新的微调版本。

| 下线日期 | 模型 / 系统                            | 建议替代方案 |
| ------------- | ----------------------------------------- | ----------------------- |
| 2024-10-28    | 新增微调训练 on `babbage-002` | `gpt-4o-mini`           |
| 2024-10-28    | 新增微调训练 on `davinci-002` | `gpt-4o-mini`           |

### 2024-06-06：GPT-4-32K 和 Vision Preview 模型

2024 年 6 月 6 日，我们向使用以下模型的开发者发出通知： `gpt-4-32k` 和 `gpt-4-vision-preview` 上述模型将分别在一年和六个月后弃用。自 2024 年 6 月 17 日起，仅这些模型的现有用户可继续使用它们。
...
2023 年 11 月 6 日，我们 [宣布](https://openai.com/blog/new-models-and-developer-products-announced-at-devday) 发布更新后的 GPT-3.5-Turbo 模型（默认配备 16k 上下文），同时弃用 `gpt-3.5-turbo-0613` 和 ` gpt-3.5-turbo-16k-0613`。自 2024 年 6 月 17 日起，仅这些模型的现有用户可继续使用。
...
2023 年 8 月 22 日，我们 [宣布](https://openai.com/blog/gpt-3-5-turbo-fine-tuning-and-api-updates) 新的微调 API（`/v1/fine_tuning/jobs`），并且原始的 `/v1/fine-tunes` API 以及旧版模型（包括使用 `/v1/fine-tunes` API 微调的模型）将于 2024 年 1 月 4 日停用。这意味着使用 API `/v1/fine-tunes` 微调的模型将无法再访问，你需要使用更新后的端点和相关基础模型重新微调新模型。
...
在 2023 年 7 月 6 日，我们 [宣布](https://openai.com/blog/gpt-4-api-general-availability) 即将停用通过 completions 端点提供服务的较旧 GPT-3 和 GPT-3.5 模型。我们还宣布即将停用我们的第一代文本嵌入模型。它们将于 2024 年 1 月 4 日下线。
...
替换模型的 `gpt-3.5-turbo-instruct` 定价可在 [定价页面](https://openai.com/api/pricing).
...
替换模型的 `babbage-002` 和 `davinci-002` models can be found on the [定价页面](https://openai.com/api/pricing).
...
2023 年 6 月 13 日，我们在 [函数调用及其他 API 更新](https://openai.com/blog/function-calling-and-other-api-updates) 博客文章中宣布了新的聊天模型版本。三个原始版本最早将于 2024 年 6 月被弃用。自 2024 年 1 月 10 日起，仅这些模型的现有用户可继续使用。

| 下线日期          | 旧模型 | 旧模型价格                                   | 建议替代方案 |
| ---------------------- | ------------ | ---------------------------------------------------- | ----------------------- |
