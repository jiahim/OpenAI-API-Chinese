...
选择你要构建的 Realtime 模型。对于 GPT-Live，请使用 [GPT-Live 提示指南](https://developers.openai.com/api/docs/guides/live-prompting).
...
`gpt-realtime-2` 是我们面向低延迟语音到语音应用的最新推理语音模型。它能够先思考再说话，更可靠地遵循指令，使用更大的上下文窗口，并以比早期 realtime 模型更高的精度调用工具。

要充分利用这些提升，请在设计提示时更具意图性。明确界定助手的职责、决策点、工具调用行为以及护栏：要做什么、何时做、以及应当避免什么。

从简单开始。不要一开始就过度提示。先从一个最小化的提示开始，运行
  评估，然后仅为测试中失败的行为添加指令。
...
将 Realtime 2 作为具备推理能力的语音智能体进行提示，而不是作为基础的语音机器人。

| 变更                                | 对提示词的影响                                                                                                                                         |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 推理                             | 允许模型在说话或调用工具之前，针对复杂任务进行内部推理。使用开场白来避免尴尬的沉默或不必要的填充内容。            |
| 提示词的精确性更加重要         | 将“提供帮助”这类宽泛的指导替换为清晰的触发器、动作和例外规则：何时行动、做什么以及何时不做。                         |
| 指令冲突的代价更高 | 移除重复 `always`, `never`, `only`、以及 `must` 规则，除非确实必要。当规则之间存在竞争时，定义优先级。                                |
| 工具行为更易于引导       | 明确指定助手何时应立即行动、询问缺失信息、确认高精度细节、在失败后重试或进行升级处理。                 |
| 开场白是一等公民行为    | 模型在较长的推理或工具使用流程之前可能会先进行简短的语音更新。引导开场白应在何时出现、应保持多短以及何时可以跳过。 |
| 扩展的上下文窗口               | `gpt-realtime-2` 将实时上下文窗口从 32k 扩展到 128k tokens，使其更适合长会话和更大的系统提示词。                |

前言并非隐藏的思维链。它们是简短的口头更新，例如
  "我现在就查看一下该订单。" 不要要求模型透露私有推理。

## 推荐的提示结构

使用简短且带有标题的章节。模型应能快速找到相关的指令。
...
`gpt-realtime-2` 可以用延迟换取更深入的推理。选用仍能为助手提供足够智能的最低推理等级，以满足工作流的需要。

建议从 `low` 开始，适用于大多数生产环境中的语音智能体。可根据任务复杂度、延迟容忍度和失败成本上下调整。

| Effort    | 使用场景                                            | 示例                                                                 |
| --------- | --------------------------------------------------- | ----------------------------------------------------------------------- |
| `minimal` | 延迟最低最为重要，且任务简单。 | 智能家居指令、计时器、简单的日历查询。                    |
| `low`     | 你需要响应速度加上基础推理能力。       | 客服支持、订单查询、简单的政策问题。                |
| `medium`  | 助手必须对多步骤任务进行推理。 | 技术支持、诊断、复杂的路由。                        |
| `high`    | 更深入的推理能显著提升成功率。       | 高精度工作流、升级决策、带约束条件的任务。 |
| `xhigh`   | 最大化推理值得为之付出额外的延迟和成本。  | 复杂规划、关键分诊、高风险工具编排。      |

除了 API 设置之外，还可以引导模型在何时以及多大程度上进行推理。
...
开场白是简短的语音播报更新，用于让语音智能体在推理、查询信息或调用工具时保持响应迅速。用得好时，开场白能让用户放心助手正在工作；用得不好时，开场白就会变成废话，并增加用户感受到的延迟。

`gpt-realtime-2` 默认会生成开场白。建议先测试默认行为。如果默认行为不符合你的产品体验，可以再显式进行调整。
...
`gpt-realtime-2` 当提示词明确指定了每种任务类型需要提供多少细节时，对长度指引的遵循效果最佳。与其告诉模型“要简洁”，不如在上下文中定义“简洁”的含义：直接回答、工具结果、排障、对比和升级等情况各自可能需要不同的回复长度。
...
> 助手：如果你希望成本最低，选择 Basic。如果你需要团队权限和共享计费，选择 Pro。如果合规审查或管理员控制很重要，选择 Enterprise。

## 设计工具行为

`gpt-realtime-2` 在工具调用方面更强，但工具行为仍然取决于提示和工具规范的设计。如果提示没有定义何时行动、何时询问、何时确认或何时恢复，助手可能会过早调用工具、提出不必要的问题，或重复失败的调用。

### 设置工具调用积极性

较高的积极性适合用于只读、低风险的操作。当工具会修改数据、触发外部副作用或依赖精确标识符时，较低的积极性更合适。

| 工具类型                           | 默认行为                                          |
| ----------------------------------- | --------------------------------------------------------- |
| 只读、低风险查询          | 在意图和必需字段明确时调用。           |
| 使用确切标识符的只读操作     | 在查询前确认标识符。                     |
| 面向用户的通信          | 在发送前起草或总结。                        |
| 账户变更                     | 调用前进行确认。                                   |
| 购买、取消、支付  | 在调用前确认金额、目标和后果。   |
| 不可逆或高影响操作 | 明确确认，并在适当时提供升级途径。 |

当你同时包含读取和写入操作时，使用这一平衡的默认值。你可以根据自己的使用场景进行调整。
...
> 助手：为确认，你希望我用存档的卡支付剩余余额 $248.16。我应该继续吗？

### 从工具失败中恢复

工具失败属于对话的一部分。良好的恢复策略应当说明发生了什么，并为用户提供清晰的下一步操作。

不要对每次失败都以相同方式处理。恢复行为应当取决于工具类型、失败模式和对用户的影响。有些失败应当静默重试。其他情况则需要请用户澄清、更正标识符、确认新的操作，或选择另一条路径。
...
> Assistant: 我找不到与 O R D dash 3 1 2 5 B 2 3 匹配的内容。我有没有哪部分听错了？

### 保持工具可用性同步

Realtime 模型乐于响应。如果提示中提到了某个实际并不存在的工具，或者工具列表与提示不匹配，模型可能会编造一个工具名称，或假装已经完成了该操作。

例如，如果提示中引用了 `lookup_order`，但提供的工具名为 `search_orders`，模型可能会调用错误的名称或模拟该操作。
...
Voice 智能体 默认倾向于做出回应。在生产环境中，它们经常会听到不应该得到语音回复的音频，例如静音、背景噪音、等待音乐、电视音频或旁人的对话。

当助手应当保持安静并继续收听时，使用一个无操作的等待工具。该工具会为模型提供一个有效的非说话动作，而不是让它说出诸如“我在”或“我没听清”之类的话。
...
将此用于未明确指向的音频，而非用于不清晰的用户请求。如果用户显然是在对助手说话，但内容无法理解，请改为请求澄清。

## 有目的地使用消息渠道

`gpt-realtime-2` 可以在 commentary 频道中生成用户可见的中间消息，并在 final 频道中生成最终的面向用户的响应。当行为取决于其出现的频道时，使用频道专属指令。

| Channel      | 用户可见？ | 用于                   |
| ------------ | ------------- | -------------------------- |
| `commentary` | 是           | 前言和工具调用。  |
| `final`      | 是           | 最终面向用户的消息。 |

例如，工具调用发生在 commentary 通道中。如果希望助手在工具使用之前、期间或之后说些什么，请相对于 commentary 通道来指定该行为。

```text
Before calling tools in the commentary channel, briefly tell the user what you are doing.
```

`gpt-realtime-2` 可以在单轮中发出多个响应阶段。在 API 输出中，这种区分由 `response.done` 事件表示，其中包含一个 `phase` 值，用于指示内容是 commentary 还是最终答案。

你可以在应用中使用该字段来分别处理每个阶段。例如，可以将 commentary 播放或显示为简短的中间更新，而 `final_answer` 可以保留给助手的完整回复。
...
模型应当只在能够有把握地理解音频时采取行动。如果音频不清晰，模型应当提出一个简短的澄清问题，而不是进行猜测。

不要让模型推断缺失的词语、调用工具、捕获实体、生成开场白，或花费隐藏的推理时间来试图重构用户可能说过的话。
...
许多实时工作流都依赖于精确值：订单 ID、追踪编号、电子邮件地址、确认码、账号、理赔号、工单 ID、支持参考号和电话号码。

语音让这件事变得困难。用户语速很快，会用不同方式将数字分组、逐字母拼读部分值、使用语气词、在对话中途自我纠正，或读出发音相近的字符。一个数字出错就可能导致查询失败或取到错误的账户。

应当保守地捕获实体信息。每次只收集一个值，仅对清晰明确的部分进行规范化，在调用工具前确认高精度值，并确保任何更正都是可恢复的。

### 每次收集一个实体

当 工作流 需要收集多个值时,应一次收集一个。这样可以避免字段混在一起,尤其是在语音对话中。
...
当用户逐字符拼读 ID、代码、名称或电子邮件地址时使用此选项。口播的是逐字输入，而不是最终值。
...
对于数字标识符，用户可以逐个说出数字、将它们分组或使用自然数短语。如果字段期望一个连续的数字值，请将清晰的数字语音转换为数字。
...
订单 ID、追踪号码、账号、理赔编号、确认码以及类似的标识符都是高精度字段。在工具调用中使用它们之前，请先确认。

对于数值类标识符，请逐位回读其值。将整个数值作为整体读出可能会掩盖错误。

示例：

> 助手：确认一下，我听到的是 8... 3... 5... 2... 1。对吗？

如果用户纠正了一个字符或数字，请在调用工具之前完整重复一遍已纠正的值。
...
电子邮件地址是重要的值。点号、连字符、下划线、重复字母以及读音相似的名称，都可能导致账号查找失败，或将消息发送到错误的地址。

请让用户拼读电子邮件地址：

> Assistant: 能否请你逐字符拼出该电子邮件地址，以便我确保完全准确？

回读时请确认准确的最终地址：

> Assistant: 为了确认一下，邮箱是 c-h-e-n at example dot com，对吗？
...
当某个任务在任何工具调用之前需要确切的值时，请使用这个完整的工作流。
...
`gpt-realtime-2` 比此前的实时模型更字面化地遵循指令。在旧模型上效果良好的提示词可能需要重新调优。

使用精确的语言。模型可能会优先遵循指令的措辞本身，而不是你所期望的更宏观的行为。宽泛或僵化的规则可能以意想不到的方式主导助手的行为，尤其是在多条规则重叠时。

谨慎使用约束性词语，例如 `must`, `only`, `never`，以及 `always`。仅在行为确实需要时才使用它们，而不是用作一般性强调。过度使用硬性约束会让助手变得僵化、过度谨慎，或无法处理合理的例外情况。
...
宽泛的版本可能在无害的只读查询（例如查询订单状态、检索可用性或读取账户信息）之前引起不必要的确认。
...
模型可能不会应用该规则，因为用户提供的是订单 ID，而不是确认码。开发者可以清楚理解预期行为，但该指令的适用范围过窄。
...
- 优先使用明确的指令，而不是依赖隐含意图。
- 除非行为确实需要严格约束，否则避免使用不必要的限制性词语。
- 尽量减少相互矛盾的指引。
- 谨慎处理分层或相互冲突的优先级指令。
- 逐步测试提示词。措辞的细微变化可能会对行为产生显著影响。
- 从早期的实时模型迁移时，预计部分提示词需要重构才能获得最佳效果。

## 分别控制语言与口音

语言和口音应分别控制。

用户的口音并不等同于他们期望使用的语言。用户可能用印地语、西班牙语、法语或普通话口音说英语，但仍然期望收到英语回复。
...
这些指令过于宽泛。模型可能将口音、填充词、回应词或零星的外语词理解为切换语言的依据。
...
`gpt-realtime-2` 可以更强地遵循口音指令，但模糊的口音提示可能导致漂移或意外切换语言。
...
使用 [Custom Voices](https://developers.openai.com/blog/updates-audio-models#custom-voices) 当标准语音无法可靠满足品牌、口音或角色要求时使用。

提示工程可以引导口音、节奏和表达方式，但无法完全替代语音设计。对于需要一致的品牌语音形象或口音保真度的用例，请考虑 [Custom Voices](https://developers.openai.com/blog/updates-audio-models#custom-voices).

Custom Voices 仅对已批准的客户开放。请联系您的客户团队获取访问权限。

## 在长会话中保持状态

`gpt-realtime-2` 将实时上下文窗口从 32k 扩展到 128k tokens，更适合长会话。对于密集的双向对话，128k tokens 大致可视为约 1-2 小时的密集原始音频上下文。具体时长会因工具调用、内部推理、注入的记录以及其他会话细节而有所不同。

对于长上下文使用场景， `gpt-realtime-2` 当它能够分辨哪些信息是最新的、哪些是背景信息，以及在来源冲突时应忽略哪些内容时，表现最佳。不要依赖模型从原始转录文本或大量上下文转储中推断来源优先级。请使用结构化的方式。

在以大量上下文（例如检索到的记录、先前的对话记录、策略、摘要、账户备注或背景文档）开启会话时，请使用结构化的模式。
...
在从更早的实时模型迁移时，将 prompt 视为行为表面，而不仅仅是要迁移的文本。

1. 使用 Codex 或强推理模型，根据最新的 Realtime 提示词指南重构提示词。附上本提示词指南的链接，以便将迁移建立在最佳实践的基础上。
2. 将推理强度设置为 `low` （而非默认值）。仅在需要更深层规划的工作流中提高推理强度。
3. 审查工具名称、参数、枚举、JSON schema 和其他设置，确保它们与预期实现一致。
4. 移除过时的示例。为正常路径、歧义、中断、工具调用和回退行为补充简短的示例。
5. 对比迁移前后的代表性对话。依据现有评估检查回归，并记录有意的行为变更。
6. 执行最终的一致性检查。确认提示词清晰区分了硬性要求、默认值、工具规则、安全规则和回退行为。
7. 运行评估，检查代表性失败案例，并迭代优化提示词，直到目标行为稳定可靠。
...
`gpt-realtime-1.5` 是 Realtime API 中的一个语音到语音模型。同样的 `gpt-realtime` 提示词指南同样适用于该模型。

语音到语音系统对于将语音打造为核心 AI 界面至关重要。 `gpt-realtime-1.5` 支持稳健、可用的实时语音智能体，能够大规模处理关键任务工作流。

与早期的实时预览模型相比， `gpt-realtime-1.5` 带来了更强的指令遵循、更可靠的工具调用、更好的语音质量以及整体更流畅的体验。这些改进使得从链式方案过渡到真正的实时体验成为可能，在降低延迟的同时生成听起来更自然、更具表现力的回复。

Realtime 模型受益于那些无法直接套用于文本模型的提示词技巧。本提示词指南首先给出一个建议的提示词骨架，然后逐部分讲解实用的技巧、可直接复制的小模式，以及可按你的用例加以改造的示例。

## 通用提示

- **持续迭代**：微小的措辞改动就能决定行为是否有效。
  - 示例：对于含义不清的音频指令，我们将“inaudible”改为“unintelligible”，改善了对噪声输入的处理效果。
- **优先使用要点而非段落**：简短清晰的要点优于冗长的段落。
- **用示例引导**：模型会紧密遵循示例中的措辞。
- **保持精确**：模糊或相互矛盾的指令会降低性能，效果与 GPT-5 类似。
- **控制语言**：如果出现意外的语言切换，可将输出固定为目标语言。
- **减少重复**：添加一条多样性规则，减少机械化的措辞。
- **使用全大写文本以示强调**：将关键规则全大写可使它们更突出，模型也更容易遵循。
- **将非文本规则转换为文本**：与其写 “IF x > 3 THEN ESCALATE”，不如写 “IF MORE THAN THREE FAILURES THEN ESCALATE”。

## 提示结构

合理组织你的提示可以让模型更容易理解上下文，并在多轮交互中保持一致性，也便于你迭代和修改有问题的部分。

- **它能做什么**：在系统提示中使用清晰、带标签的章节，方便模型查找并遵循。每个章节只聚焦一件事。
- **如何适配**：添加与领域相关的章节（例如 Compliance、Brand Policy）。删除不需要的章节（例如如果不纠结发音问题，可删除 Reference Pronunciations）。
...
本节定义智能体的身份以及“完成”的含义。示例展示了两种不同的身份，以演示当角色和目标明确时，模型对它们的遵循程度有多紧密。

- **使用时机**：模型没有采用你需要的人设、角色或任务范围。
- **它能做什么**：固定语音智能体的身份，使其回复以该角色描述为条件
- **如何适配**：根据你的使用场景修改角色
...
早期实时预览：
  ...
  `gpt-realtime-1.5` 能够比早期的实时预览模型更可靠地扮演指定角色。

## 个性与语气

`gpt-realtime-1.5` 在模仿特定人格或语气时，能够很好地遵循指令。你可以根据用例的期望来定制语音体验和表达方式。

- **使用时机**: 回复显得平淡、过于冗长，或在不同轮次之间不一致。
- **它能做什么**: 确定语气、简洁度和节奏，使回复听起来自然且一致。
- **如何适配**: 调整温度/正式程度以及默认长度。对于受监管的领域，优先采用中性且精确的表达。添加与你的用例相关的其他小节。
...
`gpt-realtime-1.5`:
  ...
  该模型能够遵循复杂指令,并在整个音频回复过程中在三种情绪之间切换。

### Speed Instructions

在 Realtime API 中， `speed` 参数会改变播放速度，而不会改变模型组织语音的方式。要让语速实际变快，需要添加能够引导语速节奏的指令。

- **使用时机**: 用户希望语速更快；仅靠播放速度（speed 参数）无法改善说话风格。
- **它能做什么**: 可调整说话风格（简洁程度、节奏），与客户端播放速度无关。
- **如何适配**: 修改速度指令以满足用例需求。
...
早期实时预览：
  ...
  通过显式的节奏控制指令， `gpt-realtime-1.5` 可以让节奏明显加快而又不会显得过于仓促。

### 语言约束

语言约束可确保模型在嘈杂背景或多语言输入等具有挑战性的条件下，依然以预期语言进行回复。

- **使用时机**：在多语言或嘈杂环境中防止意外的语言切换。
- **它能做什么**：将输出锁定为所选语言，以防止意外的语言切换。
- **如何适配**：将“English”切换为目标语言；或根据你的使用场景添加更复杂的指令。
...
这些是使用以下指令应用后的响应 `gpt-realtime-1.5`.
...
该模型能够根据自定义指令在语言之间进行切换。

### 减少重复

实时模型可以紧密遵循示例短语以保持品牌一致性，但它可能会过度使用这些短语，使回复听起来机械或重复。添加一条重复规则有助于在保持清晰度和品牌语调的同时维持表达的多样性。

- **使用时机**: 输出内容会在多个回合或会话中反复使用相同的开场白、填充词或句式。
- **它能做什么**: 添加多样性约束——减少重复短语、引导使用同义词和不同的句式结构，同时保留必要的术语。
- **如何适配**: 调整严格程度（例如“每 N 个回合不要重复使用相同的开场白”），将必须保留的短语（如法律/合规/品牌用语）加入白名单，并在需要保持一致性的地方允许更紧凑的措辞。
...
这些是应用该指令之前的响应 **之前** 应用该指令之前的响应，使用 `gpt-realtime-1.5`。之后，模型重复相同的确认： `Got it`.

![repeat before](https://developers.openai.com/cookbook/assets/images/repeat_before.png)

这些是应用该指令之前的响应 **之后** 应用该指令之前的响应，使用 `gpt-realtime-1.5`.

![repeat after](https://developers.openai.com/cookbook/assets/images/repeat_after.png)

现在模型能够变换其响应和确认，听起来不再机械。

## 参考发音

本节介绍如何在语音交互中确保模型正确发音重要词语、数字、名称和术语。

- **使用时机**: 品牌名、技术术语或地点经常被读错。
- **它能做什么**: 通过发音提示增进信任感和清晰度。
- **如何适配**: 保持列表简短；听到读错时随时更新。
...
早期实时预览：
  ...
  借助参考发音指令， `gpt-realtime-1.5` 可以正确地将 SQL 发音为 “sequel”。

### 字母数字读音

Realtime S2S 在回读关键信息（电话、信用卡、订单 ID）时，可能会模糊或混淆数字与字母。逐字显式确认可以避免误听，并使合成更清晰。

- **使用时机**: 如果模型难以捕获或回读电话号码、卡号、2FA 验证码、订单号、序列号、地址、单元号或混合字母数字字符串。
- **它能做什么**: 强制模型逐字符朗读并加上分隔符，然后与用户确认，并在修正后再次确认。可选用语音消歧符标识字母（例如，“A as in Alpha”）。
...
_提示：如果你正在遵循会话流程提示策略，可以指定哪个会话状态需要应用字母数字读音指令。_

#### 示例（对话状态中的指令）

_(取自我们对话流程中的提示 [openai-realtime-智能体](https://github.com/openai/openai-realtime-agents/blob/main/src/app/agentConfigs/customerServiceRetail/authentication.ts))_
...
这些是应用该指令之前的响应 **之前** 应用该指令之前的响应，使用 `gpt-realtime-1.5`.
...
本节介绍用于指导你的模型完成你的任务、应用最佳实践以及修复可能出现的问题的提示词指引。

或许并不意外，我们建议采用与 [GPT-4.1 类似的提示词模式以获得最佳效果](https://developers.openai.com/cookbook/examples/gpt4-1_prompting_guide).

### 指令遵循

与 GPT-4.1 和 GPT-5 一样，如果指令相互冲突、含糊或不明确， `gpt-realtime-1.5` 模型的表现会变差。

- **使用时机**: 输出偏离规则、跳过阶段或误用工具。
- **它能做什么**: 使用 LLM 在发布前指出歧义、冲突和缺失的定义。

#### **指令质量提示词（可在 ChatGPT 中使用，或配合 API 使用）**

使用以下提示配合 GPT-5 来识别提示中可修复的问题区域。
...
这条元提示可帮助你针对特定的失败模式来改进基础系统提示。提供当前的提示，并描述你所遇到的问题，模型（GPT-5）会给出细化的变体建议，从而收紧约束并减少该问题。
...
有时模型会误以为听到了某些内容并尝试回应。你可以为模型添加自定义指令，告知它在听到不清晰的音频或用户输入时该如何表现。根据你的用例调整期望的行为。例如，你可能希望模型重复同一个问题，而不是请求澄清。

- **使用时机**: 背景噪音、残缺的语句或静音会触发不必要的回复。
- **它能做什么**: 阻止误响应并给出得体的澄清。
...
这些是应用该指令之前的响应 **之后** 应用该指令之前的响应，使用 `gpt-realtime-1.5`.
  ...
  在本例中，模型在我的 _（非常）_ 大声咳嗽和音频不清晰后请求澄清。

### 背景音乐或音效

模型偶尔会在语音生成过程中产生非预期的背景音乐、哼唱、有节奏的噪音或类声音伪影。这些伪影会降低清晰度、分散用户注意力，或让智能体显得不够专业。下列指令有助于防止或显著减少这些情况的发生。

- **使用时机**: 当你在 Realtime 音频响应中观察到非预期的音乐元素或音效时使用。
- **它能做什么**: 引导模型避免生成这些不需要的音频伪影。
...
使用本节告诉模型如何使用你的函数和工具。明确说明何时调用工具、何时不调用工具、要收集哪些参数、调用进行中要说什么，以及如何处理错误或部分结果。

### 工具选择

`gpt-realtime-1.5` 能够较为严格地遵循指令。但是,如果你的指令与模型实际可访问的内容存在冲突,例如在提示中提及了未传入 tools 列表中的工具,就可能导致生成较差的响应。

- **使用时机**: 提示中提到了实际不可用的工具。
- **它能做什么**: 审查可用的工具和系统提示，以确保它们保持一致。
...
某些用例可以受益于 Realtime 模型在调用工具的同时提供音频响应。这能带来更好的用户体验，掩盖延迟。你可以根据自己的用例修改示例短语。

- **使用时机**: 用户需要在工具调用的同时立即获得确认；有助于掩盖延迟。
- **它能做什么**: 在工具调用前添加一段简短且一致的前导语。
...
使用该指令后，模型会同时输出音频回复 "I'm checking that right now" 并发起工具调用。

#### 工具调用开场白与示例短语

如果希望更精细地控制模型在调用工具时所输出的短语类型，可以在工具规格的 description 中添加示例短语。
...
有时模型会在工具调用前要求确认。在某些使用场景下，这可能会导致终端用户体验不佳，因为模型表现得不够主动。

- **使用时机**: 智能体 在执行明显的工具调用前会请求权限。
- **它能做什么**: 减少不必要的确认循环。
...
在示例中，你会注意到实时模型没有生成任何响应音频，而是直接调用了相应的工具。

_提示：如果你发现模型太快地调用工具，可以尝试软化措辞。例如，把“proactive”这样较强的词换成更温和的表述，有助于引导模型采取更冷静、更不急切的方式。_

### 工具调用性能

随着用例日益复杂、可用工具数量不断增多，关键在于明确引导模型在何时使用某个工具，同样重要的是，还要明确引导它在何时不应使用该工具。清晰的使用规则不仅能提升工具调用的准确性，还能帮助模型在恰当的时机选择恰当的工具。

- **使用时机**: 模型在工具调用性能方面表现不佳，需要明确的使用说明以减少误用。
- **它能做什么**: 添加关于何时“使用/避免使用”每个工具的说明。你也可以添加关于工具调用顺序的说明（在工具调用 A 之后，你可以调用工具调用 B 或 C）
...
_提示：如果某个工具调用可能会不可预测地失败，请添加清晰的失败处理指令，以便模型能够优雅地响应。_

### 工具层级行为

你可以针对特定工具微调模型行为，而不是套用一条全局规则。例如，你可能希望主动调用 READ 工具，而 WRITE 工具则需要明确确认。

- **使用时机**: 主动行为、确认或开场白的全局指令并不适用于每个工具。
- **它能做什么**: 添加针对单个工具的行为规则，定义模型应直接调用工具、先确认，还是在调用前先说一句开场白。
...
某些工具输出，尤其是必须逐字重复的长字符串，可能不在模型的分布范围内。训练时，工具输出通常表现为带命名字段的 JSON 对象。如果你的工具返回原始字符串，并另外要求模型“完全重复”，模型就更容易出现改写、截断或混入自加前言的情况。

一种实用的修复方法是让工具输出看起来像普通的工具结果，并将“逐字”这一要求以机器可显式的方式表达出来。

- **使用时机：** 工具返回 **较长或结构复杂的内容** （多句指令、交接 数据包、ID/链接、策略摘要、多步骤流程等），并且你观察到 **截断、改写、字段丢失、顺序重排，或模型混入自己的前言/评注**.

- **它的作用：** 将工具输出包装在一个 **简洁、明确的 JSON 信封中** （例如， `response_text` 以及诸如 `require_repeat_verbatim`, `format`，之类的标志，或 `content_type`），使响应看起来更 **符合分布** ，且期望的生成行为对 **机器而言更清晰**.

- **如何适配：** 保持 schema **简洁且稳定**。在工具的 **说明中** 以及在 **工具定义旁边** （例如，“如果 `require_repeat_verbatim` 为真，则恰好输出 `response_text` ，不输出其他内容”，或“原样渲染 `response_text` ；不要添加、遗漏或重新排序工具输出中的字段”）。
...
因为这看起来像典型的工具结果（JSON 对象），所以模型通常处理起来更容易：

- 识别“权威”内容（response_text）
...
在许多语音场景中，realtime 模型充当应答者（与用户对话），而更强的文本模型充当思考者（负责规划、策略查询、SOP 完成）。文本回复并不天然适合语音朗读，因此应答者必须在生成音频之前，把思考者生成的文本改写为适合语音输出的回答。

- **使用时机**: 当响应者的口语输出在收到 thinker 响应后听起来机械化、过长或别扭时。
- **它能做什么**: 添加清晰的指令，引导响应者将 thinker 的文本重新表述为简短、自然、以语音为先的回答。
- **如何适配**: 调整措辞风格、开场方式与篇幅限制，以匹配你的用例期望。
...
> Assistant: 你当前的信用卡余额为正值 32,323,232 澳元。

下面是带有重述指令的相同示例：

> 助手：刚刚查完了——你的信用卡余额是顺差三千二百三十二万三千二百三十二美元。上一次付款是在八月一日处理的。和你的预期一致吗？

### 常用工具

`gpt-realtime-1.5` 已经过训练，能够有效地使用以下常见工具。如果你的用例需要类似的行为，请尽量保持名称、签名和描述与这些工具接近，以最大化可靠性并使其更符合训练分布。
...
本节介绍如何将对话组织为清晰的、目标驱动的各个阶段，以便模型清楚知道每一步该做什么。它定义了每个阶段的目的、在该阶段中推进的指令，以及用于切换到下一阶段的明确“退出条件”。这可以防止模型停滞、跳过步骤或提前跳跃，并确保从问候到问题解决的整个对话保持有序。

此外，通过将提示组织为不同的对话状态，可以更容易地识别错误模式并更有效地进行迭代。

- **使用时机**：如果对话显得缺乏条理、在达成目标之前停滞，或模型难以有效地完成任务。
- **它能做什么**：将交互拆分为若干阶段，并为每个阶段设定明确的目标、指令和退出条件。
- **如何适配**：将各阶段重命名为与你的工作流相匹配的名称；修改各阶段的指令以符合你的预期行为；保持“退出条件”具体且简洁。
...
示例短语充当模型的“锚定示例”。它们展示了希望模型遵循的风格、简洁度和语气，但不会将其锁定在单一僵硬的回复中。

- **使用时机**: Responses 缺乏你的品牌风格或不一致。
- **它能做什么**: 提供示例短语，模型可以灵活使用，以保持自然简洁。
- **如何适配**: 替换示例以贴合品牌；保留“不要总是使用”的提示。
...
_注意：如果你的语音系统最终只能重复示例短语，导致语音体验变得更加机械，可以尝试添加 Variety 约束。我们见过这种做法能解决问题。_

### 会话流程 + 示例短语

在不同对话流程状态下添加示例短语是一种有用的做法，可用于教模型什么才是良好的回复：
...
随着用例变得越来越复杂，你需要一个既能扩展又能保持模型有效性的结构。关键在于在可维护性与简洁性之间取得平衡：过于僵化的状态会让模型负担过重，损害性能并让对话显得机械。

一种更好的方法是设计能够降低模型感知复杂度的流程。通过以结构化但灵活的方式处理状态，你可以让模型更容易保持专注和响应灵敏，从而提升用户体验。
...
将会话定义为一个 JSON 结构，对状态和转移都进行编码。这样便于推理覆盖范围、识别边界情况，并跟踪随时间发生的变化。由于它以代码形式存储，因此你可以像管理代码一样对流程进行版本控制、对比差异和扩展。状态机还能让你精细控制会话从一个状态转移到另一个状态的确切方式和时机。
...
在这种模式中，对话会根据当前状态实时更新系统提示和工具列表，从而动态调整。你不会一次性向模型暴露所有可能的规则和工具，而是只提供与当前对话阶段相关的内容。

当某个状态的结束条件满足后，你可以通过 session.update 进行状态切换，将提示和工具替换为下一阶段所需的内容。

这种方式可以减轻模型的认知负担，让它更容易专注于复杂任务，而不会被无关的上下文干扰。
...
在使用 Realtime 语音智能体时，通常需要一种可靠的方式来升级到人工处理。在本节中，你应根据自身使用场景修改升级时机的相关指令。

- **使用时机**: 模型难以判断何时正确地上报给人工或回退系统
- **它能做什么**: 定义快速、可靠的上报流程以及对应话术。
- **如何适配**: 插入你自己的阈值以及模型需要说出的话术。
...
`gpt-realtime-1.5` 能够遵循该指令，更可靠地交接给人工处理。
...
- 审阅 [委托与工具](https://developers.openai.com/api/docs/guides/live-delegation) 以适配后端提示词和由应用持有的上下文。
- 使用 [WebRTC](https://developers.openai.com/api/docs/guides/voice-webrtc?api=live) 或 [WebSockets](https://developers.openai.com/api/docs/guides/voice-websockets?api=live)。参见 [电话与 SIP](https://developers.openai.com/api/docs/guides/voice-sip?api=live) 了解电话集成相关内容。
- [评估语音 智能体](https://developers.openai.com/api/docs/guides/voice-agents#evaluate-your-voice-agent) 在对话质量和已核验任务结果方面的表现。

Realtime 注意事项：

- 查看之前的 [Realtime 提示指南](https://developers.openai.com/cookbook/examples/realtime_prompting_guide) 了解更多 `gpt-realtime-1.5` 示例。
- 查看 [Realtime 评估指南](https://developers.openai.com/cookbook/examples/realtime_eval_guide) 以测试具有代表性的语音智能体行为。
- 使用 [WebRTC](https://developers.openai.com/api/docs/guides/voice-webrtc?api=realtime), [WebSockets](https://developers.openai.com/api/docs/guides/voice-websockets?api=realtime)，之类的标志，或 [SIP](https://developers.openai.com/api/docs/guides/voice-sip?api=realtime).
- 了解 [Realtime 对话生命周期](https://developers.openai.com/api/docs/guides/realtime-conversations) 并查看 [Realtime 费用](https://developers.openai.com/api/docs/guides/voice-latency-cost?api=realtime).