...
> 完整文档索引请参阅 [llms.txt](/llms.txt). 你可以在页面 URL 后追加 `.md` 来获取文档页面的 Markdown 版本。

Token 计数可以让你在向模型发送请求之前，确定该请求将使用多少输入 Token。可用于：

- **优化提示词** 以适应上下文长度限制
- **估算成本** 在发出 API 调用之前
- **路由请求** 根据规模进行路由（例如，将较小的提示词路由到速度更快的模型）
- **避免意外情况** 处理图像和文件时——不再依赖基于字符数的估算

该 [输入 token 计数端点](https://developers.openai.com/api/reference/python/resources/responses/subresources/input_tokens/methods/count) 接受与 [Responses API](https://developers.openai.com/api/reference/resources/responses/methods/create)。相同的输入格式。传入文本、消息、图片、文件、工具或对话——API 会返回模型将接收到的确切 token 数量。

计数包含用于表示请求结构的格式化 token，例如消息角色和边界。这些 token 可能不会出现在你本地分词的文本或字段中。

## 为什么要使用 token 计数 API？

像 [tiktoken](https://github.com/openai/tiktoken) 这样的本地分词器适用于纯文本，但存在一些限制：

- **图像和文件** 不支持——类似 `characters / 4` 的估算不准确
- **工具和架构** 会增加难以在本地计数的 token
- **模型特定行为** 可能改变分词方式（例如推理、缓存）

token 计数 API 会处理所有这些情况。使用与发送到 `responses.create` 相同的 payload，即可获得准确的计数。然后将结果接入你的消息验证或成本估算流程。
...
图片根据大小和细节等级消耗 tokens。用于计数的 API 会返回精确数量——无需猜测。
...
你可以使用 `file_id` （来自 [Files API](https://developers.openai.com/api/reference/resources/files)）或 `image_url` （URL 或 base64 数据 URL）。详见 [图像与视觉](https://developers.openai.com/api/docs/guides/images-vision) 。

## 使用工具统计 token

工具定义（函数 schema、MCP 服务器等）会向上下文添加 token。请将其与输入一起计入：
...
[文件输入](https://developers.openai.com/api/docs/guides/file-inputs)——目前支持 PDF——，可通过 `file_id`, `file_url`，或 `file_data` 方式传递，与 `responses.create`。时一样。token 数反映了模型完整处理后的输入。
...
一些模型（包括 GPT-5 模型）会生成用于格式化或分隔响应通道、工具调用以及其他消息结构的 token。这些格式化 token 不会出现在消息内容中 `logprobs`，并且不一定在使用情况中单独列出。因此，即使报告的输出或 completion token 数与可见 token 数或 `logprobs`，中包含的 token 数一致，报告的 `reasoning_tokens` 值也可能会更高 `0`.

该 `max_output_tokens` ，并且 `max_completion_tokens` 参数会限制模型生成的所有 token，包括不可见的 token。不可见 token 的数量因模型和响应形式而异，因此不要假设报告的使用情况与可见输出之间存在固定差异。当你需要特定数量的可见输出时，请在这些限制中预留余量。

## API 参考

有关完整的参数和响应结构，请参阅 [Count input tokens API 参考](https://developers.openai.com/api/reference/python/resources/responses/subresources/input_tokens/methods/count)。该端点为：
