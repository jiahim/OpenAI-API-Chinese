...
- 当渲染后的 token 数量超过配置的阈值时，服务端
  会运行服务端压缩。
- 在此模式下无需单独 `/responses/compact` 发起调用。
- 响应流中包含加密后的压缩项。
- ZDR 注意事项：当你为 Responses 创建请求设置 `store=false`
  时，服务端压缩对 ZDR 友好。

返回的压缩项会将先前关键状态和推理带入
下一次运行，且使用的 token 更少。该项不透明，
不应被人为解读。

对于无状态的输入数组链式调用，按常规方式追加输出项。如果你
使用 `previous_response_id`,则每轮只传入新的用户消息。在以上两种
情况下，压缩项都会携带下一个窗口所需的上下文。

延迟提示：将输出项追加到先前的输入项之后，你可以
丢弃最近一个压缩项之前的项，以减小请求体积
并降低长尾延迟。最新压缩项已携带
继续对话所需的必要上下文。如果使用
`previous_response_id` 链式调用，则不要手动裁剪。

## 用户旅程

1. 调用 `/responses` 时与往常一样，但需传入 `context_management` ，并配合
   `compact_threshold` 以启用服务端压缩。
2. 在响应流式传输过程中，如果上下文大小超过阈值，服务端
   会触发一次压缩流程，在同一流中输出压缩条目，
   并在继续推理前裁剪上下文。
3. 使用以下任一模式继续你的循环：无状态的输入数组链式调用（将
   输出（包括压缩条目）追加到下一次输入数组中），或者
   `previous_response_id` 链式调用（每轮仅传入新的用户消息，并由
   将该 ID 向下传递）。
...
你发送完整的上下文窗口（消息、工具以及其他项），该
端点会返回一个新的已压缩上下文窗口，你可以将其传递给下一次
`/responses` 调用。

返回的已压缩窗口包含一个加密的压缩项，该项使用更少的 token 传递
前向关键先前状态与推理。它是不透明的，不应被人类解读。
不应被人类解读。

注意：已压缩的窗口通常包含的内容不仅仅是压缩
项。它还可以包含来自上一个窗口中保留的项。

输出处理：不要裁剪 `/responses/compact` 输出。返回的窗口
是规范化的下一个上下文窗口，请将其原样传递给下一次 `/responses`
调用。

### 独立压缩的用户流程

1. 使用 `/responses` 正常发送包含用户消息的输入项，
   助手输出和工具交互。
2. 当你的上下文窗口变大时，调用 `/responses/compact` 以生成一个
   新的压缩后的上下文窗口。你发送给 `/responses/compact`
   的窗口仍必须适合模型的上下文窗口。
3. 对于后续的 `/responses` 调用，传入返回的压缩后的窗口
