...
异步工具调用允许模型在调用某个工具后继续工作，而无需等待该工具的结果。使用它可以提前发起耗时的查询请求、并行处理请求中相互独立的部分，并在你的应用准备好结果时立即返回。

## 异步工具的工作原理

普通的 [function call](https://developers.openai.com/api/docs/guides/function-calling) 会暂停模型本轮生成，等待工具响应。在 `async: true` 上对函数或自定义工具定义进行设置，以允许模型在发出该调用后、你的应用返回输出之前继续工作。

工具仍由你的应用执行。异步工具并不会将执行转移到
  OpenAI，也不会管理你的后台任务。

这与 [后台模式](https://developers.openai.com/api/docs/guides/background)，不同，后台模式会异步运行响应生成。而异步工具调用允许模型在你的应用运行工具的同时继续工作。

当任务完成后，在后续的 Responses 请求中包含其输出。使用原始的 API `call_id` 将结果与其调用进行匹配：
...
Add `async: true` 添加到工具定义中。对应的调用项包含在 `response.output` 包含 `async: true`.
...
响应可以同时包含异步调用和回答。如果任务完成之前发生了其他对话轮次，请更新 `latest_response_id` 以便在保留原始工具 `call_id`.

更早地分派需配合 [流式传输](https://developers.openai.com/api/docs/guides/streaming-responses)，在其完整的调用项到达时启动任务，同时继续消费响应。

## 添加等待工具

等待工具让模型自行决定何时需要等待尚未完成的结果。例如，它可以发起两个价格查询请求，在等待期间处理其他独立任务，只在需要对比价格时才进入等待。

为每个异步工具添加一个 `task_handle` 参数。模型会为每次调用分配一个句柄，你的应用负责把它绑定到原始的 API 调用和正在运行的任务上。 `call_id` 在整个对话过程中保持句柄唯一，包括已完成的任务和重复发起的查询请求。

将等待工具定义为普通的同步函数：省略 `async` 或将其设置为 `false`。它的模式和行为由你的应用决定。 `wait_for_tasks` 不是 Responses 的内置工具。

在请求的 `tools` 数组中使用上述定义：
...
在处理依赖性等待之前，先注册并启动每次 launch。调用可能一起到达，也可能分散在多个响应中。下面的示例输出项展示了两次 launch 和一个同时依赖这两者的等待：
...
你的应用的注册表会将每个句柄绑定到其原始调用和正在运行的任务：
...
解析注册表中请求的句柄，仅等待这些任务。每个新完成的结果都按其原始 `call_id`，顺序返回，然后在 wait 调用自身的 `call_id`。上返回状态。这个顺序能让模型在恢复时拿到结果。

例如，在下一个请求的 `input` 数组中发送这些输出项。价格为示意：
...
将 `previous_response_id` 设为最新的响应 ID，并在 延续 请求中包含工具和指令。你的应用也可以在结果可用时即时交付，而无需 wait 调用。仅当模型的下一步依赖于尚未到达的结果时，才使用 wait 工具。

## 添加工具以向用户请求输入

异步工具可以在模型继续独立工作的同时向用户提问。例如，模型可以询问报告的目标读者，在用户回答期间收集事实，并利用该回答来定制报告。

使用以下方式定义函数 `async: true` 以请求缺失的信息或偏好。你的应用负责展示该问题、收集回复，并将其作为工具结果返回。其结构与行为由你的应用自行决定。 `request_user_input_async` 不是 Responses 的内置工具。

将此定义添加到请求的 `tools` 数组中，与 `wait_for_tasks`:
...
当完整的调用项到达时，注册该问题的 `task_handle` 和原始内容 `call_id`，然后向用户显示该问题。以下示例输出项询问报告的目标读者：
...
将该问题保留在与等待工具相同的注册表中。在用户作答期间，继续消费模型的响应。不要用一个确认已显示问题的结果来完成工具调用；该调用的结果就是用户的回答。

### 返回用户的答案

当用户回复时，将答案发送回该问题最初的 `call_id`。例如，将此输出项包含在下一个请求的 `input` 数组中使用上述定义：
...
将 `previous_response_id` 指向最新的响应 ID，并在延续请求中包含工具和说明。如果模型调用了 `wait_for_tasks` ， `report_audience_1`，则按以下方式在等待状态之前返回答案： [等待工具示例](#deliver-results-before-wait-status).

异步执行不会保持响应一直打开直到用户回复。你应指示模型继续独立工作，并等待需要该答案的步骤。如果你的应用支持忽略问题或使问题超时，请返回明确的“无答案”结果，以便模型决定如何继续。

## 兼容性

异步工具调用由 GPT-6 Astra 及更高版本模型支持。

异步执行适用于你的应用所运行的函数工具和自定义工具。它不适用于托管内置工具。使用直接工具调用；不要为 [程序化工具调用](https://developers.openai.com/api/docs/guides/tools-programmatic-tool-calling).

在 [多智能体模式](https://developers.openai.com/api/docs/guides/responses-multi-agent)，不要将异步工具与并行工具调用组合使用。