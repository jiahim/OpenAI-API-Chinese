...
Multi-智能体 让模型能够并行启动并协调子智能体，综合它们的工作以给出最终响应。这对于需要并行任务委派来处理的复杂任务尤为有效，例如代码库探索、文档编写和功能实现。

Multi-智能体 作为一项 beta 功能，在所有 GPT-5.6 模型上可用。在你的应用中启用 Multi-智能体 之前，请查看模型页面。

## 何时使用多智能体

任务通常可以被拆分为若干独立的工作块，由单个智能体按顺序完成，但多个智能体能够并行处理。多智能体 模式使根智能体能够委派给多个并发完成工作的子智能体。这可以带来多种好处：

- **并行执行。** 独立的研究、分析或实现任务可以同时进行，从而加快执行速度。
- **聚焦的上下文。** 每个子智能体接收一个有边界的任务并维护自己的上下文，从而减少不相关工作线之间的上下文干扰并提升性能。
- **模型主导的协调。** 根智能体可以创建子智能体、向它们发送额外信息、等待结果，并综合出最终答案，无需你的应用自行实现编排逻辑。

当一个任务可以划分为具体且独立的工作流时，多智能体编排最为有用，例如：
...
请注意，添加子智能体可能会增加 token 用量，并且对于依赖单一有序推理链、需要频繁写入共享可变状态，或者已经被一个缓慢的外部操作主导的任务，收益可能并不明显。

| 使用 Multi-智能体当                              | 首选单个智能体当                                 |
| ------------------------------------------------- | ----------------------------------------------------- |
| 工作可以拆分为独立的、有边界的任务 | 每一步都直接依赖上一步       |
| 分离的上下文有助于提升专注度                   | 任务足够小，能在一次简短的运行中完成 |
| 并行探索可以减少实际耗时   | 智能体会争抢同一可变资源   |
| 比较独立发现的结果可提升覆盖度  | 你需要固定的、可确定的执行图    |

## 快速开始

Python 和 JavaScript 示例使用的是 beta 版 Responses SDK。对于 HTTP
  请求，请使用 `client.beta.responses` 并传入 `responses_multi_agent=v1` 参数
  该 `betas` 参数。对于原始 HTTP 请求和 WebSocket 连接，请在请求或连接头中传入
  `OpenAI-Beta: responses_multi_agent=v1` 传入请求或连接头中。
  在 Multi-智能体 处于 beta 阶段时，项目结构可能会发生变化。

在你的 Responses API 请求中通过以下方式启用 Multi-智能体：传入 `multi_agent.enabled`。当 `multi_agent.enabled` 为 `true`，时，根 智能体 就可以生成一棵子智能体树。这些子智能体共享该请求的模型和可用工具，而 智能体 通过生成、消息传递和等待等协作原语进行协调（参见 [Multi-智能体 的工作原理](#how-multi-agent-works)）。根 智能体 负责综合各子智能体的响应并给出最终响应。
...
`max_concurrent_subagents` 设置了在整个 智能体 树中可同时处于活动状态的子智能体最大数量。它包含所有后代——子节点、孙节点以及更深层级的子智能体——但不包含根 智能体。

API 没有为该设置设定固定上限，默认值为 `3`，建议大多数工作负载使用此默认值。Multi-智能体 运行对树的深度或一次运行中创建的子智能体总数也没有固定限制。

添加一条开发者消息，以调整根模型应在何时生成子智能体。该开发者消息会叠加在为根 智能体 和子智能体注入的指令之上。

开发者消息示例包括：

- “除非用户明确要求使用子智能体、委派或并行智能体工作，否则不要派生子智能体。”
- “主动多智能体委派已启用。当并行工作能够显著提升速度或质量时，请使用子智能体。”

## 多智能体的工作原理

Responses API 为根智能体和子智能体模型提供托管编排操作以及使用这些操作的相关说明。根智能体命名为 `/root`。生成的子智能体使用如下所示的层级路径：
...
多智能体模式对子智能体的总数或树的深度没有固定限制。对于大多数任务，请使用默认值 `max_concurrent_subagents` 的值为 `3`。该设置限制了整棵树中处于活跃状态的子智能体轮次数量，包括子节点以及更深的下层子智能体。

启用多智能体模式后，Responses API 提供六种托管协作操作。你可能会看到这些操作以 `multi_agent_call` 项的形式出现。你的应用程序不应执行这些操作，也无需为其提交输出。

| Action            | Purpose                                                                        |
| ----------------- | ------------------------------------------------------------------------------ |
| `spawn_agent`     | 创建一个子智能体并分配其初始任务。                                 |
| `send_message`    | 为已有的 智能体 排队一条消息，而不开启新的轮次。             |
| `followup_task`   | 向已有的非根 智能体 分配更多工作，并启动或恢复其轮次。   |
| `wait_agent`      | 等待调用方 智能体 邮箱中的更新。                             |
| `interrupt_agent` | 中断另一个 智能体 的活动轮次，但不删除其上下文。            |
| `list_agents`     | 返回当前 智能体 树、状态以及每个 智能体 的 `last_task_message`. |

处理开发者自定义的工具调用的方式与未启用多智能体 时相同。树中的任何智能体都可以发出一个 `function_call`。你的应用必须执行该调用并提交一个匹配的 `function_call_output`.

请注意，树中的所有智能体都可以访问在API 请求的模型调用中配置的工具。

## 在 Responses API 中使用多 智能体

### HTTP 与 WebSocket 性能对比

HTTP 和 WebSocket 支持同样的多智能体能力，但建议在工具密集或长时间运行的工作流中使用 WebSocket。它的持久连接允许你的应用在函数输出可用时立即返回，从而减少延续开销，并让智能体减少等待时间。

使用 HTTP 时，响应会在每个活跃智能体都完成或暂停等待客户端执行的函数调用后结束。随后你的应用会执行所有未完成的函数调用，并在新的 Responses API 请求中提交它们的输出，使暂停的智能体得以继续。

使用 WebSocket 时，你的应用可以在函数输出可用时立即将其注入响应，而无需等待当前响应完成。处于等待中的智能体可以立即恢复，而其他智能体则继续运行。当智能体在不同时间完成或请求工具时，这有助于减少协调延迟并避免额外的请求往返。

对于需要调用多个托管工具的工作流（例如并行的网页搜索），或函数调用较少的单请求工作流，HTTP 可能已经足够。对于大多数多智能体工作流，WebSocket 通常能提供更低的延迟和更好的端到端性能。
...
![整个应用、Responses API 根以及三个子智能体之间的 WebSocket 函数调用执行。](https://developers.openai.com/images/api/multi-agent/multi-agent-2.png)

### HTTP

这些示例需要使用暴露 beta SDK 构建版本的 beta Responses API。对于 HTTP 流式传输，调用 `client.beta.responses.create` 并传入 `responses_multi_agent=v1` 时使用 `betas` 参数；这会启用 beta 类型和自动补全。在 Python 中，从 `openai.types.beta` 导入 beta 响应项类型，用于添加类型注解。
...
在 WebSocket 模式下，当 智能体 调用开发者定义的函数时，请在你的应用中执行该函数，并使用一个 conversation.item.create 事件将其结果发送到当前响应。 `response.inject` 处于等待中的 智能体 即可恢复，而无需等待整个 Multi-智能体 响应完成。
...
对于有效的 `response.inject` 请求，服务器会以以下两种事件之一进行回复：
...
如果请求不符合 `response.inject` 架构，服务端会发送一个通用错误，状态码为 `400` 并关闭 WebSocket 连接。请修正请求，然后新建一个 WebSocket 连接，再发送另一个事件。

Python 试用版 SDK 通过 `client.beta.responses.connect`。暴露 WebSocket 模式。TypeScript 试用版 SDK 通过 `ResponsesWS`。暴露 WebSocket 模式。传入 `OpenAI-Beta: responses_multi_agent=v1` 作为连接请求头；与 HTTP 流式响应不同，WebSocket 连接器目前尚不接受 `betas` 参数。

保存该响应（response）的 ID，并在你为该响应发送的每个 `response.created` 事件中都附带该 ID。 `response.inject` event 中包含它。在发送一个注入项后，继续从 WebSocket 读取，直到响应完成，并且每个注入都已生成 `response.inject.created` 或 `response.inject.failed` 事件。
...
在发送 `response.inject` 事件后，继续从 WebSocket 读取并处理确认：

- **`response.inject.created`**: 函数输出已添加到活动响应中。请继续读取该响应的事件。
- **`response.inject.failed` with `response_already_completed`**: 响应在添加函数输出之前已完成。请使用失败事件中返回的 `input` ，并在新请求中发送，以从已完成的响应继续。 `response.create` request that continues from the completed response.
- **`response.inject.failed` with `response_not_found`**: 服务端无法找到所标识的响应 `response_id`。请确认你使用的是从 `response.created`.

单次 Multi-智能体 运行可能跨越多个 Responses API 请求。在 HTTP 上，当 智能体 调用开发者定义的函数时，你的应用会执行该函数，并在一次新的 `response.create` 调用中提交其输出。在 WebSocket 上，你的应用则改为将该函数输出注入到当前响应中。

## New Multi-智能体 output items

多智能体 响应可以包含三种额外的输出项类型：

- `multi_agent_call`：记录一项托管 Multi-智能体操作，例如 `spawn_agent`.
- `multi_agent_call_output`：包含托管操作执行的结果。
- `agent_message`：从一个智能体向另一个智能体传递加密消息。

该 `call_id` field 字段将每个 `multi_agent_call` 链接到对应的 `multi_agent_call_output`.

每个条目还包含一个 `agent` 属性。对于一个 `agent_message`, `agent.agent_name` 标识接收方 智能体。使用 `author` 和 `recipient` 来 追踪 消息方向。

当你的应用程序收到一个 `multi_agent_call`，时，不要将其作为函数调用执行或回传结果。Responses API 会执行托管操作并返回对应的 `multi_agent_call_output`。如果你的应用程序需要它们用于回放或 追踪，请同时保留这两个条目。
...
智能体 归属的 SSE 事件包含一个顶层 `agent` 属性。对于一个 `agent_message` 事件， `agent.agent_name` 标识接收方 智能体。响应生命周期事件（如 `response.created` 和 `response.completed` ）描述的是整个响应而不是单个 智能体，因此它们不包含 `agent` 属性。
...
1. Compaction：
   1. 该 `/responses/compact` endpoint 在启用了多智能体时不受支持。
   2. 当 `multi_agent.enabled` 设置为 `true`，时，即使请求未配置 服务端，也会隐式启用自动 compaction `context_management`。Compaction 会分别作用于根智能体和每个子智能体，保留它们各自的上下文。用户仍可通过在请求中显式设置 `compact_threshold` 来覆盖 `context_management.compact_threshold` 。
2. `reasoning.summary` 在启用多智能体时不受支持。
3. `max_tool_calls` 在启用多智能体时不受支持。
4. `max_concurrent_subagents` 默认为 `3`，这是推荐的设置。

## 提示词指南

当启用 Multi-智能体 时，我们的系统会自动将这些指令作为一条新的开发者消息附加到根 智能体 和子智能体中。你无法编辑或删除这些指令，但应将你的开发者指令表述为对这些自动注入指令的补充。
