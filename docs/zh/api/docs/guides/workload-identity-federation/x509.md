...
X.509 工作负载身份联合让工作负载可以将 TLS 客户端证书中的身份交换为短期 OpenAI 访问令牌。然后，工作负载会同时携带该访问令牌和已通过校验的客户端证书来调用 OpenAI API。该流程取代的是 API 密钥，而不是客户端证书。

X.509 工作负载身份联合可用于 OpenAI API。Codex 不
  支持该功能。对于 Codex，请使用 OIDC 令牌或 SPIFFE JWT-SVID，并遵循
  [Codex 工作负载身份指南](https://developers.openai.com/codex/enterprise/workload-identity).

有关令牌交换的请求和响应详细信息，请参阅 [工作负载身份令牌交换参考](https://developers.openai.com/api/reference/workload-identity-federation#exchange-an-x509-certificate)。有关 Mutual TLS 权限、证书要求、激活方式、mTLS 主机以及证书轮换，请参阅 [Mutual TLS 指南](https://developers.openai.com/api/docs/guides/mutual-tls).
...
2. X.509 工作负载身份提供方会从经验证的客户端证书中派生 `openai.*` 属性。它必须派生出一个非空的 `openai.subject` 值。
3. 服务账号映射会在项目内授权派生的身份使用一个 OpenAI 服务账号。
4. 工作负载向 X.509 令牌端点上的 `mtls.auth.openai.com` 提交其证书并请求一个短时 bearer 令牌。证书来自 TLS 连接；请求体中不包含 `subject_token`.
5. 工作负载将 bearer 令牌和客户端证书提交给 API 路由，地址为 `mtls.api.openai.com` 用于 API 授权。

Bearer 令牌和证书在 API 请求上是彼此独立授权的。仅凭证书并不能授权对 OpenAI API 的调用。
...
- 工作负载对应的项目和服务账号。
- 客户端证书、其私钥，以及构建到受信任根证书的证书路径所需的任何中间证书。
- 在组织或项目级别处于有效状态的受信根证书。

将私钥排除在源代码控制之外，并限制只有使用私钥的工作负载才能访问。不要记录私钥、证书内容或返回的访问令牌。

## 配置 Mutual TLS 证书信任

X.509 工作负载身份提供方会复用你组织现有的 Mutual TLS 证书配置。它们不会上传证书，也不会维护单独的证书信任库。

请参阅 [Mutual TLS 指南](https://developers.openai.com/api/docs/guides/mutual-tls) ，了解证书
...
采用 PEM 格式的可信证书，并针对组织或
将使用 X.509 工作负载身份联合的每个项目将其激活。

如果你的客户端证书通过中间证书进行链式信任，请配置稳定的信任锚点，并在 TLS 握手期间按顺序提供叶证书及当前的中间证书。OpenAI 会使用请求中提供的中间证书，而不会从证书 URL 中获取缺失的中间证书。
...
2. 选择 **X.509** 作为 **Provider type**，然后输入名称和可选的描述。X.509 提供方不使用 OIDC issuer、audience、discovery 或 JWKS 设置。创建后无法更改提供方类型。
3. 在 **Advanced**，下，可选择性地添加一个 **Attribute conditions** CEL 表达式，用于在映射解析之前拒绝证书。
4. 在 **Attribute transformations**，为所需的 `openai.subject` 转换输入一个非空表达式。仪表板会在你选择 X.509 时添加 `subject` 行，并显示和应用该 `openai.` 前缀。选择一个用于标识工作负载的稳定证书事实。
5. 可选择性地添加使用其他唯一 `openai.*` names，然后选择 **创建**.
...
证书信息可在 `assertion.subject` 和 `assertion.subject_alt_names`。下获取。用于映射的转换结果必须是标量值。额外的转换必须具有唯一的 `openai.*` 名称。

例如，一个 **属性条件** 表达式可以将提供方限制为生产证书：
...
2. 选择目标项目和API服务账号，仅授予该工作负载所需的 接口 权限。
3. 在 **密钥** 和 **Value** 字段，要求精确 `openai.subject` 值。X.509 映射支持无断言（表示为空对象（`{}`）），或断言的键以 `openai.`.
4. Select **创建**.
...
提供商列表显示提供商 ID，映射详情显示所选的服务账号及其服务账号 ID。请记录这两个标识符；工作负载在令牌交换期间会发送它们。
...
证书链文件应首先包含叶证书，然后是任何中间证书。不要在请求正文中包含证书材料或 `subject_token` 。
...
这些示例需要支持此处所示 X.509 配置的 OpenAI SDK 版本：JavaScript 7.8.0 或更高版本（已安装 `undici` 对等依赖），Python 3.6.0 或更高版本，Go 3.54.0 或更高版本，Java 4.55.0 或更高版本，以及 Ruby 0.83.0 或更高版本。

Java 示例加载 PKCS12 密钥库以构造其 `X509ExtendedKeyManager` ，并使用平台默认信任库来构造其 `X509TrustManager`。设置 `OPENAI_X509_KEYSTORE_PATH`, `OPENAI_X509_KEYSTORE_PASSWORD`，以及 `OPENAI_X509_CERTIFICATE_ALIAS` 用于本示例。你可以改为向 SDK 提供基于 PEM 或基于硬件的管理器。

## 手动交换证书

若要直接检视或实现令牌交换协议，可将证书出示给 X.509 令牌端点：
...
该 `expires_in` 值为 `3600` 仅为示例。当已验证的客户端证书更早过期时，返回的生命周期可能会更短。

请阅读 `access_token` 值从成功响应中读取到应用程序的凭据存储或环境变量中，例如 `OPENAI_WIF_ACCESS_TOKEN`。请将其视为机密，不要打印、记录或提交它。

## 手动调用 OpenAI API

将 `OPENAI_MODEL` 设置为 `gpt-6-astra`，当前默认值，或目标项目可用的其他模型。然后将 bearer token 和已接受的客户端证书发送到 API mTLS 端点：
...
使用 bearer token 代替 API key，并在 API 请求中继续出示已接受的客户端证书。

bearer token 未与证书进行加密绑定。API 请求中复用交换证书是最直接的配置，但 API 请求可以使用另一张同样独立满足当前 API mTLS 策略的证书。

## 令牌生命周期与续期

X.509 工作负载身份令牌最多一小时后过期，并且其有效期不会超过已验证的客户端证书。该交换不会返回刷新令牌。请重复执行证书交换以获取新的访问令牌。

轮换中间证书不需要更改已配置的根证书。在后续的交换和 API 请求中提供新的完整链即可。

## 排查令牌交换问题

X.509 令牌交换返回通用的 OAuth 错误，并未暴露证书、根证书、提供方或映射等详细信息。

| 结果                  | 常见原因                                                                                                                                                                                           |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HTTP `403`              | 请求使用了与精确 `POST /oauth/token` 以外的 `mtls.auth.openai.com`.                                                                                                        |
| `invalid_subject_token` | TLS 客户端证书缺失或无效，所提供的证书链无法追溯到有效的根证书，证书已超出有效期，或者被 Mutual TLS 证书准入规则拒绝。 |
| `invalid_grant`         | 提供商或映射无效或已被禁用，提供商 **属性条件** 表达式拒绝了该身份，没有适用的根证书处于启用状态，或者没有匹配的映射。                              |
| 服务器错误            | OpenAI 返回了临时性服务器错误。请按照你平时的瞬时错误重试策略进行重试。                                                                                                         |

X.509 交换绝不会回退到 OIDC 或普通的 OAuth 流程。

## 限制

- X.509 Workload Identity Providers 不维护单独的证书信任存储。
- Bearer 令牌不绑定证书，也不使用 DPoP 或 `cnf` claim。
- 证书交换不是仅证书 API 授权。API 请求仍然需要 Bearer 令牌和被接受的客户端证书。
- OpenAI 不会从 AIA URL 拉取缺失的中间证书。请在 TLS 协商期间出示完整的证书链。
- OpenAI 在此流程中不执行证书吊销列表（CRL）或 OCSP 检查。请围绕 Mutual TLS 根证书、Provider、映射控制以及所颁发令牌的较短有效期来规划证书事件响应。
- 此流程不增加对 SPIFFE X.509-SVID 的支持。 [SPIFFE 指南](https://developers.openai.com/api/docs/guides/workload-identity-federation/spiffe) 仍然使用 JWT-SVID。