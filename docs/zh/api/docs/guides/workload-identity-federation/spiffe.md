...
使用 SPIFFE 作为工作负载身份提供方，通过交换 SPIFFE JWT-SVID 来获取一个短期有效的 OpenAI 访问令牌。这使得由 SPIRE 或其他兼容 SPIFFE 的身份提供方完成认证的工作负载能够在不存储长期 API 密钥的情况下调用 OpenAI API。

OpenAI 支持 SPIFFE JWT-SVID，它们可作为 JWT 主体令牌进行验证，并包含颁发者、受众、过期时间、颁发时间戳以及基于 JWKS 的签名。OpenAI 不支持将 SPIFFE X.509-SVID 用作工作负载身份联合主体令牌。

JWT-SVID 规范要求 `sub`, `aud`，以及 `exp` 声明。要将 JWT-SVID 与 OpenAI 一起使用，该令牌还必须包含 `iss` 和 `iat` 声明以及一个 `kid` 头，以便 OpenAI 能够根据工作负载身份提供方配置验证该令牌。

JWT-SVID 不是 OpenID Connect ID 令牌。SPIRE OIDC 发现提供方提供发现元数据和 JWKS 密钥，以便 OpenAI 能够验证 JWT-SVID；它不会改变该令牌的 SPIFFE 语义，也不要求使用 OIDC 登录流程。
...
将你的 SPIFFE 提供方配置为向需要调用 OpenAI API 的工作负载签发 JWT-SVID。这些说明使用 SPIRE 术语，但相同的 OpenAI 配置适用于任何能签发 OpenAI 可验证的、带有 issuer 和 JWKS 签名材料的 JWT-SVID 的 SPIFFE 兼容提供方。
...
- 出现在 JWT-SVID 声明中的 JWT 颁发者 URL `iss` 用于 OpenAI 验证。
- JWT-SVID 签名密钥的公共 JWKS，可通过 OIDC 发现或上传的 JWKS 提供。
- 从 SPIFFE Workload API 获取最新 JWT-SVID 的工作负载侧方式。

受众是一个精确匹配的标识符，不一定是接收 JWT-SVID 的端点。你可以使用 `https://api.openai.com/v1` 或另一个服务特定的值，只要 SPIFFE Workload API 请求和 OpenAI 提供商配置相匹配。
...
OIDC Discovery Provider 配置还需要一个密钥材料来源，例如 `server_api`, `workload_api`，或 `file`，以及一种服务机制，例如 ACME、TLS 证书或 Unix 套接字。请参阅 [SPIRE OIDC Discovery Provider 文档](https://github.com/spiffe/spire/tree/main/support/oidc-discovery-provider) 了解完整的配置选项。

SPIFFE 信任域和 JWT 颁发者是不同的概念。在本示例中，JWT-SVID 的 subject 是位于 `example.org` 信任域中的一个 SPIFFE ID，而颁发者是 HTTPS 颁发者 URL：
...
SPIRE OIDC Discovery Provider 提供一个 OIDC 发现文档和一个 JWKS 端点，OpenAI 可以在以下情况时使用 **使用已上传的 JWKS 进行令牌验证** 被禁用时。

如果 OpenAI 无法访问你的颁发者发现端点，请改用已上传 JWKS 模式。在该模式下，OpenAI 仍会将 Workload Identity Provider 颁发者与 JWT-SVID 的 `iss` claim 进行比较，但会根据你在 Workload Identity Provider 上保存的 JWKS JSON 验证签名。

> **注意：** SPIFFE JWT-SVID 规范将 JWT 头部设为可选，但 OpenAI 要求 JWT 主体令牌必须包含该 `kid` header so it can select the signing key from the configured JWKS. If your SPIFFE provider can omit `kid` , configure it to include one for OpenAI workload identity federation. `kid`，请配置为包含该头部，以便与 该公司 工作负载身份联合一起使用。

若要通过可调用 SPIFFE Workload API 的工作负载检查 JWT-SVID，请为你将在 OpenAI 中配置的同一个 audience 请求一个 JWT-SVID。在与应用程序相同的工作负载上下文中运行该命令，因为 Workload API 授权依赖于调用进程的身份。
...
如果你的工作负载拥有多个 SPIFFE ID，请请求特定的身份：
...
在配置工作负载身份联合之前，将 JWT-SVID 导出为 `TOKEN`，然后在本地运行以下示例之一以查看其 header 和 claims：
...
每个示例都会解码 JWT，但不验证令牌签名。生产环境的令牌请使用本地解码器，避免将生产环境的令牌粘贴到第三方工具中。

解码后的 SPIFFE JWT-SVID 形如：
...
使用解码后的令牌，在交换之前将你收到的令牌与 OpenAI 配置进行比较。请检查 `alg` 和 `kid` 在 header 中，以及 `iss`, `aud`, `sub`, `iat`，以及 `exp` 在 payload 中。具体的 `alg` 取值取决于你的 SPIRE Server JWT 签名密钥配置。

## 配置工作负载身份联合

在 OpenAI 中为 SPIFFE JWT-SVID 颁发者创建一个工作负载身份提供者，然后添加一个与你所信任的 SPIFFE ID 相匹配的服务账号映射。
...
2. **设置 issuer 和 audience。** 将 **OIDC Issuer URL** 为 JWT-SVID 中 `iss` 声明的精确值，例如 `https://spire-oidc.example.org`。将 **Audience** 设置为 SPIFFE Workload API 所请求的 audience 值。在本例中，该值为 `https://api.openai.com/v1`.

3. **选择 JWKS 来源。** 将 **Use uploaded JWKS for token verification** 保持未启用，前提是 OpenAI 能够访问你的 SPIRE OIDC Discovery Provider。OpenAI 会通过 OIDC 发现机制以及所发现的 JWKS 来验证 JWT-SVID 签名。

   如果 OpenAI 无法访问该 issuer，请启用 **Use uploaded JWKS for token verification**，然后设置 **JWKS JSON** 为 JWT-SVID 签名密钥对应的公钥集。上传完整的公钥 JWKS 对象，包括外层的 `keys` 数组。请勿包含任何私钥材料。

4. **仅在需要派生映射属性时添加属性转换。** 在直接从 `sub`。进行映射时，属性转换不是必需的。只有当你需要从一个或多个 token 声明派生映射值时才使用它们。详见 [主工作负载身份联合指南](https://developers.openai.com/api/docs/guides/workload-identity-federation#transform-token-claims-with-cel) 以了解转换行为。

### 设置服务账号映射

1. **创建一个服务账号映射。** 将 **Name** 到 Workload Identity Provider 内的唯一值，例如 `production-openai-wif`。使用 **Description**，例如 `Production SPIFFE workload for OpenAI API access`，以说明哪个工作负载可以使用该映射。

2. **匹配 SPIFFE ID。** 将 **Key** 为 `sub` ， **Value** 为该工作负载的 SPIFFE ID，例如 `spiffe://example.org/ns/production/sa/openai-wif`.

   对于特权工作负载，优先使用精确的 SPIFFE ID 匹配。仅当该前缀下的每个 SPIFFE ID 都应能够签发 OpenAI 访问令牌时，才使用尾部通配符。例如， `spiffe://example.org/ns/production/sa/*` 允许任何匹配的生产环境服务账号路径。

3. **选择 OpenAI 目标。** 将 **Project** 为拥有目标服务账号的 OpenAI 项目。设置 **Service account** 为 SPIFFE 工作负载可以使用的 OpenAI 服务账号，例如 `spiffe-prod-openai-wif`。勾选 `Create a new service account in this project` ，如果你希望为此映射创建一个新的服务账号而不是复用现有账号。

4. **如有必要，收窄 API 权限。** 选择合适的 **权限** 例如 `api.model.request` ， `api.vector_store.read` 以进一步收窄从此映射颁发的访问令牌的范围。保持权限为空可避免添加 WIF 专属的作用域限制；令牌仍会以所映射的服务账号身份进行授权。

## 在代码中使用令牌

配置你的 OpenAI SDK 客户端，以便使用一个全新的 SPIFFE JWT-SVID 换取由 OpenAI 颁发的访问令牌。

下方示例假设你的 SPIFFE 集成会刷新一个 JWT-SVID 并将其写入SDK `/var/run/spiffe/openai.jwt`。请确保该文件仅对工作负载可读。由于 JWT-SVID 有效期较短，请在令牌过期之前刷新该文件。作为替代方案，如果可能，可以在 subject token provider 中使用特定语言的 SPIFFE 库直接从 SPIFFE Workload API 获取 JWT-SVID，以避免使用过期的令牌文件。

设置 `OPENAI_IDENTITY_PROVIDER_ID` 和 `OPENAI_SERVICE_ACCOUNT_ID` 在工作负载环境中。该令牌文件包含外部 subject token。 `OPENAI_IDENTITY_PROVIDER_ID` 标识 OpenAI Workload Identity Provider，而 `OPENAI_SERVICE_ACCOUNT_ID` 标识目标 OpenAI 服务账号。OpenAI 然后会根据令牌声明找到该 Provider 与服务账号之间的匹配映射。
...
- 使用 JWT-SVID 进行 OpenAI 工作负载身份联合。X.509-SVID 适用于双向 TLS，但不被 OpenAI 令牌交换端点接受。
- 为 OpenAI 访问使用单一专用受众。避免使用诸如整个信任域或环境名称之类的宽泛受众。
- 尽可能匹配精确的 SPIFFE ID。仅在刻意共享的信任边界中使用通配符映射。
- 保持 JWT-SVID 的生命周期较短，以降低持有者令牌重放风险。OpenAI 访问令牌的过期时间绝不会超过用于交换的外部主体令牌。
- 谨慎轮换签名密钥。在轮换窗口期间通过 OIDC 发现同时发布新旧公钥，或者在签发使用新签名密钥的 JWT-SVID 之前更新已上传的公钥 JWKS `kid`.
- 保持 SPIRE Server 和工作负载时钟同步。显著的时钟偏移可能导致原本有效的 JWT-SVID 因尚未生效、过旧或已过期而被拒绝。
- 保护 SPIFFE Workload API 套接字。能够获取工作负载 JWT-SVID 的进程可以尝试将其交换为 OpenAI 访问令牌。
- 将 OpenAI 服务账号边界与你的应用程序及环境权限边界对齐。不要在互不相关的 SPIFFE 工作负载之间共享高权限服务账号。
- 监控令牌交换失败中的颁发者、受众、签名密钥和映射不匹配情况。