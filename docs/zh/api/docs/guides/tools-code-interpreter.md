...
- 处理包含多样化数据和格式的文件
- 生成包含数据和图表图像的文件
- 迭代地编写并运行代码以解决问题——例如，对于编写后无法运行的代码，模型可以不断重写并重新运行，直到成功
- 在我们最新的推理模型（例如 [o3](https://developers.openai.com/api/docs/models/o3) 和 [o4-mini](https://developers.openai.com/api/docs/models/o4-mini)）中增强视觉智能。模型可以使用该工具对图像进行裁剪、缩放、旋转以及其他处理与转换操作。

下面是一个调用 [Responses API](https://developers.openai.com/api/reference/resources/responses) 并使用 Code Interpreter 工具调用的示例：
...
虽然我们把这个工具称为 Code Interpreter，但模型将其识别为 "python
  tool"。模型通常能够理解那些提及 code interpreter
  tool 的提示，不过最明确的调用方式是在提示中请求 "the
  python tool"。

## Containers

Code Interpreter 工具需要一个 [容器对象](https://developers.openai.com/api/reference/resources/containers)。容器是一台完全沙箱化的虚拟机，模型可以在其中运行 Python 代码。该容器可以包含你上传的文件或它生成的文件。

有两种创建容器的方式：

1. Auto 模式：如上例所示，你可以在创建新的 Response 对象时，通过在工具配置中传入 `"container": { "type": "auto", "memory_limit": "4g", "file_ids": ["file-1", "file-2"] }` 属性来实现。这会自动创建一个新的容器，或复用模型上下文中上一个 `code_interpreter_call` 项所使用的处于活跃状态的容器。如果省略 `memory_limit` ，则会保持容器默认的 1 GB 层级。在本 API 请求的输出中查找 `code_interpreter_call` 项，以找到 `container_id` 所生成或使用的容器。
2. 显式模式：在这里，你显式地 [创建一个容器](https://developers.openai.com/api/reference/resources/containers/methods/create) 通过使用 `v1/containers` 端点，包括你需要的 `memory_limit` （例如 `"memory_limit": "4g"`），并将其 `id` 作为 `container` 值赋给 Response 对象工具配置中的对应字段。例如：
...
你可以选择 `1g` （默认）、 `4g`, `16g`，或 `64g`。更高的层级为会话提供更多内存，并按 [Code Interpreter 的内置工具费率](https://developers.openai.com/api/docs/pricing#built-in-tools) 计费。所选层级适用于该容器的整个生命周期，无论它是自动创建的还是通过容器 API 创建的。 `memory_limit` applies for the entire life of that container, whether it was created automatically or via the containers 接口.

请注意，使用 auto 模式创建的容器也可通过 [`/v1/containers`](https://developers.openai.com/api/reference/resources/containers) 端点访问。

### Expiration

我们强烈建议你将容器视为临时的，并将与此工具使用相关的所有数据存储在你自己的系统中。过期详情：

- 如果容器在 20 分钟内未被使用，则会过期。发生这种情况时，在 `v1/responses` 中使用该容器将失败。你仍然可以在容器过期时查看其元数据快照，但与该容器相关的所有数据都将从我们的系统中丢弃且无法恢复。你应该在容器处于活动状态时下载可能需要的任何文件。
- 无法将容器从过期状态恢复为活动状态。请改为创建一个新容器并重新上传文件。请注意，旧容器内存中的任何状态（例如 Python 对象）都将丢失。
- 任何容器操作（例如检索容器，或者在容器中添加或删除文件）都会自动刷新容器的 `last_active_at` 时间。

## 处理文件

运行 Code Interpreter 时，模型可以创建自己的文件。例如，如果你要求它构建图表或创建 CSV，它会直接在容器上创建这些图像。创建时，它会在 `annotations` 中引用这些文件。示例如下：
...
你可以通过调用 [get container file content](https://developers.openai.com/api/reference/resources/containers/subresources/files/subresources/content/methods/retrieve) 方法下载这些构建的文件。

任何 [模型输入中的文件](https://developers.openai.com/api/docs/guides/file-inputs) 都会自动上传到容器。你无需显式将其上传到容器。

### 上传和下载文件

使用以下接口向你的容器添加新文件 [创建容器文件](https://developers.openai.com/api/reference/resources/containers/subresources/files/methods/create)。该接口既支持 multipart 上传，也支持使用以下字段的 JSON 请求体： `file_id`.
使用以下接口列出容器中的现有文件 [列出容器文件](https://developers.openai.com/api/reference/resources/containers/subresources/files/methods/list) 并通过以下接口下载文件字节内容： [获取容器文件内容](https://developers.openai.com/api/reference/resources/containers/subresources/files/subresources/content/methods/retrieve).

### 处理引用

模型生成的文件和图像会作为注解返回在助手的消息上。 `container_file_citation` 注解指向在容器中创建的文件。它们包含文件路径 `container_id`, `file_id`，以及 `filename`。你可以解析这些注解以呈现下载链接或对文件进行其他处理。
