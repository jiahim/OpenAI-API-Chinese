...
OpenAI API 提供了一个一致的接口来访问最先进的 AI [模型](https://developers.openai.com/api/docs/models) 用于文本生成、自然语言处理、计算机视觉等。开始使用时，请先创建一个 API 密钥，并运行你的第一次 API 调用。了解如何生成文本、分析图像、构建 智能体 等。
...
保存在安全的位置，例如 [`.zshrc`
文件](https://www.freecodecamp.org/news/how-do-zsh-configuration-files-work/) 或者
计算机上的另一个文本文件。生成 API 密钥后，将它导出
...
要在 Node.js、Deno 或 Bun 等 服务端 JavaScript 环境中使用 OpenAI API，你可以使用官方的 [OpenAI SDK for TypeScript and JavaScript](https://github.com/openai/openai-node)。先使用 SDK 进行安装 [npm](https://www.npmjs.com/) 或你常用的包管理器：
...
安装好 OpenAI SDK 后，新建一个文件，文件名 `example.mjs` ，并将示例代码复制到其中：
...
执行以下命令运行代码 `node example.mjs` （或 Deno、Bun 中对应的命令）。稍等片刻，你将看到 API 请求的输出。
...
要在 Python 中使用 OpenAI API，你可以使用官方的 [OpenAI SDK for Python](https://github.com/openai/openai-python)。先使用 SDK 进行安装 [pip](https://pypi.org/project/pip/):
...
安装好 OpenAI SDK 后，新建一个文件，文件名 `example.py` ，并将示例代码复制到其中：
...
执行以下命令运行代码 `python example.py`。稍等片刻，你将看到 API 请求的输出。
...
向 API 发出的简单请求如下，目标是 [Responses API](https://developers.openai.com/api/reference/resources/responses) 所示：
...
OpenAI 为 Java 编程语言提供 API 帮助库，当前处于 beta 阶段。你可以使用以下配置添加 Maven 依赖：
...
OpenAI 为 Go 编程语言提供 API 帮助库，当前处于 beta 阶段。你可以使用下面的代码导入该库：
...
向 API 发出的第一个请求如下，目标是 [Responses API](https://developers.openai.com/api/reference/resources/responses) 所示：
...
安装好 OpenAI SDK 后，新建一个文件，文件名 `example.rb` ，并将示例代码复制到其中：
...
执行以下命令运行代码 `ruby example.rb`。稍等片刻，你将看到 API 请求的输出。
...
恭喜你成功运行了一次免费的测试 API 请求！开始使用更高额度构建实际应用，并使用 [我们的模型](https://developers.openai.com/api/docs/models) 生成文本、音频、图像、视频等内容。
...
将图片链接、上传的文件或 PDF 文档直接发送给模型，以提取文本、对内容进行分类或检测视觉元素。
...
通过附加 [工具](https://developers.openai.com/api/docs/guides/tools)，让模型能够访问外部数据和函数。可以使用内置工具，例如网页搜索或文件搜索，也可以自定义工具来调用 API、运行代码或与第三方系统集成。
...
使用服务端发送 [流式事件](https://developers.openai.com/api/docs/guides/streaming-responses) 以在结果生成时实时显示，也可以使用 [Realtime API](https://developers.openai.com/api/docs/guides/realtime) 构建交互式语音应用以及支持文本、音频和图像输入的应用。
...
使用 OpenAI 平台构建 [智能体](https://developers.openai.com/api/docs/guides/agents) 能够代表你的用户采取行动，例如 [控制计算机](https://developers.openai.com/api/docs/guides/tools-computer-use)。在你的服务器上使用 [Agents SDK](https://developers.openai.com/api/docs/guides/agents) 创建编排逻辑。
