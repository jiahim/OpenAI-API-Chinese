...
OpenAI API 提供了一个一致的接口，用于访问最先进的 AI [模型](https://developers.openai.com/api/docs/models) ，支持文本生成、自然语言处理、计算机视觉等场景。创建一个 API Key 并运行你的第一次 API 调用即可开始使用。探索如何生成文本、分析图像、构建智能体等。
...
安全地 [访问该 API](https://developers.openai.com/api/reference/overview)。请将该密钥
存放在安全的位置，例如 [`.zshrc`
...
要在 Node.js、Deno 或 Bun 等 服务端 JavaScript 环境中使用 OpenAI API，你可以使用官方的 [OpenAI SDK for TypeScript and JavaScript](https://github.com/openai/openai-node)。先通过以下方式安装 SDK： [npm](https://www.npmjs.com/) 或你常用的包管理器：
...
安装好 OpenAI SDK 后，新建一个文件 `example.mjs` 并将示例代码复制到该文件中：
...
使用以下命令执行代码 `node example.mjs` （或 Deno、Bun 中对应的命令）。稍等片刻，你应当能看到 API 请求的输出。
...
要在 Python 中使用 OpenAI API，你可以使用官方的 [OpenAI SDK for Python](https://github.com/openai/openai-python)。先通过以下方式安装 SDK： [pip](https://pypi.org/project/pip/):
...
安装好 OpenAI SDK 后，新建一个文件 `example.py` 并将示例代码复制到该文件中：
...
该公司 与 Microsoft 合作，提供了一个官方支持的 C# API 客户端 OpenAI。你可以使用 .NET CLI 从 [NuGet](https://www.nuget.org/).

```
dotnet add package OpenAI
```

一个向 API 发起的简单请求，目标是 [Responses API](https://developers.openai.com/api/reference/resources/responses) 大致如下：
...
向API 发起的第一个请求 [Responses API](https://developers.openai.com/api/reference/resources/responses) 大致如下：
...
安装好 OpenAI SDK 后，新建一个文件 `example.rb` 并将示例代码复制到该文件中：
...
通过附加 [工具](https://developers.openai.com/api/docs/guides/tools)，为模型提供对外部数据和函数的访问。使用网页搜索或文件搜索等内置工具，也可以自定义工具用于调用API、运行代码或与第三方系统集成。
...
使用服务端发送 [流式事件](https://developers.openai.com/api/docs/guides/streaming-responses) 以在结果生成时即时展示，或使用 [Realtime API](https://developers.openai.com/api/docs/guides/realtime) 构建支持文本、音频和图像输入的交互式语音应用。
...
使用 OpenAI 平台来构建 [智能体](https://developers.openai.com/api/docs/guides/agents) ，使其能够代表你的用户采取行动——例如 [控制计算机](https://developers.openai.com/api/docs/guides/tools-computer-use)。使用 Agents SDK [智能体开发工具包](https://developers.openai.com/api/docs/guides/agents) 在你的服务端创建编排逻辑。
