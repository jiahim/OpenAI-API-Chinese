...
- `conversation: optional string or ResponseConversationParam or null`
  ...
  此响应所属的对话。 `input_items` 针对此响应请求的 conversation ID。
  此响应的输入项和输出项会在此响应完成后自动添加到此对话中。
...
- `input: optional string or array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more or null`
  ...
  - `array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more`
    ...
    - `EasyInputMessage object { content, role, phase, type }`
      ...
      发送给模型的消息输入，其角色表示指令遵循
      的优先级层次。使用 `developer` 或 `system` 角色给出的指令
      优先于使用 `user` 角色给出的指令。带有
      `assistant` 角色的消息假定由模型在之前的
      交互中生成。
      ...
      - `content: string or ResponseInputMessageContentList`
        ...
        - `ResponseInputMessageContentList = array of ResponseInputContent`
          ...
          一个由一条或多条输入项组成的列表，发送给模型，包含不同的内容
          类型。
          ...
          - `ResponseInputImage object { detail, type, file_id, 2 more }`
            ...
            - `image_url: optional string or null`
              ...
              发送给模型的图像 URL。可以是完整的 URL，也可以是 base64 编码的 data URL 中的图像。
      ...
      - `phase: optional "commentary" or "final_answer" or null`
        ...
        将 `assistant` 消息标记为中间说明性内容（`commentary`）或最终答案（`final_answer`).
        对于 `gpt-5.3-codex` 及更高版本等模型，在发送后续请求时，请在所有助手消息上保留并重新发送
        阶段；省略该字段可能导致性能下降。用户消息不使用此字段。
    ...
    - `ComputerCall object { id, call_id, pending_safety_checks, 4 more }`
      ...
      - `action: optional ComputerAction`
        ...
        - `Click object { button, type, x, 2 more }`
          ...
          - `button: "left" or "right" or "wheel" or 2 more`
            ...
            指示点击时按下的是哪个鼠标按钮。取值之一为 `left`, `right`, `wheel`, `back`，或 `forward`.
      ...
      - `actions: optional ComputerActionList`
        ...
        为以下对象展开后的批量操作 `computer_use`。每个操作包含一个
        `type` 判别字段以及操作专属字段。
    ...
    - `FunctionCallOutput object { output, type, id, 5 more }`
      ...
      - `status: optional "in_progress" or "completed" or "incomplete" or null`
        ...
        item 的状态。取值之一为 `in_progress`, `completed`，或 `incomplete`。当通过 API 返回项时填充。
    ...
    - `ToolSearchOutput object { tools, type, id, 3 more }`
      ...
      - `tools: array of object { name, parameters, strict, 6 more }  or object { type, vector_store_ids, filters, 2 more }  or object { type }  or 13 more`
        ...
        - `Function object { name, parameters, strict, 6 more }`
          ...
          定义你自己的代码中模型可以选择调用的函数。了解更多关于 [函数调用](https://platform.openai.com/docs/guides/function-calling).
        ...
        - `FileSearch object { type, vector_store_ids, filters, 2 more }`
          ...
          用于从已上传文件中搜索相关内容的一个工具。详细了解 [文件搜索 工具](https://platform.openai.com/docs/guides/tools-file-search).
          ...
          - `filters: optional ComparisonFilter or CompoundFilter or null`
            ...
            - `ComparisonFilter object { key, type, value }`
              ...
              用于使用指定的比较运算将给定的属性键与给定值进行比较的筛选器。
        ...
        - `WebSearch object { type, external_web_access, filters, 2 more }`
          ...
          - `external_web_access: optional boolean`
            ...
            允许 网页搜索 使用实时互联网访问。省略时默认为 true。当为 false 时，网页搜索 工具以离线/仅缓存模式运行，不会获取新的外部内容。
          ...
          - `search_context_size: optional "low" or "medium" or "high"`
            ...
            用于搜索的上下文窗口空间的高级指导量。取值之一为 `low`, `medium`，或 `high`. `medium` 是默认值。
          ...
          - `user_location: optional object { city, country, region, 2 more }  or null`
            ...
            - `country: optional string or null`
              ...
              两位字母的 [ISO 国家代码](https://en.wikipedia.org/wiki/ISO_3166-1) 用户的，例如。 `US`.
        ...
        - `Mcp object { server_label, type, allowed_callers, 9 more }`
          ...
          - `allowed_tools: optional array of string or object { read_only, tool_names }  or null`
            ...
            - `McpToolFilter object { read_only, tool_names }`
              ...
              - `read_only: optional boolean`
                ...
                指示工具是否修改数据或是否为只读。如果 MCP 服务器被
                标注为 [annotated with `readOnlyHint`](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations-readonlyhint),
                ，则该工具会匹配此过滤器。
          ...
          - `tunnel_id: optional string`
            ...
            要使用的 Secure MCP Tunnel ID，用于替代直接的服务端 URL。二选一：
            `server_url`, `connector_id`，或 `tunnel_id` 必填。
        ...
        - `ImageGeneration object { type, action, background, 9 more }`
          ...
          - `input_fidelity: optional "high" or "low" or null`
            ...
            控制模型在匹配输入图像的风格和特征（尤其是面部特征）时所付出的努力程度。此参数仅在 `gpt-image-1` 和 `gpt-image-1.5` 及更高版本的模型中受支持，在 `gpt-image-1-mini`。中不受支持。支持 `high` 和 `low`。默认为 `low`.
          ...
          - `size: optional string or "1024x1024" or "1024x1536" or "1536x1024" or "auto"`
            ...
            生成图像的尺寸。对于 `gpt-image-2` 和 `gpt-image-2-2026-04-21`，支持任意分辨率， `WIDTHxHEIGHT` 以字符串形式表示，例如 `1536x864`。宽度和高度都必须能被 16 整除，且请求的宽高比必须介于 1:3 和 3:1 之间。高于 `2560x1440` 的分辨率为实验性，最大支持的分辨率为 `3840x2160`。所请求的尺寸还必须满足模型当前的像素和边长限制。标准尺寸 `1024x1024`, `1536x1024`，和 `1024x1536` 由 GPT 图像模型支持； `auto` 由支持自动尺寸的模型支持。对于 `dall-e-2`，使用以下之一 `256x256`, `512x512`，或 `1024x1024`。对于 `dall-e-3`，使用以下之一 `1024x1024`, `1792x1024`，或 `1024x1792`.
        ...
        - `Custom object { name, type, allowed_callers, 4 more }`
          ...
          使用指定格式处理输入的自定义工具。了解更多关于   [custom tools](/docs/guides/function-calling#custom-tools)
          ...
          - `tools: array of object { name, type, allowed_callers, 6 more }  or object { name, type, allowed_callers, 4 more }`
            ...
            - `Function object { name, type, allowed_callers, 6 more }`
              ...
              - `output_schema: optional map[unknown] or null`
                ...
                用于描述此函数工具的字符串输出中所编码 JSON 值的 JSON Schema。这不描述 content 数组输出。
        ...
        - `WebSearchPreview object { type, search_content_types, search_context_size, user_location }`
          ...
          该工具会在网页上搜索相关信息以用于回复中。详细了解 [网页搜索工具](https://platform.openai.com/docs/guides/tools-web-search).
    ...
    - `Reasoning object { id, summary, type, 3 more }`
      ...
      若你在手动管理上下文，请务必在后续对话轮次中将这些项包含在你发给 `input` Responses API 的请求中
      。
      [的管理上下文中](/docs/guides/conversation-state).
      ...
      - `encrypted_content: optional string or null`
        ...
        推理项的加密内容。该内容默认由
        为通过 `POST /v1/responses` 和 WebSocket
        `response.create` 请求返回的推理项填充。

        在流式传输时，请在后续请求中使用已完成的推理项及其
        `encrypted_content` 来自 `response.output_item.done` 事件的
        。 `encrypted_content` 在
        `response.output_item.added` 可能不完整的情况下，这一点尤为
        重要，例如 `store` 是 `false` ，或在使用 Zero Data Retention 时。
    ...
    - `McpCall object { id, arguments, name, 6 more }`
      ...
      - `status: optional "in_progress" or "completed" or "incomplete" or 2 more`
        ...
        工具调用的状态。可为以下之一： `in_progress`, `completed`, `incomplete`, `calling`，或 `failed`.
...
- `model: optional string or null`
  ...
  用于生成响应的模型 ID，例如 `gpt-4o` 或 `o3`。OpenAI 提供了多种具有不同能力、性能特征和价格的模型。请参阅 [模型指南](/docs/models) 以浏览和比较可用模型。
...
- `personality: optional string or "friendly" or "pragmatic"`
  ...
  应用于此请求的模型自有样式预设。省略此参数将使用模型的默认样式。支持的值可能会随时间增加。值最多为 64 个字符。
...
- `previous_response_id: optional string or null`
  ...
  模型上一次响应的唯一 ID。使用它可以创建多轮对话。了解更多关于 [对话状态](/docs/guides/conversation-state)。的信息。不能与 `conversation`.

- `reasoning: optional Reasoning or null`

  **仅限 gpt-5 和 o 系列模型** 的配置选项，用于 [推理模型](https://platform.openai.com/docs/guides/reasoning).

  - `context: optional "auto" or "current_turn" or "all_turns" or null`

    控制在后续轮次中哪些推理项会被重新渲染回模型。
    如果省略或设置为 `auto`，模型将自行决定上下文模式。
    `gpt-5.6` 模型族默认为 `all_turns`；早期模型默认为
    `current_turn`.
  ...
  - `effort: optional ReasoningEffort or null`
    ...
    降低推理投入程度可以让响应更快，并减少响应中用于推理的 token 数量。并非所有推理模型都支持
    所有取值。请参阅
    推理指南
...
- `text: optional object { format, verbosity }  or null`
  ...
  - `format: optional ResponseFormatTextConfig`
    ...
    将 `{ "type": "json_schema" }` 设置为，
    可启用结构化输出，确保模型匹配你提供的 JSON schema。更多信息请参阅
    ...
    **不建议用于 gpt-4o 及更新模型：**
    ...
    - `ResponseFormatTextJSONSchemaConfig object { name, schema, type, 2 more }`
      ...
      - `strict: optional boolean or null`
        ...
        是否在生成输出时启用严格的架构遵循。
        若设置为 true，模型将始终遵循所定义的精确架构
        字段。仅支持 JSON Schema 的一个子集，当 `schema` 时可用。了解更多信息，请参阅
        `strict` 是 `true`。指南。 [结构化输出
        指南](/docs/guides/structured-outputs).

    - `ResponseFormatJSONObject object { type }`

      JSON 对象响应格式。一种较早的生成 JSON 响应的方法。
      对于支持它的模型，推荐使用 `json_schema` 。请注意，如果没有系统或用户消息指示，
      模型将不会生成 JSON
      输出。
...
- `truncation: optional "auto" or "disabled"`
  ...
  用于模型响应的截断策略。 - `auto`：如果此响应的输入超出模型的上下文窗口大小，模型将通过从对话开头丢弃条目来截断响应以适应上下文窗口。 - `disabled` （默认）：如果输入大小将超出模型的上下文窗口大小，请求将失败并返回 400 错误。
