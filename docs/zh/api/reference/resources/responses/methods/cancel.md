...
- `Response object { id, created_at, error, 33 more }`
  ...
  - `incomplete_details: object { reason }  or null`
    ...
    - `reason: optional "max_output_tokens" or "max_messages" or "content_filter" or "steered"`
      ...
      响应未完成的原因。 `steered` 表示
      响应在一次
      WebSocket `response.steer` 事件后于安全输出边界处停止。服务端随后可以使用排队的输入
      自动创建一个后续响应。
  ...
  - `instructions: string or array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more or null`
    ...
    当与 `previous_response_id`，一起使用时，来自上一个响应的指令将
    不会延续到下一个响应。这使得以下操作变得简单
    用于在新响应中替换 system（或 developer）消息。
    ...
    - `InputItemList = array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more`
      ...
      - `EasyInputMessage object { content, role, phase, type }`
        ...
        发送给模型的消息输入，其角色表示指令的层级。使用
        或 `developer` 或 `system` 角色给出的指令优先级高于使用
        角色给出的指令。使用 `user` 角色的消息假定为模型在之前的
        `assistant` 交互中生成的内容。
        交互。
          ...
          - `ResponseInputMessageContentList = array of ResponseInputContent`
            ...
            - `ResponseInputFile object { type, detail, file_data, 4 more }`
              ...
              - `detail: optional "auto" or "low" or "high"`
                ...
                发送给模型的文件的细节级别。使用 `auto` 可由系统选择细节级别；对于 GPT-5.6 及更高版本的模型， `auto` 使用高质量渲染，可能会增加输入 token 使用量。使用 `low` 可获得更低成本的渲染，或使用 `high` 以更高质量渲染文件。默认为 `auto`.
        ...
        - `phase: optional "commentary" or "final_answer" or null`
          ...
          将 `assistant` 消息标记为中间推理内容（`commentary`）或最终回复（`final_answer`).
          对于像 `gpt-5.3-codex` 及更高版本的模型，发送后续请求时，请在所有助手消息上保留并重新发送
          阶段——若省略该字段，可能导致性能下降。不适用于用户消息。
      ...
      - `ComputerCall object { id, call_id, pending_safety_checks, 4 more }`
        ...
        - `action: optional ComputerAction`
          ...
          - `Click object { button, type, x, 2 more }`
            ...
            - `button: "left" or "right" or "wheel" or 2 more`
              ...
              表示点击时按下了哪个鼠标按键。取值之一为 `left`, `right`, `wheel`, `back`，或 `forward`.
        ...
        - `actions: optional ComputerActionList`
          ...
          为 `computer_use`。展平的批量操作。每个操作包含一个
          `type` 判别字段以及操作专属字段。
      ...
      - `FunctionCallOutput object { output, type, id, 5 more }`
        ...
        - `status: optional "in_progress" or "completed" or "incomplete" or null`
          ...
          项的状态。取值之一为 `in_progress`, `completed`，或 `incomplete`。当条目经由 API 返回时填充。
        ...
        - `tools: array of object { name, parameters, strict, 6 more }  or object { type, vector_store_ids, filters, 2 more }  or object { type }  or 13 more`
            ...
            - `filters: optional ComparisonFilter or CompoundFilter or null`
              ...
              - `CompoundFilter object { filters, type }`
                ...
                - `filters: array of ComparisonFilter or unknown`
                  ...
                  要组合的筛选器数组。各项可以为 `ComparisonFilter` 或 `CompoundFilter`.
          ...
          - `WebSearch object { type, external_web_access, filters, 2 more }`
            ...
            - `type: "web_search" or "web_search_2025_08_26"`
              ...
              网页搜索工具的类型。其值为以下之一 `web_search` 或 `web_search_2025_08_26`.
          ...
          - `Mcp object { server_label, type, allowed_callers, 9 more }`
            ...
            - `authorization: optional string`
              ...
              服务器 URL 或服务连接器一起使用。你的应用必须处理 OAuth 授权流程，并在此处提供令牌。
              必须处理 OAuth 授权流程，并在此处提供令牌。

            - `connector_id: optional "connector_dropbox" or "connector_gmail" or "connector_googlecalendar" or 5 more`

              服务连接器的标识符，例如 ChatGPT 中提供的连接器。必须提供
              `server_url`, `connector_id`，或 `tunnel_id` 以下之一。详细了解
          ...
          - `ImageGeneration object { type, action, background, 9 more }`
            ...
            - `background: optional "transparent" or "opaque" or "auto"`
              ...
              模型，此支持处于 `gpt-image-2` 和 `gpt-image-2-2026-04-21`，该支持处于预览阶段。当使用
            ...
            - `input_fidelity: optional "high" or "low" or null`
              ...
              控制模型在匹配输入图像的风格和特征（尤其是面部特征）时将投入多少精力。此参数仅支持 `gpt-image-1` 和 `gpt-image-1.5` 及更高版本的模型，不支持 `gpt-image-1-mini`。支持取值： `high` 和 `low`。默认为 `low`.
            ...
            - `partial_images: optional number`
              ...
              在流式模式下要生成的中间图像数量，范围从 0（默认值）到 3。
            ...
            - `tools: array of object { name, type, allowed_callers, 6 more }  or object { name, type, allowed_callers, 4 more }`
              ...
              - `Function object { name, type, allowed_callers, 6 more }`
                ...
                - `output_schema: optional map[unknown] or null`
                  ...
                  用于描述此函数工具的字符串输出中 JSON 值编码的 JSON Schema。此 Schema 不用于描述 content 数组输出。
          ...
          - `WebSearchPreview object { type, search_content_types, search_context_size, user_location }`
            ...
            此工具会在网页中搜索与响应相关的结果。了解更多关于 [网页搜索 工具](/api/docs/guides/tools-web-search).

            - `type: "web_search_preview" or "web_search_preview_2025_03_11"`

              网页搜索工具的类型。其值为以下之一 `web_search_preview` 或 `web_search_preview_2025_03_11`.
      ...
      - `ConfigurationUpdate object { type, id, reasoning }`
        ...
        - `reasoning: optional object { effort }`
          ...
          - `effort: optional ReasoningEffort or null`
            ...
            在另一次配置更新将其替换之前，后续响应所使用的推理 effort
            配置更新将其替换。
      ...
      - `Reasoning object { id, summary, type, 3 more }`
        ...
        的描述。如果你手动管理上下文，请务必将这些项包含在你发送给 `input` to the Responses API
        的后续对话轮次中，前提是你正在
        ...
        - `encrypted_content: optional string or null`
          ...
          推理项的加密内容。默认情况下，由
          以及 WebSocket `POST /v1/responses` 返回的推理项填充该字段
          `response.create` 请求时填充。

          流式传输时，请使用已完成的推理项及其
          `encrypted_content` 来自 `response.output_item.done` 事件，在
          后续请求中可能不完整。 `encrypted_content` 在
          `response.output_item.added` 中尤其如此。当
          使用 `store` 时 `false` 或使用 Zero Data Retention 时，这一点尤为重要。
      ...
      - `LocalShellCall object { id, action, call_id, 2 more }`
        ...
        - `action: object { command, env, type, 3 more }`
          ...
          - `type: "exec"`
            ...
            本地 shell 操作（action）的类型，固定为 `exec`.
        ...
        - `type: "local_shell_call"`
          ...
          本地 shell 调用的类型，固定为 `local_shell_call`.
      ...
      - `LocalShellCallOutput object { id, output, type, status }`
        ...
        - `type: "local_shell_call_output"`
          ...
          本地 shell 工具调用输出的类型，固定为 `local_shell_call_output`.

          - `"local_shell_call_output"`

        - `status: optional "in_progress" or "completed" or "incomplete" or null`

          项的状态。取值之一为 `in_progress`, `completed`，或 `incomplete`.
      ...
      - `ShellCall object { action, call_id, type, 4 more }`
        ...
        - `id: optional string or null`
          ...
          shell 工具调用的唯一 ID。通过 API 返回该条目时填充此字段。
  ...
  - `output: array of ResponseOutputItem`
    ...
    - 数组中项的长度和顺序取决于 `output` 模型的响应。
      与其访问数组中的第一项并。
    - 假设它是一个包含模型生成内容的 `output` message
      对象，你或许可以考虑使用 `assistant` 属性，该属性会在
      存在时返回模型生成的内容。 `output_text` property where
      在 SDK 中受支持。
    ...
    - `ShellCallOutput object { id, call_id, max_output_length, 5 more }`
      ...
      - `id: string`
        ...
        shell 调用输出的唯一 ID。通过 API 返回该条目时填充。
  ...
  - `tools: array of object { name, parameters, strict, 6 more }  or object { type, vector_store_ids, filters, 2 more }  or object { type }  or 13 more`
    ...
    - **内置工具**：由 OpenAI 提供、可扩展模型能力的工具，例如
      模型的网页搜索 [网页搜索](/api/docs/guides/tools-web-search)
      或 [文件搜索](/api/docs/guides/tools-file-search)。详细了解
      [内置工具的信息](/api/docs/guides/tools).
    - **MCP 工具**：通过自定义 MCP 服务器或 Google Drive 和 SharePoint 等预定义连接器与第三方系统集成。详细了解
  ...
  - `max_output_tokens: optional number or null`
    ...
    响应可生成词元数的上限，包括可见输出词元和 [推理词元](/api/docs/guides/reasoning).

  - `max_tool_calls: optional number or null`

    单个响应中可处理的内置工具调用总次数上限。该上限适用于所有内置工具调用，而非按单个工具计算。模型后续任何进一步的工具调用尝试都将被忽略。
  ...
  - `safety_identifier: optional string or null`
    ...
    一个稳定的标识符，用于帮助检测可能违反OpenAI使用策略的应用程序用户。
    该 ID 应为用于唯一标识每个用户的字符串，最大长度为 64 个字符。建议对用户名或电子邮件地址进行哈希处理，以避免向我们发送任何身份识别信息。 [了解更多](/api/docs/guides/safety-best-practices#implement-safety-identifiers).

  - `service_tier: optional ServiceTier or null`

    指定用于处理该请求的处理类型。

    - 如果设置为 'auto'，则请求将使用 Project 设置中配置的服务层级进行处理。除非另有配置，否则 Project 将使用 'default'。
    ...
    当设置了 `service_tier` 参数时，响应正文将根据实际用于处理该请求的处理模式包含相应的 `service_tier` 值。此响应值可能与参数中设置的值不同。
  ...
  - `text: optional ResponseTextConfig`
    ...
    - `format: optional ResponseFormatTextConfig`
      ...
      从而确保模型输出与提供的 JSON schema 一致。更多信息请参阅
      ...
      **不建议用于 gpt-4o 及更新模型：**
      ...
      可确保模型生成的消息为合法 JSON。对于支持结构化输出的模型，建议优先使用 `json_schema`
      。
      ...
      - `ResponseFormatTextJSONSchemaConfig object { name, schema, type, 2 more }`
        ...
        - `strict: optional boolean or null`
          ...
          在生成输出时是否启用严格的 schema 遵从。
          若设置为 true，模型将始终遵循在
          字段中定义的精确 schema。仅在启用 `schema` 时支持 JSON Schema 的一个子集。更多详情请阅读
          `strict` 时 `true`。指南。 [结构化输出
          指南](/api/docs/guides/structured-outputs).

      - `ResponseFormatJSONObject object { type }`

        JSON 对象响应格式。一种较旧的生成 JSON 响应的方法。
        对于支持 `json_schema` 的模型，建议使用该方法。请注意，
