> 完整文档索引请参阅 [llms.txt](/llms.txt)。通过在页面 URL 后附加 `.md` 可获取文档页面的 Markdown 版本。
...
- `Response object { id, created_at, error, 33 more }`
  ...
  - `incomplete_details: object { reason }  or null`
    ...
    - `reason: optional "max_output_tokens" or "max_messages" or "content_filter" or "steered"`
      ...
      响应未完成的原因。 `steered` 表示
      响应在某个 WebSocket
      WebSocket `response.steer` 事件之后在安全输出边界处停止。然后服务端可以
      使用排队的输入自动创建一个后续响应。
  ...
  - `instructions: string or array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more or null`
    ...
    与 `previous_response_id`，一起使用时，之前响应中的指令将不会延续到下一个响应。这使得实现
    之前响应的指令不会延续到下一个响应。这使得实现
    在新响应中替换系统（或开发者）消息。
    ...
    - `InputItemList = array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more`
      ...
      - `EasyInputMessage object { content, role, phase, type }`
        ...
        使用 `developer` 或 `system` 角色给出的指令优先级
        高于使用 `user` 角色给出的指令。
        `assistant` 角色的消息假定由模型在先前的
        交互中生成。
          ...
          - `ResponseInputMessageContentList = array of ResponseInputContent`
            ...
            - `ResponseInputText object { text, type, prompt_cache_breakpoint }`
              ...
              - `prompt_cache_breakpoint: optional object { mode }`
                ...
                标记可复用提示前缀的精确结束位置。该断点会沿用请求的 TTL `prompt_cache_options.ttl`；边界不会对齐到 token 块。
            ...
            - `ResponseInputFile object { type, detail, file_data, 4 more }`
              ...
              - `detail: optional "auto" or "low" or "high"`
                ...
                发送给模型的文件的细节级别。使用 `auto` 让系统选择细节级别；对于 GPT-5.6 及更高版本的模型， `auto` 使用高质量渲染，这可能会增加输入 token 使用量。使用 `low` 进行更低成本的渲染，或 `high` 以更高质量渲染文件。默认为 `auto`.
        ...
        - `phase: optional "commentary" or "final_answer" or null`
          ...
          将消息标记为中间评论（ `assistant` message as intermediate commentary (`commentary`）或最终回答（`final_answer`).
        ...
        - `action: optional ComputerAction`
          ...
          - `Drag object { path, type, keys }`
            ...
            - `path: array of object { x, y }`
              ...
              表示拖动操作路径的坐标数组。坐标将以对象数组的形式显示，例如
      ...
      - `FunctionCallOutput object { output, type, id, 5 more }`
        ...
        - `status: optional "in_progress" or "completed" or "incomplete" or null`
          ...
          该项的状态。取值为 `in_progress`, `completed`，或 `incomplete`。当通过 API 返回 item 时填充。
      ...
      - `ToolSearchOutput object { tools, type, id, 3 more }`
        ...
        - `tools: array of object { name, parameters, strict, 6 more }  or object { type, vector_store_ids, filters, 2 more }  or object { type }  or 13 more`
          ...
          - `Function object { name, parameters, strict, 6 more }`
            ...
            定义你自己代码中可供模型选择调用的函数。详细了解 [函数调用](https://platform.openai.com/docs/guides/function-calling).
            ...
            - `output_schema: optional map[unknown] or null`
              ...
              描述该函数以字符串形式输出的 JSON 值所对应结构的 JSON schema 对象。
          ...
          - `FileSearch object { type, vector_store_ids, filters, 2 more }`
            ...
            - `ranking_options: optional object { hybrid_search, ranker, score_threshold }`
              ...
              - `score_threshold: optional number`
                ...
                文件搜索 的分数阈值，介于 0 到 1 之间的数值。数值越接近 1，将尝试仅返回最相关的结果，但返回的结果数量可能更少。
          ...
          - `WebSearch object { type, external_web_access, filters, 2 more }`
            ...
            - `search_context_size: optional "low" or "medium" or "high"`
              ...
              用于搜索的上下文窗口空间使用量的高级指导。取值之一 `low`, `medium`，或 `high`. `medium` 为默认值。
          ...
          - `Mcp object { server_label, type, allowed_callers, 9 more }`
            ...
            - `tunnel_id: optional string`
              ...
              用于替代直接服务端 URL 的安全 MCP 隧道 ID。必须提供
              `server_url`, `connector_id`，或 `tunnel_id` 中的至少一个。
          ...
          - `ImageGeneration object { type, action, background, 9 more }`
            ...
            - `input_fidelity: optional "high" or "low" or null`
              ...
              控制模型在匹配输入图像风格和特征（尤其是面部特征）时所投入的精力。此参数仅支持 `gpt-image-1` 和 `gpt-image-1.5` 及更高版本的模型，不支持 `gpt-image-1-mini`。支持 `high` 和 `low`。默认为 `low`.
            ...
            - `size: optional string or "1024x1024" or "1024x1536" or "1536x1024" or "auto"`
              ...
              生成图像的尺寸。对于 `gpt-image-2` 和 `gpt-image-2-2026-04-21`，支持任意分辨率，例如以 `WIDTHxHEIGHT` 字符串形式表示，例如 `1536x864`。宽度和高度都必须能被 16 整除，且请求的宽高比必须在 1:3 到 3:1 之间。高于 `2560x1440` 的分辨率为实验性，最大支持的分辨率为 `3840x2160`。请求的尺寸还必须满足模型当前的像素和边长限制。标准尺寸 `1024x1024`, `1536x1024`，和 `1024x1536` 由 GPT 图像模型支持； `auto` 支持允许自动调整大小的模型。对于 `dall-e-2`，请使用以下之一 `256x256`, `512x512`，或 `1024x1024`。对于 `dall-e-3`，请使用以下之一 `1024x1024`, `1792x1024`，或 `1024x1792`.
            ...
            - `tools: array of object { name, type, allowed_callers, 6 more }  or object { name, type, allowed_callers, 4 more }`
              ...
              - `Function object { name, type, allowed_callers, 6 more }`
                ...
                - `output_schema: optional map[unknown] or null`
                  ...
                  用于描述该函数工具字符串输出中编码的 JSON 值的 JSON Schema。此描述不适用于 content 数组输出。
          ...
          - `WebSearchPreview object { type, search_content_types, search_context_size, user_location }`
            ...
            此工具会搜索网页以获取相关结果，用于回复中。详细了解 [网页搜索 工具](https://platform.openai.com/docs/guides/tools-web-search).
      ...
      - `Reasoning object { id, summary, type, 3 more }`
        ...
        推理模型在生成响应时使用的思维链的描述
        。如果你是手动管理对话，请确保将这些项包含到 `input` 对 Responses API 的请求中，
        以便在对话的后续轮次中传递它们。
        ...
        - `encrypted_content: optional string or null`
          ...
          用于后续请求。该 `encrypted_content` 在
          `response.output_item.added` 时可能不完整。这在
          时尤其重要 `store` 是 `false` 或在使用 Zero Data Retention 时。
      ...
      - `Compaction object { encrypted_content, type, id }`
        ...
        由以下来源生成的压缩条目 [`v1/responses/compact` API](/docs/api-reference/responses/compact).
      ...
      - `ShellCallOutput object { call_id, output, type, 4 more }`
        ...
        - `output: array of ResponseFunctionShellCallOutputContent`
          ...
          捕获的 stdout 和 stderr 输出块，以及其关联的结果。
      ...
      - `ApplyPatchCallOutput object { call_id, status, type, 3 more }`
        ...
        - `output: optional string or null`
          ...
          apply patch 工具的可选的、可供人类阅读的日志文本（例如补丁结果或错误）。
  ...
  - `output: array of ResponseOutputItem`
    ...
    - 数组中项的长度和顺序取决于 `output` 取决于模型的响应。
      取决于模型的响应。
    - 与其直接访问数组中的第一项并 `output` 数组中的第一项并
      假定它是一条包含模型生成内容的 `assistant` 包含模型生成内容的消息，可以考虑使用
      属性（如果SDK支持的话）。 `output_text` 属性（如果支持
      支持的话）。
    ...
    - `ShellCallOutput object { id, call_id, max_output_length, 5 more }`
      ...
      - `id: string`
        ...
        shell 调用输出的唯一 ID。当该条目通过 API 返回时会填充此字段。
      ...
      - `output: array of object { outcome, stderr, stdout, created_by }`
        ...
        - `outcome: object { type }  or object { exit_code, type }`
          ...
          表示 shell 调用输出块的结果，类型为退出结果（带有退出码）或超时结果。
  ...
  - `tool_choice: ToolChoiceOptions or ToolChoiceAllowed or ToolChoiceTypes or 6 more`
    ...
    - `ToolChoiceCustom object { name, type }`
      ...
      - `type: "custom"`
        ...
        对于 custom 工具调用，type 始终为 `custom`.
  ...
  - `tools: array of object { name, parameters, strict, 6 more }  or object { type, vector_store_ids, filters, 2 more }  or object { type }  or 13 more`
    ...
    通过设置以下参数来指定要使用的工具 `tool_choice` 。

    我们支持以下类别的工具：

    - **内置工具**：由 OpenAI 提供，用于扩展模型功能的工具，例如
  ...
  - `top_p: number or null`
    ...
    一种温度采样的替代方法，称为核采样，
    在该方法中模型只考虑 top_p 概率质量范围内的 token 结果。
    因此 0.1 表示仅考虑构成前 10% 概率质量的 token。
    会被纳入考虑。

    我们通常建议修改此项或 `temperature` 但不要两者同时修改。
  ...
  - `max_output_tokens: optional number or null`
    ...
    响应可生成的 token 数量上限，包括可见输出 token 和 [reasoning tokens](/docs/guides/reasoning).

  - `max_tool_calls: optional number or null`

    可在一次响应中处理的内置工具的最大调用总次数。该最大次数适用于所有内置工具调用，而非针对单个工具。模型后续任何调用工具的尝试都将被忽略。
    ...
    - `input: object { categories, category_applied_input_types, category_scores, 3 more }  or object { code, message, type }`
      ...
      - `ModerationResult object { categories, category_applied_input_types, category_scores, 3 more }`
        ...
        - `categories: map[boolean]`
          ...
          一个将审核分类映射到布尔值的字典，如果输入在该分类下被标记，则对应值为 True。
  ...
  - `output_text: optional string or null`
    ...
    仅限 SDK 的便捷属性，包含所有项中汇总的文本输出
    来自所有 `output_text` 项的 `output` 数组（如果有的话）。
  ...
  - `previous_response_id: optional string or null`
    ...
    前一个模型响应的唯一 ID。使用它来
    创建多轮对话。了解更多关于
    [对话状态](/docs/guides/conversation-state)。不能与以下字段同时使用： `conversation`.
  ...
  - `prompt: optional ResponsePrompt or null`
    ...
    - `variables: optional map[string or ResponseInputText or ResponseInputImage or ResponseInputFile] or null`
      ...
      用于在你的提示模板变量里替换的可选值映射。替换值可以是字符串，
      也可以是其他响应输入类型，
      例如图像或文件。
  ...
  - `prompt_cache_retention: optional "in_memory" or "24h" or null`
    ...
    对于同时支持 `in_memory` 和 `24h`，默认值取决于你组织的数据保留策略：
  ...
  - `reasoning: optional Reasoning or null`
    ...
    - `effort: optional ReasoningEffort or null`
      ...
      降低推理投入可以让响应更快，并减少响应中用于推理的 token 数量。并非所有推理模型都支持每个
      值。有关模型特定的支持情况，请参阅
      推理指南
  ...
  - `safety_identifier: optional string or null`
    ...
    这些 ID 应为字符串，唯一标识每个用户，最大长度为 64 个字符。建议对用户名或电子邮件地址进行哈希处理，以避免向我们发送任何可识别信息。 [了解更多](/docs/guides/safety-best-practices#safety-identifiers).

  - `service_tier: optional ServiceTier or null`

    指定用于处理该请求的处理类型。

    - 如果设置为 'auto'，则该请求将使用在项目设置中配置的服务层级进行处理。除非另行配置，否则项目将使用 'default'。
    - 如果设置为 'default'，则该请求将按所选模型的标准定价和性能进行处理。
    - 如果设置为 '[flex](/docs/guides/flex-processing)'，则该请求将使用 Flex Processing 服务层级进行处理。
    - 若要在 [Fast 模式](/api/docs/guides/fast-mode) 中启用此功能，请在请求中包含该 `service_tier=fast` 或 `service_tier=priority` 参数，适用于 Responses 或 Chat Completions。响应中将显示 `service_tier=priority` ，无论你是否在请求中指定 `service_tier=fast` 或 `priority` 。
    - 如果设置为 'ultrafast'，则该请求将使用受访问控制的 Ultrafast Processing 服务层级进行处理。该层级目前可用于 `gpt-5.6-sol`；通过该层级返回的响应将显示 `service_tier=ultrafast`.
    - 当未设置时，默认行为为 'auto'。

    当 `service_tier` 参数被设置时，响应体将根据实际用于处理该请求的处理模式，包含相应的 `service_tier` 值。该响应值可能与参数中设置的值不同。
  ...
  - `text: optional ResponseTextConfig`
    ...
    - `format: optional ResponseFormatTextConfig`
      ...
      从而确保模型输出与你提供的 JSON schema 一致。更多信息请参阅
      ...
      **不建议用于 gpt-4o 及更新模型：**
      ...
      - `ResponseFormatTextJSONSchemaConfig object { name, schema, type, 2 more }`
        ...
        - `strict: optional boolean or null`
          ...
          如果设置为 true，模型将始终遵循在
          字段中定义的精确 schema。仅支持 JSON Schema 的一个子集，当 `schema` 字段中定义的精确 schema。仅支持 JSON Schema 的一个子集，当
      ...
      - `ResponseFormatJSONObject object { type }`
        ...
        使用 `json_schema` 对于支持它的模型是推荐方式。请注意，
        除非系统或用户消息明确要求，否则模型不会生成 JSON
        执行此操作。
    ...
    - `verbosity: optional "low" or "medium" or "high" or null`
      ...
      限制模型响应的详细程度。较低的值会产生更简洁的响应，而较高的值会产生更详细的响应。
  ...
  - `top_logprobs: optional number or null`
    ...
    介于 0 和 20 之间的整数，指定每个 token 位置要返回的最可能 token 的最大数量，每个 token 都有对应的对数概率。
    每个 token 都具有关联的对数
  ...
  - `truncation: optional "auto" or "disabled" or null`
    ...
    - `auto`：如果此 Response 的输入超过
      模型的上下文窗口大小，模型将通过删除对话开头的项目来截断响应，
      以适应上下文窗口。
  ...
  - `user: optional string`
    ...
    用于标识终端用户的稳定标识符。
    用于通过更好地对相似请求进行分桶来提升缓存命中率，并帮助 OpenAI 检测和防范滥用行为。 [了解更多](/docs/guides/safety-best-practices#safety-identifiers).
