...
- `model: "gpt-6-astra" or "gpt-5.6-sol" or "gpt-5.6-terra" or 100 more or string or null`
  ...
  用于生成响应的模型 ID，例如 `gpt-6-astra`。OpenAI 提供种类丰富的模型，在能力、性能特征和价格点上各有不同。请参阅 [模型指南](/docs/models) 以浏览和比较可用的模型。
  ...
  - `array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more`
    ...
    - `EasyInputMessage object { content, role, phase, type }`
        ...
        - `ResponseInputMessageContentList = array of ResponseInputContent`
          ...
          - `ResponseInputFile object { type, detail, file_data, 4 more }`
            ...
            - `detail: optional "auto" or "low" or "high"`
              ...
              发送到模型的文件的细节级别。使用 `auto` 可让系统自动选择细节级别；对于 GPT-5.6 及更高版本的模型， `auto` 使用高质量渲染，可能会增加输入 token 用量。使用 `low` 以获得更低成本的渲染，或 `high` 以更高质量渲染文件。默认为 `auto`.
      ...
      - `phase: optional "commentary" or "final_answer" or null`
        ...
        将该 `assistant` 消息标记为中间注释（`commentary`）或最终答案（`final_answer`).
        对于类似 `gpt-5.3-codex` 及更高版本的模型，在发送后续请求时，请在所有助手消息上保留并重新发送
        阶段——删除它可能会降低性能。用户消息不使用。
    ...
    - `FileSearchCall object { id, queries, status, 2 more }`
      ...
      - `results: optional array of object { attributes, file_id, filename, 2 more }  or null`
        ...
        - `attributes: optional map[string or number or boolean] or null`
          ...
          格式存储对象的附加信息，并通过 API 或仪表板查询对象。键为长度不
          超过 64 个字符的字符串。值为长度不超过 512 个字符的字符串、布尔值或数字。
          超过 64 个字符的字符串。值为长度不超过 512 个字符的字符串、布尔值或数字。
          值或数字。
    ...
    - `ComputerCall object { id, call_id, pending_safety_checks, 4 more }`
      ...
      - `action: optional ComputerAction`
        ...
        - `Click object { button, type, x, 2 more }`
          ...
          - `button: "left" or "right" or "wheel" or 2 more`
            ...
            指明点击时按下的鼠标按键。取值之一： `left`, `right`, `wheel`, `back`，或 `forward`.
      ...
      - `actions: optional ComputerActionList`
        ...
        的扁平化批量操作。每个操作包含一个 `computer_use`。鉴别字段以及操作特定的字段。
        `type` 鉴别字段以及操作特定的字段。
      ...
      - `tools: array of object { name, parameters, strict, 6 more }  or object { type, vector_store_ids, filters, 2 more }  or object { type }  or 13 more`
        ...
        - `Function object { name, parameters, strict, 6 more }`
          ...
          - `output_schema: optional map[unknown] or null`
            ...
            一个 JSON schema 对象，用于描述该函数字符串输出中所编码的 JSON 值。
        ...
        - `FileSearch object { type, vector_store_ids, filters, 2 more }`
          ...
          - `ranking_options: optional object { hybrid_search, ranker, score_threshold }`
            ...
            - `hybrid_search: optional object { embedding_weight, text_weight }`
              ...
              用于控制在启用混合搜索时，倒数排名融合算法中语义嵌入匹配相对于稀疏关键词匹配平衡程度的权重。
        ...
        - `WebSearch object { type, external_web_access, filters, 2 more }`
          ...
          - `type: "web_search" or "web_search_2025_08_26"`
            ...
            网页搜索工具的类型。下列之一： `web_search` 或 `web_search_2025_08_26`.
          ...
          - `search_context_size: optional "low" or "medium" or "high"`
            ...
            用于搜索的上下文窗口空间的高级指引，取值为以下之一 `low`, `medium`，或 `high`. `medium` 为默认值。
        ...
        - `Mcp object { server_label, type, allowed_callers, 9 more }`
          ...
          - `connector_id: optional "connector_dropbox" or "connector_gmail" or "connector_googlecalendar" or 5 more`
            ...
            关于服务连接器 [here](/docs/guides/tools-remote-mcp#connectors).
              ...
              - `network_policy: optional ContainerNetworkPolicyDisabled or ContainerNetworkPolicyAllowlist`
                ...
                - `ContainerNetworkPolicyAllowlist object { allowed_domains, type, domain_secrets }`
                  ...
                  - `type: "allowlist"`
                    ...
                    仅允许访问指定域的出站网络访问。Always `allowlist`.
        ...
        - `ImageGeneration object { type, action, background, 9 more }`
          ...
          - `background: optional "transparent" or "opaque" or "auto"`
            ...
            `gpt-image-2-2026-04-21`，该支持目前为预览版。当使用
          ...
          - `input_fidelity: optional "high" or "low" or null`
            ...
            控制模型在匹配输入图像的风格和特征（尤其是面部特征）时所投入的精力。该参数仅支持 `gpt-image-1` 和 `gpt-image-1.5` 及更高版本的模型，不支持 `gpt-image-1-mini`。支持的取值包括 `high` 和 `low`。默认为 `low`.
          ...
          - `size: optional string or "1024x1024" or "1024x1536" or "1536x1024" or "auto"`
            ...
            生成图像的尺寸。对于 `gpt-image-2` 和 `gpt-image-2-2026-04-21`，支持以 `WIDTHxHEIGHT` 字符串形式指定任意分辨率，例如 `1536x864`。宽度和高度都必须能被 16 整除，且请求的宽高比必须在 1:3 到 3:1 之间。高于 `2560x1440` 的分辨率属于实验性功能，且最大支持的分辨率为 `3840x2160`。请求的尺寸还必须满足模型当前的像素和边长限制。GPT 图像模型支持的标准尺寸为 `1024x1024`, `1536x1024`，以及 `1024x1536` 由 GPT 图像模型支持； `auto` 由支持自动尺寸的模型支持。对于 `dall-e-2`，请使用以下值之一 `256x256`, `512x512`，或 `1024x1024`。对于 `dall-e-3`，请使用以下值之一 `1024x1024`, `1792x1024`，或 `1024x1792`.
          ...
          - `tools: array of object { name, type, allowed_callers, 6 more }  or object { name, type, allowed_callers, 4 more }`
            ...
            - `Function object { name, type, allowed_callers, 6 more }`
              ...
              - `output_schema: optional map[unknown] or null`
                ...
                用于描述此函数工具中以字符串形式编码的 JSON 值的 JSON Schema。此描述不适用于 content 数组形式的输出。
        ...
        - `WebSearchPreview object { type, search_content_types, search_context_size, user_location }`
          ...
          该工具会在网页中搜索相关结果以用于回复中。详细了解 [网页搜索工具](https://platform.openai.com/docs/guides/tools-web-search).

          - `type: "web_search_preview" or "web_search_preview_2025_03_11"`

            网页搜索工具的类型。下列之一： `web_search_preview` 或 `web_search_preview_2025_03_11`.
    ...
    - `Reasoning object { id, summary, type, 3 more }`
      ...
      推理模型在生成过程中使用的思维链描述
      响应中。请确保在将它们发回 `input` 到 Responses API
      时，包含这些项，以便在对话的后续回合中
      ...
      - `encrypted_content: optional string or null`
        ...
        推理项的加密内容。默认情况下，这会填充由
        和 WebSocket `POST /v1/responses` 请求返回的推理项内容。
        `response.create` 在流式传输时，

        请使用已完成的推理项及其来自
        `encrypted_content` 事件中的 `response.output_item.done` 字段，在
        后续请求中传递。该 `encrypted_content` 字段在
        `response.output_item.added` 可能不完整。这一点在以下情况下尤其
        重要： `store` 使用 `false` 或使用 Zero Data Retention 时。
    ...
    - `ApplyPatchCallOutput object { call_id, status, type, 3 more }`
      ...
      - `output: optional string or null`
        ...
        apply patch 工具可选的可读日志文本（例如补丁结果或错误）。
...
- `instructions: optional string or null`
  ...
  与以下参数一起使用时 `previous_response_id`，上一条响应中的指令不会延续到下一条响应。这便于在新响应中替换系统（或开发者）消息。

- `previous_response_id: optional string or null`

  上一条发送给模型的响应的唯一 ID。使用它可以创建多轮对话。详细了解 [对话状态](/docs/guides/conversation-state)。无法与 `conversation`.
...
- `prompt_cache_options: optional object { mode, ttl }  or null`
  ...
  提示缓存的选项。支持 `gpt-5.6` 及更高版本模型。默认情况下，OpenAI 会自动选择一个隐式缓存断点。你可以使用 `prompt_cache_breakpoint`。在内容块上添加显式断点。每个请求最多可以写入四个断点。在缓存匹配时，OpenAI 会考虑对话中最多最新的 80 个断点，且没有内容块回溯限制。将 `mode` 设为 `explicit` 以禁用隐式断点。 `ttl` 默认为 `30m`，目前这是唯一受支持的值。详见 [提示缓存指南](/docs/guides/prompt-caching) 中的最新说明。
  ...
  - `ttl: optional "30m"`
    ...
    请求写入的每个隐式和显式缓存断点所应用的最小生命周期。默认为 `30m`，目前这是唯一受支持的值。后端可能保留缓存条目的时间更长。
...
- `service_tier: optional "auto" or "default" or "fast" or 2 more or null`
  ...
  指定用于处理该请求的处理类型。   - 若设置为 'auto'，则请求将按照 Project 设置中配置的服务层级进行处理。除非另行配置，否则 Project 将使用 'default'。   - 若设置为 'default'，则请求将按照所选模型的标准定价和性能进行处理。   - 若设置为 '[flex](/docs/guides/flex-processing)'，则请求将使用 Flex Processing 服务层级进行处理。   - 若要在 [Fast mode](/api/docs/guides/fast-mode) ，请在请求级别传入 `service_tier=fast` 或 `service_tier=priority` 参数（适用于 Responses 或 Chat Completions）。对于具有专用 Fast 层级的模型，两个值都会解析为 `service_tier=fast`；对于其他模型，两个值都会解析为 `service_tier=priority`。   - 未设置时，默认行为为 'auto'。
  当 `service_tier` 参数被设置时，响应体中将包含 `service_tier` 值，该值基于实际用于处理该请求的处理模式。此响应值可能与参数中设置的值不同。
  ...
  - `output: array of Message or object { id, call_id, code, 2 more }  or object { id, call_id, result, 2 more }  or 25 more`
    ...
    - `Message object { id, content, role, 3 more }`
      ...
      - `content: array of ResponseInputText or ResponseOutputText or TextContent or 6 more`
        ...
        - `ComputerScreenshotContent object { detail, file_id, image_url, 2 more }`
          ...
          - `detail: ImageDetail`
            ...
            要发送给模型的截图图像的细节级别。取值为 `high`, `low`, `auto`，或 `original`。默认为 `auto`.
      ...
      - `status: "in_progress" or "completed" or "incomplete"`
        ...
        条目的状态，取以下值之一 `in_progress`, `completed`，或 `incomplete`. 当通过 API 返回条目时填充。
      ...
      - `phase: optional "commentary" or "final_answer" or null`
        ...
        将该 `assistant` 消息标记为中间注释（`commentary`）或最终答案（`final_answer`）。对于类似 `gpt-5.3-codex` 及更高版本的模型，在发送后续请求时，请在所有助手消息上保留并重新发送 phase——省略它可能会降低性能。不用于用户消息。
    ...
    - `ShellCallOutput object { id, call_id, max_output_length, 5 more }`
      ...
      - `output: array of object { outcome, stderr, stdout, created_by }`
        ...
        - `outcome: object { type }  or object { exit_code, type }`
          ...
          表示 shell 调用输出块对应的退出结果（包含退出码）或超时结果。
