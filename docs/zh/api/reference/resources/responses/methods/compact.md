...
了解何时以及如何在 [对话状态指南](/api/docs/guides/conversation-state#managing-the-context-window)。中压缩长时间运行的对话。有关兼容 ZDR 的压缩详细信息，请参阅 [压缩（高级）](/api/docs/guides/conversation-state#compaction-advanced).

### Body Parameters

- `model: "gpt-6-astra" or "gpt-5.6-sol" or "gpt-5.6-terra" or 100 more or string or null`

  用于生成响应的模型 ID，例如 `gpt-6-astra`。OpenAI 提供各种具有不同能力、性能特征和价格水平的模型。请参阅 [模型指南](/api/docs/models) 以浏览和比较可用的模型。
...
- `input: optional string or array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more or null`
  ...
  - `array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more`
    ...
    - `EasyInputMessage object { content, role, phase, type }`
      ...
      提供给模型的消息输入，其角色表明指令的
      优先级。使用 `developer` 或 `system` 角色给出的指令优先
      于使用 `user` 角色给出的指令。带有
      `assistant` 角色的消息被认为是在之前的
      交互中由模型生成的。
        ...
        - `ResponseInputMessageContentList = array of ResponseInputContent`
          ...
          - `ResponseInputText object { text, type, prompt_cache_breakpoint }`
            ...
            - `prompt_cache_breakpoint: optional object { mode }`
              ...
              标记可复用提示前缀的确切结束位置。该断点继承请求的 `prompt_cache_options.ttl`；边界不会向下取整到 token 块。
          ...
          - `ResponseInputImage object { detail, type, file_id, 2 more }`
            ...
            - `detail: ImageDetail`
              ...
              发送给模型的图像细节级别。取值之一为 `high`, `low`, `auto`，或 `original`。默认为 `auto`.
          ...
          - `ResponseInputFile object { type, detail, file_data, 4 more }`
            ...
            - `detail: optional "auto" or "low" or "high"`
              ...
              发送给模型的文件细节级别。使用 `auto` 让系统选择细节级别；对于 GPT-5.6 及更高版本的模型， `auto` 使用高质量渲染，这可能会增加输入 token 使用量。使用 `low` 进行低成本渲染，或 `high` 以更高质量渲染文件。默认为 `auto`.
      ...
      - `phase: optional "commentary" or "final_answer" or null`
        ...
        对于类似 `gpt-5.3-codex` 及更高版本的模型，发送后续请求时，请在所有助手消息上保留并重新发送
        阶段；删除它可能会降低性能。不适用于用户消息。
      ...
      - `action: optional ComputerAction`
        ...
        - `Click object { button, type, x, 2 more }`
          ...
          - `button: "left" or "right" or "wheel" or 2 more`
            ...
            指示点击时按下的鼠标按键。可选值之一： `left`, `right`, `wheel`, `back`，或 `forward`.
    ...
    - `FunctionCallOutput object { output, type, id, 5 more }`
      ...
      - `status: optional "in_progress" or "completed" or "incomplete" or null`
        ...
        条目的状态。可选值之一： `in_progress`, `completed`，或 `incomplete`。当通过 API 返回条目时填充。
    ...
    - `ToolSearchOutput object { tools, type, id, 3 more }`
      ...
      - `tools: array of object { name, parameters, strict, 6 more }  or object { type, vector_store_ids, filters, 2 more }  or object { type }  or 13 more`
        ...
        - `Function object { name, parameters, strict, 6 more }`
          ...
          在你自己代码中定义一个可供模型选择的函数。详细了解 [函数调用](/api/docs/guides/function-calling).
        ...
        - `FileSearch object { type, vector_store_ids, filters, 2 more }`
          ...
          用于从已上传文件中搜索相关内容的一个工具。详细了解 [文件搜索 工具](/api/docs/guides/tools-file-search).
          ...
          - `max_num_results: optional number`
            ...
            要返回的最大结果数。此数字应介于 1 到 50 之间（含 1 和 50）。
          ...
          - `ranking_options: optional object { hybrid_search, ranker, score_threshold }`
            ...
            - `hybrid_search: optional object { embedding_weight, text_weight }`
              ...
              启用混合搜索时，用于控制倒数排名融合如何平衡语义嵌入匹配与稀疏关键词匹配的权重。
            ...
            - `score_threshold: optional number`
              ...
              文件搜索的分数阈值，介于 0 和 1 之间。数值越接近 1，越会尝试仅返回最相关的结果，但返回的结果可能更少。
        ...
        - `WebSearch object { type, external_web_access, filters, 2 more }`
          ...
          - `external_web_access: optional boolean`
            ...
            允许网页搜索实时访问互联网。省略时默认为 true。设为 false 时，网页搜索工具将以离线/仅缓存模式运行，不会获取新的外部内容。
          ...
          - `search_context_size: optional "low" or "medium" or "high"`
            ...
            用于搜索的上下文窗口空间的高级用量指导。可选值为 `low`, `medium`，或 `high`. `medium` 为默认值。
        ...
        - `Mcp object { server_label, type, allowed_callers, 9 more }`
          ...
          - `authorization: optional string`
            ...
            需要配合自定义 MCP 服务器 URL 或服务连接器使用。你的应用
        ...
        - `ImageGeneration object { type, action, background, 9 more }`
          ...
          - `input_fidelity: optional "high" or "low" or null`
            ...
            控制模型在匹配输入图像的风格和特征（尤其是面部特征）时投入的努力程度。此参数仅在 `gpt-image-1` 和 `gpt-image-1.5` 及更高版本的模型中受支持，在 `gpt-image-1-mini`。中不受支持。支持 `high` 和 `low`。默认为 `low`.
          ...
          - `tools: array of object { name, type, allowed_callers, 6 more }  or object { name, type, allowed_callers, 4 more }`
            ...
            - `Function object { name, type, allowed_callers, 6 more }`
              ...
              - `output_schema: optional map[unknown] or null`
                ...
                描述此函数工具的字符串输出中所编码 JSON 值的 JSON Schema。这并不描述 content 数组的输出。
        ...
        - `WebSearchPreview object { type, search_content_types, search_context_size, user_location }`
          ...
          此工具会在网页上搜索相关结果以在响应中采用。了解更多关于 [网页搜索工具](/api/docs/guides/tools-web-search).
    ...
    - `Reasoning object { id, summary, type, 3 more }`
      ...
      。如果你正在进行手动 `input` 上下文管理，请务必将这些项包含在Responses API
      的后续会话轮次中，以便
      ...
      - `encrypted_content: optional string or null`
        ...
        推理项的加密内容。默认情况下填充此字段，该内容由
        返回的推理项填充 `POST /v1/responses` 和 WebSocket
        `response.create` 请求。
        ...
        尤其重要，当 `store` 为 `false` 或使用 Zero Data Retention 时。
    ...
    - `ImageGenerationCall object { id, result, status, 3 more }`
      ...
      - `size: optional string or "1024x1024" or "1024x1536" or "1536x1024" or null`
        ...
        以字符串表示的图像尺寸，例如 `WIDTHxHEIGHT` 字符串，示例： `1536x864`.
    ...
    - `ShellCall object { action, call_id, type, 4 more }`
      ...
      - `id: optional string or null`
        ...
        shell 工具调用的唯一标识符。在通过 API 返回此条目时填充。
...
- `instructions: optional string or null`
  ...
  当与 `previous_response_id`，一起使用时，之前响应中的指令不会延续到下一个响应。这让你可以在新响应中轻松替换系统（或开发者）消息。

- `previous_response_id: optional string or null`

  上一次模型响应的唯一 ID。使用它可以创建多轮对话。详细了解 [对话状态](/api/docs/guides/conversation-state)。不能与 `conversation`.
...
- `prompt_cache_options: optional object { mode, ttl }  or null`
  ...
  提示缓存的选项。受以下模型支持： `gpt-5.6` 及更高版本模型。默认情况下，OpenAI 会自动选择一个隐式缓存断点。你可以使用 `prompt_cache_breakpoint`。为内容块添加显式断点。每个请求最多可写入四个断点。对于缓存匹配，OpenAI 会在对话中考虑最多最近的 80 个断点，且没有内容块回溯限制。设置 `mode` 为 `explicit` 以禁用隐式断点。 `ttl` 默认为 `30m`，这是当前唯一支持的值。详见 [提示缓存指南](/api/docs/guides/prompt-caching) 了解当前详情。

  - `mode: optional "implicit" or "explicit"`

    控制是否由 OpenAI 自动创建隐式缓存断点。默认为 `implicit`。当设为 `implicit`，时，OpenAI 会创建一个隐式断点，并在请求中写入最多三个最新的显式断点。当设为 `explicit`，时，OpenAI 不会创建隐式断点，并在请求中写入最多四个最新的显式断点。如果没有显式断点，请求不会使用提示缓存。
  ...
  - `ttl: optional "30m"`
    ...
    请求中每个隐式和显式缓存断点所使用的最短生命周期。默认为 `30m`，这是当前唯一支持的值。后端可以将缓存条目保留更长时间。
...
- `service_tier: optional "auto" or "default" or "fast" or 2 more or null`
  ...
  指定用于处理该请求的处理类型。   - 若设为 'auto'，则该请求将按照项目设置中配置的服务层级进行处理。除非另行配置，否则该项目将使用 'default'。   - 若设为 'default'，则该请求将按所选模型的标准定价和性能进行处理。   - 若设为 '[flex](/api/docs/guides/flex-processing)'，则该请求将使用 Flex Processing 服务层级进行处理。   - 若要在请求级别启用 [快速模式](/api/docs/guides/fast-mode) ，请在 Responses 或 Chat Completions 中传入 `service_tier=fast` 或 `service_tier=priority` 参数。对于具有专属快速层级的模型，两个值都会解析为 `service_tier=fast`；对于其他模型，两个值都会解析为 `service_tier=priority`。   - 当未设置时，默认行为为 'auto'。
  当 `service_tier` 参数设置后，响应正文将根据实际用于处理该请求的处理模式， `service_tier` 返回值。此响应值可能与该参数中设置的值不同。
...
- `CompactedResponse object { id, created_at, object, 2 more }`
  ...
  - `output: array of Message or object { id, call_id, code, 2 more }  or object { id, call_id, result, 2 more }  or 25 more`
    ...
    - `Message object { id, content, role, 3 more }`
      ...
      - `content: array of ResponseInputText or ResponseOutputText or TextContent or 6 more`
        ...
        - `ComputerScreenshotContent object { detail, file_id, image_url, 2 more }`
          ...
          - `detail: ImageDetail`
            ...
            发送给模型的截图图像的细节等级。取值为 `high`, `low`, `auto`，或 `original`。默认为 `auto`.
      ...
      - `phase: optional "commentary" or "final_answer" or null`
        ...
        将 `assistant` 消息标记为中间评注（`commentary`）或最终答案（`final_answer`）。对于类似 `gpt-5.3-codex` 及更高版本的模型，在发送后续请求时，请在所有助手消息上保留并重新发送 phase——省略它可能会降低性能。不用于用户消息。
  ...
  - `usage: ResponseUsage`
    ...
    压缩通道的 token 统计，包括缓存、推理和总 token。
