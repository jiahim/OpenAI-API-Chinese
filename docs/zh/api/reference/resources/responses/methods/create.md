...
- `conversation: optional string or ResponseConversationParam or null`
  ...
  此响应所属的会话。该会话中的条目会前置拼接到 `input_items` 本次响应请求中。
  此响应的输入条目和输出条目会在该响应完成后自动添加到此会话。
...
- `include: optional array of ResponseIncludable or null`
  ...
  - `reasoning.encrypted_content`: 在推理条目输出中包含加密版本的推理令牌。这样可以在无状态地使用 Responses API（例如当 `store` 参数设置为 `false`，时，或当组织已加入零数据保留计划时）时，在多轮对话中使用推理条目。
...
- `input: optional string or array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more`
  ...
  - `InputItemList = array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more`
    ...
    - `EasyInputMessage object { content, role, phase, type }`
      ...
      传递给模型的消息输入，通过角色来表明指令的
      优先级关系。使用 `developer` 或 `system` 角色给出的指令优先
      于使用 `user` 角色给出的指令。带有
        ...
        - `ResponseInputMessageContentList = array of ResponseInputContent`
          ...
          - `ResponseInputFile object { type, detail, file_data, 4 more }`
            ...
            - `detail: optional "auto" or "low" or "high"`
              ...
              发送给模型的文件的细节级别。使用 `auto` 让系统选择细节级别；对于 GPT-5.6 及更高版本的模型， `auto` 使用高质量渲染，这可能会增加输入 token 用量。使用 `low` 用于以更低成本进行渲染，或 `high` 以更高质量渲染文件。默认为 `auto`.
      ...
      - `phase: optional "commentary" or "final_answer" or null`
        ...
        阶段，作用于所有助手消息——省略该字段可能导致性能下降。用户消息不使用此字段。
    ...
    - `FileSearchCall object { id, queries, status, 2 more }`
      ...
      - `type: "file_search_call"`
        ...
        该文件搜索工具调用的类型，始终为 `file_search_call`.
    ...
    - `ComputerCall object { id, call_id, pending_safety_checks, 4 more }`
      ...
      - `actions: optional ComputerActionList`
        ...
        的扁平化批量操作。每个操作都包含一个 `computer_use`。判别字段和特定于操作的字段。
        `type` 判别字段和特定于操作的字段。
    ...
    - `ComputerCallOutput object { call_id, output, type, 3 more }`
      ...
      - `status: optional "in_progress" or "completed" or "incomplete" or null`
        ...
        消息输入的状态。其值之一为 `in_progress`, `completed`，或 `incomplete`。当通过 API 返回输入项时填充。
    ...
    - `FunctionCallOutput object { output, type, id, 5 more }`
      ...
      - `id: optional string or null`
        ...
        函数工具调用输出的唯一 ID。通过 API 返回此条目时填充。
      ...
      - `status: optional "in_progress" or "completed" or "incomplete" or null`
        ...
        该项的状态。可选值为 `in_progress`, `completed`，或 `incomplete`。当通过 API 返回项时填充。
      ...
      - `tools: array of object { name, parameters, strict, 6 more }  or object { type, vector_store_ids, filters, 2 more }  or object { type }  or 13 more`
        ...
        - `FileSearch object { type, vector_store_ids, filters, 2 more }`
          ...
          - `filters: optional ComparisonFilter or CompoundFilter or null`
            ...
            - `ComparisonFilter object { key, type, value }`
              ...
              用于通过定义的比较运算将指定属性键与给定值进行比较的筛选条件。
        ...
        - `WebSearch object { type, external_web_access, filters, 2 more }`
          ...
          - `external_web_access: optional boolean`
            ...
            允许网页搜索使用实时互联网访问。省略时默认为 true。当设置为 false 时，网页搜索工具以离线/仅缓存模式运行，不会获取新的外部内容。
        ...
        - `Mcp object { server_label, type, allowed_callers, 9 more }`
          ...
          - `connector_id: optional "connector_dropbox" or "connector_gmail" or "connector_googlecalendar" or 5 more`
            ...
            服务连接器的标识符，例如 ChatGPT 中可用的连接器。必须提供以下之一。详细了解
            `server_url`, `connector_id`，或 `tunnel_id` 必须提供以下之一。了解更多
        ...
        - `CodeInterpreter object { container, type, allowed_callers }`
          ...
          - `container: string or object { type, file_ids, memory_limit, network_policy }`
            ...
            可供代码使用的已上传文件 ID，以及一个可选的内存限制
            设置的对象。 `memory_limit` 设置。
        ...
        - `ImageGeneration object { type, action, background, 9 more }`
          ...
          - `size: optional string or "1024x1024" or "1024x1536" or "1536x1024" or "auto"`
            ...
            生成图像的尺寸。对于 `gpt-image-2`, `gpt-image-2-2026-04-21`, `gpt-image-2.5-sunburst`, `gpt-image-2.5-sunburst-2026-09-08`, `gpt-image-2.5-flare`，和 `gpt-image-2.5-flare-2026-09-08`，支持任意分辨率，形式为 `WIDTHxHEIGHT` 字符串，例如 `1536x864`。宽度和高度都必须能被 16 整除，且所请求的宽高比必须介于 1:3 与 3:1 之间。高于 `2560x1440` 的分辨率为实验性功能，最大支持的分辨率为 `3840x2160`。所请求的尺寸还必须满足模型当前的像素和边长限制。标准尺寸 `1024x1024`, `1536x1024`，和 `1024x1536` 受 GPT 图像模型支持； `auto` 适用于允许自动调整大小的模型。对于 `dall-e-2`，请使用以下之一 `256x256`, `512x512`，或 `1024x1024`。对于 `dall-e-3`，请使用以下之一 `1024x1024`, `1792x1024`，或 `1024x1792`.
          ...
          - `tools: array of object { name, type, allowed_callers, 6 more }  or object { name, type, allowed_callers, 4 more }`
            ...
            - `Function object { name, type, allowed_callers, 6 more }`
              ...
              - `defer_loading: optional boolean`
                ...
                此 function 是否应被延后并通过 tool search 发现。

              - `description: optional string or null`

              - `output_schema: optional map[unknown] or null`

                描述此 function 工具的字符串输出中所编码 JSON 值的 JSON Schema。这并不描述 content 数组输出。
        ...
        - `ToolSearch object { type, description, execution, parameters }`
          ...
          用于延后工具的托管或 BYOT tool search 配置。
    ...
    - `Reasoning object { id, summary, type, 3 more }`
      ...
      对推理模型在生成响应时所用思维链的描述。
      如果你是手动管理上下文，请确保在对话的后续轮次中将这些项包含在提交给 `input` Responses API 的请求中。
      的请求中。
      ...
      - `encrypted_content: optional string or null`
        ...
        使用。 `encrypted_content` 中的
        `response.output_item.added` 可能不完整。这在
        为 `store` 时 `false` 或使用 Zero Data Retention 时尤其重要。
    ...
    - `McpCall object { id, arguments, name, 6 more }`
      ...
      - `approval_request_id: optional string or null`
        ...
        MCP 工具调用审批请求的唯一标识符。
        在后续输入中包含此值以批准或拒绝相应的工具调用。 `mcp_approval_response` input 中，以批准或拒绝相应的工具调用。
...
- `max_tool_calls: optional number or null`
  ...
  单个响应中可处理的内置工具调用总次数上限。该上限适用于所有内置工具调用，而非按单个工具计算。模型后续对工具的任何调用尝试都将被忽略。
...
- `previous_response_id: optional string or null`
  ...
  模型上一次响应的唯一 ID。使用它来
  创建多轮对话。详细了解
  [对话状态](/api/docs/guides/conversation-state)。不能与以下参数同时使用： `conversation`.
...
- `prompt: optional ResponsePrompt or null`
  ...
  - `variables: optional map[string or ResponseInputText or ResponseInputImage or ResponseInputFile] or null`
    ...
    可选的映射，用于在你的提示模板中替换变量。
    替换值可以是字符串，也可以是其他响应输入类型，例如图像或文件。
    响应输入类型，比如图像或文件。
...
- `prompt_cache_key: optional string or null`
  ...
  由 OpenAI 用于缓存相似请求的响应，以优化你的缓存命中率。替换 `user` 字段。 [了解更多](/api/docs/guides/prompt-caching).

- `prompt_cache_options: optional object { comparison_response_id, mode, prewarm, ttl }`

  提示缓存的选项。支持 `gpt-5.6` 及更高版本的模型。默认情况下，OpenAI 会自动选择一个隐式缓存断点。你可以在内容块上通过 `prompt_cache_breakpoint`。添加显式断点。每个请求最多可以写入四个断点。对于缓存匹配，OpenAI 会考虑会话中最多最新的 80 个断点，且没有内容块回溯限制。将 `mode` 通过 `explicit` 设置为 `ttl` 默认为 `30m`，这是当前唯一支持的值。详见 [prompt caching guide](/api/docs/guides/prompt-caching) 了解最新详情。

  - `comparison_response_id: optional string or null`

    用于在诊断 prompt cache 复用情况时作为对比的响应 ID。在该功能启用时，提供此字段会请求 prompt cache 诊断。

  - `mode: optional "implicit" or "explicit"`

    控制是否由 OpenAI 自动创建隐式缓存断点。默认为 `implicit`。当设置为 `implicit`，时，OpenAI 会创建一个隐式断点，并在请求中写入最多最近的三个显式断点。当设置为 `explicit`，时，OpenAI 不会创建隐式断点，并写入最多最近的四个显式断点。如果不存在显式断点，则该请求不会使用 prompt caching。
  ...
  - `prewarm: optional boolean`
    ...
    准备 prompt cache 但不生成输出。默认为 `false`。之一。设置为 `true`，会覆盖 `generate` 字段为 `false`.

  - `ttl: optional "30m"`

    请求所写入的每个隐式和显式缓存断点的最短保留时长。默认为 `30m`，这是当前唯一支持的值。后端可能会保留缓存条目更长时间。
...
- `prompt_cache_retention: optional "in_memory" or "24h" or null`
  ...
  prompt cache 的保留策略。设置为 `24h` 以启用扩展 prompt caching，使缓存前缀保持更长时间，最长可达 24 小时。 [了解更多](/api/docs/guides/prompt-caching#prompt-cache-retention).
  此字段表示最长保留策略，而
  `prompt_cache_options.ttl` 表示最短缓存保留时长。两者
  字段彼此独立，不会相互影响。
...
- `safety_identifier: optional string or null`
  ...
  这些 ID 应为字符串，用于唯一标识每个用户，最大长度为 64 个字符。我们建议对用户名或电子邮件地址进行哈希处理，以避免向我们发送任何身份信息。 [了解更多](/api/docs/guides/safety-best-practices#implement-safety-identifiers).

- `service_tier: optional ServiceTier or null`

  指定用于处理请求的服务等级。

  - 如果设置为 'auto'，则请求将使用项目设置中配置的服务等级进行处理。除非另有配置，否则项目将使用 'default'。
  - 如果设置为 'default'，则请求将按所选模型的标准定价和性能进行处理。
  - 如果设置为 '[flex](/api/docs/guides/flex-processing)'，则请求将使用 Flex Processing 服务层级处理。
  - 若要在请求级别启用 [Fast mode](/api/docs/guides/fast-mode) ，请为 Responses 或 Chat Completions 包含相应的 `service_tier=fast` 或 `service_tier=priority` 参数。响应将显示 `service_tier=priority` ，无论你是否在请求中指定了 `service_tier=fast` 或 `priority` 。
  - 如果设置为 'ultrafast'，则请求将使用受访问控制的 Ultrafast Processing 服务层级处理。该层级目前可用于 `gpt-5.6-sol`；通过该层级返回的响应将显示 `service_tier=ultrafast`.
  - 当未设置时，默认行为为 'auto'。

  当设置了 `service_tier` 参数时，响应主体将包含基于实际用于处理该请求的处理模式的 `service_tier` 值。此响应值可能与该参数中设置的值不同。
...
- `store: optional boolean or null`
  ...
  API 进行检索。
  省略时默认为 true。
  如果设置为 true，响应数据将被存储至少 30 天，但须遵守 [数据保留例外情况](/api/docs/guides/your-data#v1responses).
...
- `stream: optional boolean or null`
  ...
  请参阅下文 [流式部分](/api/reference/resources/responses/streaming-events)
  了解更多信息。

- `stream_options: optional object { include_obfuscation }  or null`

  流式响应的相关选项。仅当你设置了 `stream: true`.

  - `include_obfuscation: optional boolean`

    为 true 时，将启用流混淆。流混淆会向
    字段添加随机字符 `obfuscation` 流式增量事件上的某个字段，以
    规范化负载大小，作为对某些侧信道攻击的缓解措施。
    这些混淆字段默认包含在内，但会引入少量
    到数据流的开销。你可以设置 `include_obfuscation` 通过
    为 false 以在信任你的应用与
...
- `text: optional ResponseTextConfig`
  ...
  - `format: optional ResponseFormatTextConfig`
    ...
    **不建议用于 gpt-4o 及更新的模型：**
    ...
    确保模型生成的消息是合法的 JSON。对于支持它的模型，建议使用 `json_schema`
    。
    ...
    - `ResponseFormatTextJSONSchemaConfig object { name, schema, type, 2 more }`
      ...
      - `strict: optional boolean or null`
        ...
        是否在生成输出时启用严格的模式遵循。
        如果设置为 true，模型将始终遵循
        字段中定义的精确模式。仅在 `schema` 时支持 JSON Schema 的一个子集。如需了解更多信息，请阅读
        `strict` 时 `true`。指南。 [Structured Outputs
        指南](/api/docs/guides/structured-outputs).

    - `ResponseFormatJSONObject object { type }`

      JSON 对象响应格式。一种较旧的生成 JSON 响应的方法。
      使用 `json_schema` 建议用于支持该功能的模型。请注意，
...
- `tools: optional array of object { name, parameters, strict, 6 more }  or object { type, vector_store_ids, filters, 2 more }  or object { type }  or 13 more`
  ...
  - **内置工具**：由 OpenAI 提供的工具，用于扩展模型的
    ...
    [内置工具](/api/docs/guides/tools).
  - **MCP Tools**：通过自定义 MCP 服务器或 Google Drive、SharePoint 等预定义连接器与第三方系统集成。了解更多关于
...
- `top_logprobs: optional number or null`
  ...
  一个介于 0 到 20 之间的整数，用于指定在每个 token 位置返回的最多最可能 token 数量，每个 token 带有对应的
  对数概率。在某些情况下，返回的 token 数量可能少于
  请求的数量。
  请求的数量。
...
- `top_p: optional number or null`
  ...
  一种称为核采样的温度采样替代方法，
  模型只考虑概率质量排名前 top_p 的 token 结果。
  因此 0.1 表示只考虑概率质量排名前 10% 的 token。
  被考虑。
...
- `user: optional string`
  ...
  你最终用户的稳定标识符。
  用于通过更好地对相似请求进行分桶来提高缓存命中率，并帮助OpenAI检测和防止滥用。 [了解更多](/api/docs/guides/safety-best-practices#implement-safety-identifiers).
...
- `Response object { id, created_at, error, 33 more }`
  ...
  - `incomplete_details: object { reason }  or null`
    ...
    - `reason: optional "max_output_tokens" or "max_messages" or "content_filter" or "steered"`
      ...
      响应未完成的原因。 `steered` 表示
      响应在某个
      WebSocket `response.steer` 事件之后的安全输出边界处停止。服务端随后可以使用排队的输入自动创建
      一个后续响应。
  ...
  - `output: array of ResponseOutputItem`
    ...
    - 数组中项的长度和顺序取决于 `output` 相关因素
      模型的响应。
    - 不要直接访问数组中的第一个项并 `output` 假定它是
      一条包含由 `assistant` 模型生成内容的
      消息，你可以考虑使用 `output_text` 属性（在支持的
      SDK中可用）。
  ...
  - `conversation: optional object { id }  or null`
    ...
    此 response 所属的对话。此 response 的输入项和输出项已自动添加到此对话中。

    - `id: string`

      此 response 所关联的对话的唯一 ID。
  ...
  - `moderation: optional object { input, output }  or null`
    ...
    response 输入和输出的审核结果，前提是请求了已审核的补全。
    ...
    - `input: object { categories, category_applied_input_types, category_scores, 3 more }  or object { code, message, type }`
      ...
      - `ModerationResult object { categories, category_applied_input_types, category_scores, 3 more }`
        ...
        - `type: "moderation_result"`
          ...
          对象类型，通常为 `moderation_result` ，表示审核成功的结果。
  ...
  - `output_text: optional string or null`
    ...
    SDK专属便捷属性，包含来自所有输出项的聚合文本输出（如有）
    来自所有 `output_text` 数组中的项， `output` 如有任何项存在。
  ...
  - `prompt_cache_options: optional object { mode, ttl, comparison_response_id }`
    ...
    应用于该响应的提示缓存选项。适用于 `gpt-5.6` 及更高版本模型。
