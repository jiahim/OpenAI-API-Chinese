...
[tools](/api/docs/guides/tools) 例如 [网页搜索](/api/docs/guides/tools-web-search)
...
- `access_programs: optional object { cyber }`
  ...
  - `cyber: optional "standard" or "daybreak_blue" or "daybreak_red"`
    ...
    用于此请求的 Cyber 访问计划。支持的值有 `standard`, `daybreak_blue`，以及 `daybreak_red`。如果省略，API 会根据模型的 Cyber 层级以及你的组织和项目访问权限来解析该计划，但需遵守模型特定的资格限制。默认情况下，没有 Cyber 层级的模型使用 Standard。Blue 层级的模型在获得授权时使用 Daybreak Blue；否则，除非模型要求 Daybreak 访问权限，否则回退到 Standard。Red 层级的模型使用 Daybreak Red 并需要授权。需要但不可用的 Daybreak 访问权限的请求会返回 403。隐式的 Standard 回退在响应的 access_programs 字段中表示为 null，而不是显式的 Standard 选择。
...
- `conversation: optional string or ResponseConversationParam or null`
  ...
  此响应所属的对话。该对话中的项目会被前置到 `input_items` 中，用于本次响应请求。
  此响应的输入项和输出项会在该响应完成后自动添加到此对话中。
...
- `include: optional array of ResponseIncludable or null`
  ...
  - `reasoning.encrypted_content`：在推理项输出中包含推理 token 的加密版本。这使得在无状态地使用 Responses API 时（例如当 `store` 参数设置为 `false`，或当组织已加入零数据留存计划时），推理项可以用于多轮对话。
...
- `input: optional string or array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more`
  ...
  - `InputItemList = array of EasyInputMessage or object { content, role, status, type }  or ResponseOutputMessage or 30 more`
    ...
    提供给模型的一个或多个输入项的列表，包含
    不同的内容类型。

    - `EasyInputMessage object { content, role, phase, type }`

      发送给模型的消息输入，带有指示指令遵循
      层级的角色。使用 `developer` 或 `system` 角色给出的指令
      优先于使用 `user` role。带有该
        ...
        - `ResponseInputMessageContentList = array of ResponseInputContent`
          ...
          - `ResponseInputImage object { detail, type, file_id, 2 more }`
            ...
            - `image_url: optional string or null`
              ...
              要发送给模型的图像 URL。可以是完全限定的 URL，或在 data URL 中进行 base64 编码的图像。
          ...
          - `ResponseInputFile object { type, detail, file_data, 4 more }`
            ...
            - `detail: optional "auto" or "low" or "high"`
              ...
              发送给模型的文件的细节级别。使用 `auto` 以让系统选择细节级别；对于 GPT-5.6 及更高版本的模型， `auto` 使用高质量渲染，这可能会增加输入 token 的使用量。使用 `low` 以使用较低成本的渲染，或使用 `high` 以更高质量渲染文件。默认为 `auto`.
      ...
      - `phase: optional "commentary" or "final_answer" or null`
        ...
        对于类似 `gpt-5.3-codex` 及更高的模型，在发送后续请求时，请在所有助手消息上保留并重新发送
    ...
    - `Message object { content, role, status, type }`
      ...
      - `status: optional "in_progress" or "completed" or "incomplete"`
        ...
        项目的状态，取值为以下之一 `in_progress`, `completed`，或
    ...
    - `ComputerCall object { id, call_id, pending_safety_checks, 4 more }`
      ...
      - `action: optional ComputerAction`
        ...
        - `Screenshot object { type }`
          ...
          - `type: "screenshot"`
            ...
            指定事件类型。对于 screenshot 操作，此属性始终设置为 `screenshot`.
      ...
      - `actions: optional ComputerActionList`
        ...
        扁平化的批量操作， `computer_use`。每个操作包含一个
        `type` 判别字段和操作专属字段。
    ...
    - `FunctionCallOutput object { output, type, id, 5 more }`
      ...
      - `id: optional string or null`
        ...
        函数工具调用输出的唯一 ID。当此项通过 API 返回时会填充。
      ...
      - `status: optional "in_progress" or "completed" or "incomplete" or null`
        ...
        条目的状态。可选值为 `in_progress`, `completed`，或 `incomplete`。当项目通过 API 返回时填充。
    ...
    - `ToolSearchOutput object { tools, type, id, 3 more }`
      ...
      - `tools: array of object { name, parameters, strict, 6 more }  or object { type, vector_store_ids, filters, 2 more }  or object { type }  or 13 more`
        ...
        - `FileSearch object { type, vector_store_ids, filters, 2 more }`
          ...
          用于从已上传文件中搜索相关内容的工具。详细了解 [文件搜索 工具](/api/docs/guides/tools-file-search).
          ...
          - `ranking_options: optional object { hybrid_search, ranker, score_threshold }`
            ...
            - `hybrid_search: optional object { embedding_weight, text_weight }`
              ...
              在使用混合搜索时，用于控制倒数排名融合在语义嵌入匹配与稀疏关键词匹配之间权衡的权重。
            ...
            - `score_threshold: optional number`
              ...
              文件搜索的分数阈值，介于 0 到 1 之间的数值。越接近 1 的数值将尝试仅返回最相关的结果，但可能返回的结果数量较少。
        ...
        - `WebSearch object { type, external_web_access, filters, 2 more }`
          ...
          - `external_web_access: optional boolean`
            ...
            允许 网页搜索进行实时互联网访问。省略时默认为 true。当值为 false 时，网页搜索工具以离线/仅缓存模式运行，不会获取新的外部内容。
          ...
          - `user_location: optional object { city, country, region, 2 more }  or null`
            ...
            美国。若要避免此回退，请传递 `{"type": "approximate"}` 不含
            location 字段的方式。若要本地化结果，请提供相关的位置字段。
            ...
            - `country: optional string or null`
              ...
              两个字母的 [ISO 国家代码](https://en.wikipedia.org/wiki/ISO_3166-1) 用户区域，例如。 `US`.

            - `region: optional string or null`

              用户区域的自由文本输入，例如 `California`.

            - `timezone: optional string or null`

              该 [IANA 时区](https://timeapi.io/documentation/iana-timezones) 用户区域，例如。 `America/Los_Angeles`.
        ...
        - `Mcp object { server_label, type, allowed_callers, 9 more }`
          ...
          - `tunnel_id: optional string`
            ...
            要使用的 Secure MCP Tunnel ID，以代替直接服务器 URL。
        ...
        - `ImageGeneration object { type, action, background, 9 more }`
          ...
          - `size: optional string or "1024x1024" or "1024x1536" or "1536x1024" or "auto"`
            ...
            生成图像的尺寸。对于 `gpt-image-2`, `gpt-image-2-2026-04-21`, `gpt-image-2.5-sunburst`, `gpt-image-2.5-sunburst-2026-09-08`, `gpt-image-2.5-flare`，以及 `gpt-image-2.5-flare-2026-09-08`，支持以字符串形式指定任意分辨率，例如 `WIDTHxHEIGHT` 字符串，例如 `1536x864`。宽度和高度都必须能被 16 整除，并且所请求的宽高比必须介于 1:3 和 3:1 之间。高于 `2560x1440` 的分辨率属于实验性，并且最大支持的分辨率为 `3840x2160`。所请求的尺寸还必须满足模型当前的像素和边长限制。GPT 图像模型支持的标准尺寸为 `1024x1024`, `1536x1024`，以及 `1024x1536` 由 GPT 图像模型支持； `auto` 适用于允许自动调整尺寸的模型。对于 `dall-e-2`，请使用以下其中之一： `256x256`, `512x512`，或 `1024x1024`。对于 `dall-e-3`，请使用以下其中之一： `1024x1024`, `1792x1024`，或 `1024x1792`.
          ...
          - `tools: array of object { name, type, allowed_callers, 6 more }  or object { name, type, allowed_callers, 4 more }`
            ...
            - `Function object { name, type, allowed_callers, 6 more }`
              ...
              - `output_schema: optional map[unknown] or null`
                ...
                用于描述此函数工具中以字符串编码的 JSON 值的 JSON Schema。这不描述 content 数组形式的输出。
        ...
        - `WebSearchPreview object { type, search_content_types, search_context_size, user_location }`
          ...
          此工具会在网络上搜索相关结果以用于回复中。了解更多关于 [网页搜索工具](/api/docs/guides/tools-web-search).
          ...
          - `user_location: optional object { type, city, country, 2 more }  or null`
            ...
            用户的大致位置。如果省略或为 null，则默认为美国。若要避免此回退行为，请传入 `{"type": "approximate"}` 时不带 location 字段。若要本地化结果，请提供相关的 location 字段。
    ...
    - `Reasoning object { id, summary, type, 3 more }`
      ...
      。如果你是手动管理上下文，请确保在后续对话轮次中将这些项传回 `input` 到 Responses API
      中
      ...
      - `encrypted_content: optional string or null`
        ...
        推理条目的加密内容。默认情况下填充该字段，针对
        以及 WebSocket `POST /v1/responses` 请求返回的推理条目
        `response.create` 。

        在流式传输时，请使用已完成的推理条目及其
        `encrypted_content` 中的 `response.output_item.done` 事件
        后续请求的内容。 `encrypted_content` in
        `response.output_item.added` 可能不完整。在以下情况下这一点尤其
        重要：当 `store` is `false` 或使用 Zero Data Retention 时。
    ...
    - `Compaction object { encrypted_content, type, id }`
      ...
      由 接口 [`v1/responses/compact` API](/api/reference/resources/responses/methods/compact).
    ...
    - `ShellCallOutput object { call_id, output, type, 4 more }`
      ...
      - `output: array of ResponseFunctionShellCallOutputContent`
        ...
        - `stderr: string`
          ...
          捕获的该 shell 调用的 stderr 输出。

        - `stdout: string`

          捕获的该 shell 调用的 stdout 输出。

      - `type: "shell_call_output"`

        该项目的类型。始终为 `shell_call_output`.
    ...
    - `ApplyPatchCallOutput object { call_id, status, type, 3 more }`
      ...
      - `type: "apply_patch_call_output"`
        ...
        该项目的类型。始终为 `apply_patch_call_output`.
    ...
    - `McpApprovalRequest object { id, arguments, name, 2 more }`
      ...
      - `type: "mcp_approval_request"`
        ...
        该项目的类型。始终为 `mcp_approval_request`.
    ...
    - `McpApprovalResponse object { approval_request_id, approve, type, 2 more }`
      ...
      - `type: "mcp_approval_response"`
        ...
        该项目的类型。始终为 `mcp_approval_response`.
    ...
    - `McpCall object { id, arguments, name, 6 more }`
      ...
      - `status: optional "in_progress" or "completed" or "incomplete" or 2 more`
        ...
        该工具调用的状态。其值为 `in_progress`, `completed`, `incomplete`, `calling`，或 `failed`.
...
- `instructions: optional string or null`
  ...
  的指令不会延续到下一次响应。这样可以轻松
  在新的响应中替换系统（或开发者）消息。

- `max_output_tokens: optional number or null`

  可在一次响应中生成的最大 token 数上限，包括可见输出 token 和 [推理 token](/api/docs/guides/reasoning).

- `max_tool_calls: optional number or null`

  在一次响应中可处理的内置工具调用总次数上限。该上限适用于所有内置工具调用，而非单个工具。模型对工具的后续调用尝试将被忽略。
...
- `model: optional ResponsesModel`
  ...
  提供多种不同能力、性能
  特性和价位的模型。参阅 [模型指南](/api/docs/models)
  以浏览和比较可用的模型。
...
- `prompt_cache_key: optional string or null`
  ...
  由 OpenAI 用于缓存类似请求的响应,以优化你的缓存命中率。取代 `user` 字段。 [了解更多](/api/docs/guides/prompt-caching).

- `prompt_cache_options: optional object { comparison_response_id, mode, prewarm, ttl }`

  提示词缓存的选项。支持 `gpt-5.6` 及更高版本模型。默认情况下,OpenAI 会自动选择一个隐式缓存断点。你可以使用 `prompt_cache_breakpoint`.在内容块上添加显式断点。每个请求最多可以写入四个断点。在缓存匹配时,OpenAI 会考虑对话中最近的最多 80 个断点,且没有内容块回溯限制。将 `mode` 通过 `explicit` 设置为 `ttl` 以禁用隐式断点。 `30m`，默认为 [,这是当前唯一支持的值。参见](/api/docs/guides/prompt-caching) 提示词缓存指南。

  - `comparison_response_id: optional string or null`

    在诊断提示词缓存复用时要比较的响应 ID。提供此字段会在启用该功能时请求提示词缓存诊断。

  - `mode: optional "implicit" or "explicit"`

    控制 OpenAI 是否自动创建隐式缓存断点。默认为 `implicit`.使用 `implicit`,OpenAI 会创建一个隐式断点,并在请求中写入最多最近的三个显式断点。使用 `explicit`,OpenAI 不会创建隐式断点,并写入最多最近的四个显式断点。如果没有显式断点,则该请求不使用提示词缓存。
  ...
  - `prewarm: optional boolean`
    ...
    在不生成输出的情况下预热提示词缓存。默认为 `false`。当设置为 `true`,会覆盖 `generate` 字段为 `false`.

  - `ttl: optional "30m"`

    应用于请求所写入的每个隐式和显式缓存断点的最短生存时间。默认为 `30m`，这是当前唯一受支持的值。后端可能会将缓存条目保留更长时间。
...
- `reasoning: optional Reasoning or null`
  ...
  - `summary: optional "auto" or "concise" or "detailed" or null`
    ...
    `concise` 支持 `computer-use-preview` models 以及之后的所有 reasoning 模型 `gpt-5`.
...
- `safety_identifier: optional string or null`
  ...
  这些 ID 应为字符串，唯一标识每个用户，最大长度为 64 个字符。建议对其用户名或电子邮件地址进行哈希处理，以避免向我们发送任何身份识别信息。 [了解更多](/api/docs/guides/safety-best-practices#implement-safety-identifiers).
...
- `service_tier: optional ServiceTier or null`
  ...
  当 `service_tier` 参数被设置时，响应体将包含 `service_tier` 值基于实际用于处理该请求的处理模式。此响应值可能与参数中设置的值不同。
...
- `stream: optional boolean or null`
  ...
  流式传输给客户端，方式为使用 [服务端发送事件](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events#Event_stream_format).
...
- `stream_options: optional object { include_obfuscation }  or null`
  ...
  - `include_obfuscation: optional boolean`
    ...
    开销。如果你的应用程序与 OpenAI API 之间的网络链路可信，你可以将 `include_obfuscation` 通过
    设置为 false 以优化带宽。
    你的应用程序与 该公司 接口 之间。
    ...
    **不建议用于 gpt-4o 及更新模型：**
    ...
    - `ResponseFormatTextJSONSchemaConfig object { name, schema, type, 2 more }`
      ...
      - `strict: optional boolean or null`
        ...
        如果设置为 true，模型将始终遵循在
        字段中定义的精确 schema。仅当 `schema` 为 true 时才支持 JSON Schema 的一个子集。要了解更多信息，请参阅
        `strict` is `true`。指南 [结构化输出
...
- `tool_choice: optional ToolChoiceOptions or ToolChoiceAllowed or ToolChoiceTypes or 6 more`
  ...
  模型在生成响应时应如何选择要使用的工具（或多个工具）。请参阅
  参数了解如何指定模型可调用的工具 `tools` （但原文该片段被切分为独立项，需结合上下文翻译）参数以了解如何指定模型可以调用的工具
  。
  ...
  - `ToolChoiceTypes object { type }`
    ...
    - `type: "file_search" or "web_search_preview" or "computer" or 5 more`
      ...
      模型应使用的 托管工具的类型。了解更多关于
      [内置工具的信息](/api/docs/guides/tools).
...
- `tools: optional array of object { name, parameters, strict, 6 more }  or object { type, vector_store_ids, filters, 2 more }  or object { type }  or 13 more`
  ...
  - **内置工具**：由 OpenAI 提供的工具，用于扩展模型
    的能力，例如 [网页搜索](/api/docs/guides/tools-web-search)
...
- `user: optional string`
  ...
  此字段正在被 `safety_identifier` 和 `prompt_cache_key`。取代。请使用 `prompt_cache_key` 以保持缓存优化。
  用于标识最终用户的稳定标识符。
...
- `Response object { id, access_programs, created_at, 34 more }`
  ...
  - `incomplete_details: object { reason }  or null`
    ...
    - `reason: optional "max_output_tokens" or "max_messages" or "content_filter" or "steered"`
      ...
      响应未完成的原因。 `steered` 表示
      响应在一个 WebSocket
      WebSocket `response.steer` 事件之后的安全输出边界处停止。然后服务端可以使用排队中的输入自动创建一个后续响应。
      自动创建一个后续响应，其中包含已排队的输入。
  ...
  - `output: array of ResponseOutputItem`
    ...
    - 数组中项的长度和顺序 `output` 数组内容取决于
      模型的响应。
    - 你可以考虑使用以下属性， `output` 而非直接访问
      数组中的第一项 `assistant` 并假设它是一条包含模型生成内容的
      消息：在受支持的 SDK 中， `output_text` 可以使用该属性
      。
    ...
    - `ShellCallOutput object { id, call_id, max_output_length, 5 more }`
      ...
      - `max_output_length: number or null`
        ...
        shell 命令输出的最大长度。该字段由模型生成，需要与原始输出一起回传。
  ...
  - `completed_at: optional number or null`
    ...
    仅在状态为以下值时出现 `completed`.

  - `conversation: optional object { id }  or null`

    此 Response 所属的会话。本次 Response 的输入项和输出项已自动添加到该会话中。
  ...
  - `moderation: optional object { input, output }  or null`
    ...
    对 Response 输入和输出的审核结果（如果请求了已审核的 completions）。
    ...
    - `input: object { categories, category_applied_input_types, category_scores, 3 more }  or object { code, message, type }`
      ...
      - `ModerationResult object { categories, category_applied_input_types, category_scores, 3 more }`
        ...
        - `categories: map[boolean]`
          ...
          一个由审核类别映射到布尔值的字典；若输入被该类别标记则为 True。
  ...
  - `output_text: optional string or null`
    ...
    SDK 专属便捷属性，包含来自所有输出项的汇总文本
    来自所有 `output_text` 数组中的 `output` 项（若存在）。
  ...
  - `usage: optional ResponseUsage`
    ...
    - `input_tokens_details: object { cache_write_tokens, cached_tokens }`
      ...
      - `cached_tokens: number`
        ...
        从缓存中读取的 token 数量。
        [详细了解 prompt 缓存](/api/docs/guides/prompt-caching).
