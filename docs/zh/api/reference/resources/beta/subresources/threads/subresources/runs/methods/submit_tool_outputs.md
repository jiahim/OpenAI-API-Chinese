...
当一次运行具有 `status: "requires_action"` 和 `required_action.type` 是 `submit_tool_outputs`，此端点可用于在工具调用全部完成后提交其输出。所有输出必须在单个请求中提交。
...
- `tool_outputs: array of object { output, tool_call_id }`
  ...
  - `tool_call_id: optional string`
    ...
    运行对象中 `required_action` 对象内要提交输出的工具调用的 ID。

- `stream: optional boolean or null`

  如果 `true`，则以服务器发送事件的形式返回运行期间发生的事件流，当运行进入终止状态并附带 `data: [DONE]` 消息时结束。
...
- `Run object { id, assistant_id, cancelled_at, 24 more }`
  ...
  - `incomplete_details: object { reason }  or null`
    ...
    关于运行不完整的详细信息。如果运行并非不完整，则为 `null` 。

    - `reason: optional "max_completion_tokens" or "max_prompt_tokens"`

      运行不完整的原因。此字段将指向运行过程中达到的特定令牌限制。
  ...
  - `instructions: string`
    ...
    用于此运行的指令，即 [assistant](/docs/api-reference/assistants) 用于此运行的指令。
  ...
  - `model: string`
    ...
    该模型 [assistant](/docs/api-reference/assistants) 用于此运行的指令。
    ...
    - `submit_tool_outputs: object { tool_calls }`
      ...
      - `tool_calls: array of RequiredActionFunctionToolCall`
        ...
        - `id: string`
          ...
          工具调用的 ID。当你在使用 [提交工具输出以运行](/docs/api-reference/runs/submitToolOutputs) 端点时，必须引用此 ID。
  ...
  - `response_format: AssistantResponseFormatOption or null`
    ...
    设置 `{ "type": "json_schema", "json_schema": {...} }` 可启用结构化输出，确保模型匹配你提供的 JSON 架构。更多信息请参阅 [结构化输出指南](/docs/guides/structured-outputs).

    设置 `{ "type": "json_object" }` 可启用 JSON 模式，确保模型生成的消息是有效的 JSON。

    **重要提示：** 使用 JSON 模式时，你 **必须** 通过系统或用户消息自行指示模型生成 JSON。否则，模型可能会生成无休止的空格流，直到生成达到 token 限制，导致请求长时间运行且看似“卡住”。另请注意，如果 `finish_reason="length"`，则消息内容可能被部分截断，这表示生成超过了 `max_tokens` 或对话超出了最大上下文长度。
    ...
    - `ResponseFormatJSONSchema object { json_schema, type }`
      ...
      - `json_schema: object { name, description, schema, strict }`
        ...
        - `strict: optional boolean or null`
          ...
          是否在生成输出时启用严格模式遵循。
          如果设为 true，模型将始终遵循定义的确切模式
          中的 `schema` 字段。当
          `strict` 时，仅支持 JSON Schema 的一个子集 `true`。如需了解更多，请阅读 [结构化输出
  ...
  - `thread_id: string`
    ...
    用于执行此运行的 [thread](/docs/api-reference/threads) ，作为此运行的一部分被执行。
  ...
  - `tools: array of CodeInterpreterTool or FileSearchTool or FunctionTool`
    ...
    工具列表 [assistant](/docs/api-reference/assistants) 用于此运行的指令。
    ...
    - `FileSearchTool object { type, file_search }`
      ...
      - `file_search: optional object { max_num_results, ranking_options }`
        ...
        - `max_num_results: optional number`
          ...
          文件搜索工具应输出的最大结果数。对于 `gpt-4*` 模型默认值为 5，对于 `gpt-3.5-turbo`. 此数字应在 1 到 50（含）之间。
        ...
        - `ranking_options: optional object { score_threshold, ranker }`
          ...
          请参阅 [文件搜索工具文档](/docs/assistants/tools/file-search#customizing-file-search-settings) 。
    ...
    - `FunctionTool object { function, type }`
      ...
      - `function: FunctionDefinition`
        ...
        - `parameters: optional FunctionParameters`
          ...
          函数接受的参数，以 JSON Schema 对象描述。请参阅 [指南](/docs/guides/function-calling) 以获取示例，以及 [JSON Schema 参考](https://json-schema.org/understanding-json-schema/) 了解该格式的文档。

          省略 `parameters` 定义了一个参数列表为空的函数。

        - `strict: optional boolean or null`

          是否在生成函数调用时启用严格模式以严格遵守模式。如果设为 true，模型将遵循 `parameters` 字段。当 `strict` 时，仅支持 JSON Schema 的一个子集 `true`。中定义的精确模式。了解更多关于结构化输出的信息，请参阅 [函数调用指南](/docs/guides/function-calling).
  ...
  - `truncation_strategy: object { type, last_messages }  or null`
    ...
    控制运行前线程将被截断的方式。使用此项控制运行的初始上下文窗口。

    - `type: "auto" or "last_messages"`

      用于线程的截断策略。默认值为 `auto`。如果设为 `last_messages`，线程将被截断为线程中最近的 n 条消息。当设为 `auto`，时，线程中间的消息将被丢弃以适配模型的上下文长度， `max_prompt_tokens`.
